...
def test_deterministic_json_serialization():
    ...
    # Get the log hash which uses deterministic JSON serialization
    log_hash = math.get_log_hash()
    
    # Manually serialize the log to verify deterministic serialization
    ...
    # Verify that the serialization is consistent across runs
    log2 = []
...
def test_combined_operation_overflow():
    ...
    # Test overflow in nested operations: _safe_add(_safe_mul(a, b), c)
    # Use values that will cause overflow in multiplication
    a = BigNum128(BigNum128.MAX_VALUE // 2)
    b = BigNum128(BigNum128.SCALE * 3)  # 3.0
    c = BigNum128(BigNum128.SCALE)      # 1.0
    
    try:
        # This should trigger overflow in the multiplication step
    ...
    # Test another combination that should overflow in addition
    max_val = BigNum128(BigNum128.MAX_VALUE)
    one = BigNum128(BigNum128.SCALE)
    
    try:
        # max_val + 1 should overflow
...
def test_cross_module_integration():
    ...
    # This test verifies that CertifiedMath works correctly with LogContext
    # and that PQC CID and quantum metadata are properly propagated
    
    log = []
    
    # Use LogContext for thread-safe handling
    with LogContext(log) as ctx_log:
        math = CertifiedMath(ctx_log)
        
        # Perform operations with PQC CID and quantum metadata
        a = BigNum128.from_string("1.234567")
        b = BigNum128.from_string("2.345678")
        
        quantum_metadata = {
            "phase": "V13 Phase 3",
            "entropy_level": "QRNG-VDF",
            "test_id": "cross_module_001"
        }
        ...
        # Verify that all log entries have the correct PQC CID and quantum metadata
        for entry in log:
...
def test_audit_trail_completeness():
    ...
    # Perform various operations including edge cases
    a = BigNum128.from_string("1.234567")
    b = BigNum128.from_string("2.345678")
    ...
    # Operation that triggers exception (should still be logged)
    try:
        zero = BigNum128(0)
        result2 = math.div(a, zero, pqc_cid="AUDIT_001", quantum_metadata={"op": "div_by_zero"})
    except MathValidationError:
        # Exception is expected, but the operation should still be logged
        pass
    
    # Verify log completeness
    # Note: Division by zero exception is caught and doesn't add to the log count
...
def test_reference_hash_verification():
    ...
    # Perform a known operation sequence
    log = []
    math = CertifiedMath(log)
    ...
    # In a real audit scenario, this would be compared against a stored reference hash
    ...
    # Verify consistency by repeating the same operations
    log2 = []
    math2 = CertifiedMath(log2)
...
def test_stress_performance_determinism():
    ...
    # Perform a large number of operations to test determinism under load
    log1 = []
    ...
    # Repeat with identical operations
    log2 = []
    math2 = CertifiedMath(log2)
...
def test_multi_threaded_determinism():
    ...
    # Simulate multi-threaded behavior using separate log contexts
    logs = []
    maths = []
    
    # Create 5 separate contexts (simulating 5 threads)
    for i in range(5):
        log = []
        math = CertifiedMath(log)
        logs.append(log)
    ...
    # Perform identical operations in each "thread" to verify deterministic behavior
    ...
    # Verify that each thread's log is consistent and isolated
    hashes = []
    for i, log in enumerate(logs):
        # Each log should have 2 entries
        ...
        # Compute hash for this thread
        sorted_log = sorted(log, key=lambda x: x.get('log_index', 0))
        serialized = json.dumps(sorted_log, sort_keys=True, separators=(',', ':'))
        hash_val = hashlib.sha256(serialized.encode('utf-8')).hexdigest()
        hashes.append(hash_val)
    
    # Verify that identical operations in different threads produce identical hashes
    # (First two threads should have identical operations and thus identical hashes)
...
def test_configuration_consistency():
    ...
    # Perform identical operations with original configuration
    log2 = []
    math2 = CertifiedMath(log2)
    ...
    # Results should be different due to different series precision
