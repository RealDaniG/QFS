...
def test_governance_advisory_logic():
    ...
    assert "Review: High-value request (>10k)" in signal["reasons"]
...
def test_bounty_advisory_logic():
    ...
    assert adv["payload"]["timestamp"] == ts
    signal = adv["payload"]["signal"]
    assert "Quality: Valid reference format detected" in signal["reasons"]
...
def test_social_advisory_logic():
    ...
    assert adv["payload"]["timestamp"] == ts
    assert "Urgency: Critical keywords (scam)" in adv["payload"]["signal"]["reasons"]
