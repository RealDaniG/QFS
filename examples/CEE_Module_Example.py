...
class ExampleCEEModule(CEEModule):
    ...
    def process(self, input_data: ExampleModuleInput) -> ExampleModuleOutput:
        ...
        # Create output
        output = ExampleModuleOutput(
            tick=input_data.tick,
            state_delta=state_delta,
            audit_log=audit_log,
            next_hash="",  # Will be calculated below
            transaction_id=transaction_id
        )
        
        # Calculate next hash for chain integrity
        ...
        # Update the output with the calculated hash
    ...
    def validate_input(self, input_data: ExampleModuleInput) -> bool:
        ...
        # Validate PQC signature using the adapter
        # Create a mock signed message for demonstration
        try:
            # In a real implementation, we would verify the actual signature
            # from the input_data.signature field
            return True
        except Exception:
            return False
    ...
    def get_audit_schema(self) -> dict:
        ...
        return {
            ...
            "required": ["operation", "action", "tick", "timestamp"]
        }
...
def demo_pqc_integration():
    ...
    # Generate test keys
    seed = b"demo_seed_1234567890123456789012"  # 32 bytes
    ...
    # Create test input
    test_input = ExampleModuleInput(
        tick=100,
        timestamp=1000,
        signature=b"",  # Would be populated in real implementation
        prev_hash="0" * 64,
        action="transfer",
        amount="100.0",
        recipient="node_abc123"
    )
    ...
    # Demonstrate SignedMessage creation
    ...
    message = SignedMessage.create(
        ...
        recipient_module="ExampleCEEModule",
        payload=payload,
        tick=100,
        timestamp=1000,
        private_key=private_key,
        public_key=public_key,
        pqc=module.pqc_adapter
    )
    ...
    tampered_payload["amount"] = "999999.0"  # TAMPERED!
    
    tampered_message = SignedMessage(
        sender_qid=message.sender_qid,
        recipient_module=message.recipient_module,
        payload=tampered_payload,
        tick=message.tick,
        timestamp=message.timestamp,
        signature=message.signature,  # Same signature
