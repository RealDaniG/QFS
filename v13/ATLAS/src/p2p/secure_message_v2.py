...
class SecureMessageV2:
    ...
    """
    def __init__(self, ciphertext: bytes, nonce: bytes, sequence_num: int, timestamp: float):
        self.ciphertext = ciphertext
        self.nonce = nonce
        self.sequence_num = sequence_num
        self.timestamp = timestamp
        # Commit to the content and ordering immediately
        self.message_hash = hashlib.sha256(
            ciphertext + nonce + sequence_num.to_bytes(8, 'big')
        ).digest()
    ...
    def serialize(self) -> bytes:
        ...
        return json.dumps({
            ...
            "ts": self.timestamp,
            "hash": base64.b64encode(self.message_hash).decode('utf-8')
        }).encode('utf-8')
...
class MessageSequenceManager:
    ...
    """
    def __init__(self):
        # Maps peer_id -> (last_sent_seq, last_received_seq)
    ...
    def validate_incoming_sequence(self, peer_id: str, seq_num: int) -> bool:
        ...
        # Check for replay (strict monotonicity)
        ...
        if seq_num > last_recv + 1:
            ...
            # We allow gaps for now (UDP-like behavior or dropped packets), 
            # but log them. Strict mode would reject or buffer.
