...
def test_transactions_rejects_invalid_payload(client: TestClient) -> None:
    # amount must be > 0 and required fields must exist; FastAPI/Pydantic should emit 4xx.
    response = client.post(
        "/api/v1/transactions/",
        json={"receiver": "user999", "amount": -1},
...
def test_transactions_minimal_happy_path(client: TestClient) -> None:
    payload = {
        "sender": "ignored_by_server",
        "receiver": "user_456",
        "amount": 1.25,
        "asset": "QFS",
        "metadata": {"purpose": "phase-c"},
    }
...
def test_transactions_forbidden_case(client: TestClient) -> None:
    # Create a transaction owned by others directly in the in-memory processor,
    # then attempt to fetch as the default current_user (user123). This should 403.
    from src.api.routes import transactions as tx_routes

    foreign_tx = tx_routes.transaction_processor.create_transaction(
        sender="user_other",
        receiver="user_else",
        amount=2.0,
        asset="QFS",
        metadata={"purpose": "phase-c-forbidden"},
    )
