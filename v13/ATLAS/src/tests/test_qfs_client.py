...
from src.qfs_types_models import Transaction, DeterminismReport
...
async def test_transaction_validation(qfs_client):
    ...
    with pytest.raises(ValueError, match="operation_type"):
        tx = Transaction(
            transaction_id=None, operation_type="", creator_id="user_123", data={}
        )
        await qfs_client.submit_transaction(tx)
    with pytest.raises(ValueError, match="creator_id"):
        tx = Transaction(
            transaction_id=None, operation_type="test", creator_id="", data={}
        )
        await qfs_client.submit_transaction(tx)
