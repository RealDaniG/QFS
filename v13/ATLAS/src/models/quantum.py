...
class QuantumSignature(BaseModel):
    ...
    timestamp: Optional[str] = Field(
        None,
        description="Signature timestamp in ISO format"
    )
...
class QuantumSignatureVerify(BaseModel):
    ...
    algorithm: Optional[str] = Field(
        "QFS-QUANTUM-256",
        description="Signature algorithm (default: QFS-QUANTUM-256)"
    )
...
class EntangledPair(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(
        default_factory=dict,
        description="Additional entanglement metadata"
    )
...
class QuantumCircuitRequest(BaseModel):
    ...
    measurements: List[Dict[str, Any]] = Field(
        ...
        description="List of measurements to perform"
    )
    shots: int = Field(
        1024,
        ge=1,
        le=10000,
        description="Number of measurement shots to perform"
    )
...
class QuantumCircuitResult(BaseModel):
    ...
    backend: str = Field(
        "simulator",
        description="Quantum backend used for execution"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict,
