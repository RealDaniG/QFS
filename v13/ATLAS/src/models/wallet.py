...
class WalletCreate(BaseModel):
    ...
    description: Optional[str] = Field(None, max_length=500, description='Optional wallet description')
    asset: str = Field('QFS', description='Default asset for this wallet')
    metadata: Dict[str, Any] = Field(default_factory=dict, description='Additional wallet metadata')
...
class WalletResponse(WalletCreate):
    ...
    balances: List[WalletBalance] = Field(default_factory=list, description='List of asset balances')
...
class WalletUpdate(BaseModel):
    ...
    name: Optional[str] = Field(None, min_length=1, max_length=100, description='New wallet name')
    description: Optional[str] = Field(None, max_length=500, description='New wallet description')
    is_active: Optional[bool] = Field(None, description='Set wallet active/inactive')
    metadata: Optional[Dict[str, Any]] = Field(None, description='Updated metadata (replaces existing metadata)')
...
class WalletTransfer(BaseModel):
    ...
    memo: Optional[str] = Field(None, max_length=200, description='Optional transfer memo')
...
class WalletAddress(BaseModel):
    ...
    label: Optional[str] = Field(None, max_length=100, description='Optional address label')
    created_at: str = Field(..., description='Creation timestamp in ISO format')
...
class WalletHistoryEntry(BaseModel):
    ...
    amount: QAmount = Field(..., description='Amount (positive for credit, negative for debit)')
    asset: str = Field(..., description='Asset symbol')
