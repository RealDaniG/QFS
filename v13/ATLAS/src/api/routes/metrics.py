...
async def get_storage_metrics():
    ...
    try:
        ...
        metrics = {
            "nodes_registered": len(storage_engine.nodes),
            "objects_stored": len(storage_engine.objects),
            "shards_created": len(storage_engine.shards),
            "eligible_nodes": len(storage_engine.get_eligible_nodes()),
            "current_epoch": storage_engine.current_epoch,
            "total_atr_fees_collected": str(storage_engine.total_atr_fees_collected.value) if hasattr(storage_engine, 'total_atr_fees_collected') else "0",
            "total_nod_rewards_distributed": str(storage_engine.total_nod_rewards_distributed.value) if hasattr(storage_engine, 'total_nod_rewards_distributed') else "0",
            "storage_events_logged": len(storage_engine.storage_event_log) if hasattr(storage_engine, 'storage_event_log') else 0,
        }
    ...
    try:
        ...
        for node_id, node in storage_engine.nodes.items():
            node_metrics[node_id] = {
                ...
                "port": node.port,
                "status": node.status,
                "is_aegis_verified": node.is_aegis_verified,
                "aegis_verification_epoch": node.aegis_verification_epoch,
                "bytes_stored": str(node.bytes_stored.value) if hasattr(node, 'bytes_stored') else "0",
                "uptime_bucket": node.uptime_bucket,
                "proofs_verified": node.proofs_verified,
            }
...
async def get_storage_economics_summary():
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve economics metrics"
        )
...
async def get_prometheus_metrics():
    ...
    try:
        ...
        metrics_lines = [
            "# HELP qfs_storage_nodes_registered Total number of registered storage nodes",
            "# TYPE qfs_storage_nodes_registered gauge",
            f"qfs_storage_nodes_registered {node_count}",
            "",
            "# HELP qfs_storage_objects_stored Total number of stored objects",
            "# TYPE qfs_storage_objects_stored gauge",
            f"qfs_storage_objects_stored {object_count}",
            "",
            "# HELP qfs_storage_shards_created Total number of created shards",
            "# TYPE qfs_storage_shards_created gauge",
            f"qfs_storage_shards_created {shard_count}",
            "",
            "# HELP qfs_storage_eligible_nodes Number of eligible storage nodes",
            "# TYPE qfs_storage_eligible_nodes gauge",
            f"qfs_storage_eligible_nodes {eligible_node_count}",
            "",
            "# HELP qfs_storage_current_epoch Current storage epoch",
            "# TYPE qfs_storage_current_epoch gauge",
            f"qfs_storage_current_epoch {storage_engine.current_epoch}",
        ]
