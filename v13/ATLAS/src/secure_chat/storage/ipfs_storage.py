...
class IPFSStorage:
    ...
    def __init__(
        self, ipfs_endpoint: str = "http://localhost:5001", pin=True, timeout=30
    ):
        """
            ...
            async with session.post(f"{self.endpoint}/api/v0/add", data=data) as resp:
                ...
                if self.pin:
                    ...
                    async with session.post(
                        f"{self.endpoint}/api/v0/pin/add", data=pin_data
                    ) as pin_resp:
                        if pin_resp.status != 200:
                            ...
                            logger.warning(
                                f"Failed to pin CID {cid}: {pin_resp.status}"
                            )
    ...
    async def retrieve(self, content_hash: str) -> Optional[bytes]:
        ...
        try:
            ...
            async with session.get(
                f"{self.endpoint}/api/v0/cat", params={"arg": content_hash}
            ) as resp:
                if resp.status == 200:
    ...
    async def exists(self, content_hash: str) -> bool:
        ...
        try:
            session = await self._get_session()
            async with session.post(
                f"{self.endpoint}/api/v0/object/stat", data={"arg": content_hash}
            ) as resp:
                if resp.status == 200:
                    return True
                elif resp.status == 404:
                    ...
                    error_text = await resp.text()
                    raise RuntimeError(
                        f"IPFS stat failed: {resp.status} - {error_text}"
                    )
        except aiohttp.ClientError as e:
            import logging
    ...
    def get_cache_stats(self) -> Dict[str, int]:
        ...
        return {
            ...
            "cache_size_bytes": sum((len(v) for v in self._local_cache.values())),
        }
