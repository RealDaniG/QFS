...
def test_create_thread_authenticated(client: TestClient) -> None:
    ...
    payload = {'participants': ['user_456'], 'title': 'Integration Thread', 'metadata': {'purpose': 'test'}}
    response = client.post('/api/v1/secure-chat/threads', json=payload, headers=_auth_headers())
    assert response.status_code == 201
    body = response.json()
    assert 'thread_id' in body
    assert body['creator_id'] == 'user_123'
...
def test_send_message_authenticated(client: TestClient) -> None:
    ...
    t_resp = client.post('/api/v1/secure-chat/threads', json={'participants': ['user_789']}, headers=_auth_headers())
    assert t_resp.status_code == 201
    thread_id = t_resp.json()['thread_id']
    m_payload = {'thread_id': thread_id, 'content_hash': 'a' * 64, 'content_size': 42, 'content_type': 'text/plain', 'message_type': 'text', 'metadata': {'kind': 'unit-test'}}
    m_resp = client.post('/api/v1/secure-chat/messages', json=m_payload, headers=_auth_headers())
