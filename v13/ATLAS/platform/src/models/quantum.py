...
class QuantumSignature(BaseModel):
    ...
    timestamp: Optional[str] = Field(None, description='Signature timestamp in ISO format')
...
class QuantumSignatureVerify(BaseModel):
    ...
    algorithm: Optional[str] = Field('QFS-QUANTUM-256', description='Signature algorithm (default: QFS-QUANTUM-256)')
...
class EntangledPair(BaseModel):
    ...
    created_at: Optional[str] = Field(None, description='Creation timestamp in ISO format')
    metadata: Dict[str, Any] = Field(default_factory=dict, description='Additional entanglement metadata')
...
class QuantumState(BaseModel):
    ...
    timestamp: Optional[str] = Field(None, description='Measurement timestamp in ISO format')
...
class QuantumCircuitRequest(BaseModel):
    ...
    gates: List[Dict[str, Any]] = Field(..., description='List of quantum gates to apply')
    measurements: List[Dict[str, Any]] = Field(default_factory=list, description='List of measurements to perform')
    shots: int = Field(1024, ge=1, le=10000, description='Number of measurement shots to perform')
...
class QuantumCircuitResult(BaseModel):
    ...
    counts: Dict[str, int] = Field(..., description='Measurement results with their counts')
...
class QuantumRandomRequest(BaseModel):
    ...
    count: int = Field(1, ge=1, le=1000, description='Number of random numbers to generate')
...
class QuantumRandomResponse(BaseModel):
    ...
    random_numbers: List[int] = Field(..., description='List of generated random numbers')
