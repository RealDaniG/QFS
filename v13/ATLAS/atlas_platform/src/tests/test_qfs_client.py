...
async def test_submit_transaction(qfs_client):
    ...
    tx = Transaction(transaction_id=None, operation_type='secure_chat_thread_created', creator_id='user_123', data={'participants': ['user_123', 'user_456'], 'title': 'Test Thread', 'metadata': {}, 'timestamp': datetime.now(timezone.utc).isoformat()})
...
async def test_get_state(qfs_client):
    ...
    tx = Transaction(transaction_id=None, operation_type='secure_chat_thread_created', creator_id='user_123', data={'participants': ['user_123'], 'title': 'Test Thread', 'metadata': {}, 'timestamp': datetime.now(timezone.utc).isoformat()})
...
async def test_replay_transaction(qfs_client):
    ...
    tx = Transaction(transaction_id='test_tx_123', operation_type='secure_chat_thread_created', creator_id='user_123', data={'participants': ['user_123'], 'title': 'Test Thread', 'metadata': {}, 'timestamp': datetime.now(timezone.utc).isoformat()})
    receipt = await qfs_client.submit_transaction(tx)
...
def test_operation_bundle_serialization():
    ...
    bundle = OperationBundle(operations=[{'type': 'secure_chat_thread_created', 'creator_id': 'user_123', 'data': {'title': 'Test'}, 'nonce': 0}], bundle_hash='test_hash', timestamp='2023-01-01T00:00:00Z', creator_id='user_123')
...
async def test_transaction_validation(qfs_client):
    ...
    with pytest.raises(ValueError, match='operation_type'):
        tx = Transaction(transaction_id=None, operation_type='', creator_id='user_123', data={})
        await qfs_client.submit_transaction(tx)
    with pytest.raises(ValueError, match='creator_id'):
        tx = Transaction(transaction_id=None, operation_type='test', creator_id='', data={})
        await qfs_client.submit_transaction(tx)
