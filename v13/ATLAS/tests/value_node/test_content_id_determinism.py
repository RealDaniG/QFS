...
@pytest.mark.parametrize(
    "payload, metadata",
    [
        ({"text": "Hello, QFS!"}, {"creator_id": "user_1", "content_type": "post"}),
        ({"text": "Hello, QFS!", "tags": ["qfs", "atlas"]}, {"creator_id": "user_1", "content_type": "post"}),
    ],
)
def test_content_id_deterministic_for_same_input(payload: Dict[str, Any], metadata: Dict[str, Any]) -> None:
    ...
    assert cid_1 == cid_2
    assert len(cid_1) == 64  # hex-encoded SHA-256
...
def test_canonical_serialization_stable_field_order() -> None:
    ...
    payload_b = {"lang": "en", "text": "Hello"}  # same keys, different order
