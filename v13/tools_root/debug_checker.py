"""
Debug AST checker to identify problematic files
"""
import ast
import pathlib
import sys
...
EXCLUDED_FILES = {'deterministic_helpers.py', 'ast_checker.py', 'targeted_checker.py', 'simple_checker.py', 'comprehensive_checker.py'}
...
def check_file(filename: str) -> List[Tuple[int, str]]:
    ...
    try:
        ...
        return [(0, f'RecursionError in {filename}: File may be too complex for checker')]
    except Exception as e:
        return [(0, f'Error checking {filename}: {e}')]
...
def check_directory(directory: str) -> dict:
    ...
    for i, filepath in enumerate(py_files):
        if not should_exclude_file(filepath):
            print(f'Checking file {i + 1}/{len(py_files)}: {filepath}')
            try:
                ...
                results[str(filepath)] = [(0, f'Error checking file: {e}')]
        else:
            print(f'Skipping excluded file {i + 1}/{len(py_files)}: {filepath}')
    return results

def main():
    """Main function to run the debug checker."""
    print('Debug AST Checker')
    print('=' * 20)
    target = 'v13'
    print(f'Checking: {target}')
    results = check_directory(target)
    if results:
        print('\nX Zero-Simulation Compliance Issues Found:')
        print('-' * 45)
