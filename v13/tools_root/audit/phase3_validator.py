...
class Phase3Validator:
    ...
    def generate_evidence_package(self, genesis_topology: Dict[str, Any], harmonic_state: TokenStateBundle, psi_field_engine: Any, harmonic_economics: HarmonicEconomics, adversary_results: Dict[str, Any]=None) -> EvidencePackage:
        """
        ...
        psi_field_evidence = generate_psi_field_evidence(psi_field_engine, harmonic_state, delta_curl_threshold=harmonic_state.parameters['δ_curl'].value)
        harmonic_economics_evidence = {'economic_health_report': harmonic_economics.get_economic_health_report(), 'state_history_size': len(harmonic_economics.economic_state_history), 'violation_summary': {v.value: count for v, count in harmonic_economics.violation_counters.items()}}
        if adversary_results is None:
            adversary_results = {'adversary_tests': [], 'detection_summary': {}}
        evidence_data = {'genesis_evidence': genesis_evidence, 'psi_field_evidence': psi_field_evidence, 'harmonic_economics_evidence': harmonic_economics_evidence, 'adversary_results': adversary_results}
        merkle_root = self._build_merkle_tree(evidence_data)
        manifest_data = {'evidence_data': evidence_data, 'merkle_root': merkle_root, 'timestamp': harmonic_state.timestamp, 'version': 'QFSV13-Phase3-Evidence-v1.0'}
        manifest_hash = self._compute_deterministic_hash(manifest_data)
        evidence_package = EvidencePackage(genesis_evidence=genesis_evidence, psi_field_evidence=psi_field_evidence, harmonic_economics_evidence=harmonic_economics_evidence, adversary_results=adversary_results, merkle_root=merkle_root, manifest_hash=manifest_hash)
    ...
    def sign_evidence_package(self, evidence_package: EvidencePackage, private_key: bytes) -> EvidencePackage:
        """
        ...
        data_to_sign = {'manifest_hash': evidence_package.manifest_hash, 'merkle_root': evidence_package.merkle_root, 'timestamp': str(evidence_package.genesis_evidence.get('timestamp', 0))}
        data_json = json.dumps(data_to_sign, sort_keys=True, separators=(',', ':'))
        signature_result = self.pqc.sign_message(private_key=private_key, data=data_json.encode('utf-8'), log_list=[], pqc_cid='PHASE3_EVIDENCE_SIGNING', quantum_metadata={'component': 'phase3_validator'}, deterministic_timestamp=0)
    ...
    def export_evidence_package(self, evidence_package: EvidencePackage) -> str:
        ...
        manifest = {'manifest_hash': evidence_package.manifest_hash, 'merkle_root': evidence_package.merkle_root, 'signature': evidence_package.signature, 'timestamp': evidence_package.genesis_evidence.get('timestamp', 0), 'version': 'QFSV13-Phase3-Evidence-v1.0'}
    ...
    def validate_evidence_package(self, evidence_dir: str=None) -> bool:
        ...
        if self.pqc and manifest.get('signature'):
            try:
                data_to_verify = {'manifest_hash': manifest['manifest_hash'], 'merkle_root': manifest['merkle_root'], 'timestamp': str(manifest.get('timestamp', 0))}
                data_json = json.dumps(data_to_verify, sort_keys=True, separators=(',', ':'))
                signature_bytes = bytes.fromhex(manifest['signature'])
                verification_result = self.pqc.verify_signature(public_key=b'', data=data_json.encode('utf-8'), signature=signature_bytes, log_list=[], pqc_cid='PHASE3_EVIDENCE_VERIFICATION', quantum_metadata={'component': 'phase3_validator'}, deterministic_timestamp=0)
