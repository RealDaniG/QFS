...
class BigNum128:
    ...
    def _ensure_constants_initialized(self):
        """Ensure that constants are initialized."""
        if self.cm is None:
            from .CertifiedMath import CertifiedMath
            self.cm = CertifiedMath()
        if self.SCALE is None:
            self.SCALE = self.cm.pow(10, 18)
            self.MAX_VALUE = self.cm.sub(self.cm.pow(2, 128), 1)
    ...
    def from_int(cls, integer_val: int):
        ...
        cm = CertifiedMath()
        temp_instance = cls(0)
        return cls(cm.mul(integer_val, temp_instance.SCALE))
    ...
    def from_string(cls, s: str):
        ...
        if '.' in s:
            ...
            value = cm.add(cm.mul(int(integer_part), cls.SCALE), int(decimal_part_scaled))
        else:
            if not s.isdigit():
