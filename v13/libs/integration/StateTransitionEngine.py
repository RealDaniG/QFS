...
class StateTransitionEngine:
    ...
    def apply_state_transition(self, current_token_bundle: TokenStateBundle, allocated_rewards: Dict[str, AllocatedReward], log_list: List[Dict[str, Any]], nod_allocations: Optional[Dict[str, BigNum128]]=None, call_context: str='user_rewards', governance_outcomes: Optional[Dict[str, Any]]=None, pqc_cid: Optional[str]=None, quantum_metadata: Optional[Dict[str, Any]]=None, deterministic_timestamp: int=0) -> StateTransitionResult:
        """
        ...
        try:
            if nod_allocations is not None and len(nod_allocations) > 0:
                if call_context not in ['nod_allocation', 'governance']:
                    log_list.append({'operation': 'nod_transfer_firewall_violation', 'call_context': call_context, 'nod_allocations_count': len(nod_allocations), 'error_code': 'INVARIANT_VIOLATION_NOD_TRANSFER', 'timestamp': deterministic_timestamp})
            ...
            new_chr_state = self._apply_rewards_to_token_state(current_token_bundle.chr_state, 'CHR', allocated_rewards, log_list, pqc_cid, quantum_metadata)
            new_flx_state = self._apply_rewards_to_token_state(current_token_bundle.flx_state, 'FLX', allocated_rewards, log_list, pqc_cid, quantum_metadata)
            new_psi_sync_state = self._apply_rewards_to_token_state(current_token_bundle.psi_sync_state, 'PsiSync', allocated_rewards, log_list, pqc_cid, quantum_metadata)
            new_atr_state = self._apply_rewards_to_token_state(current_token_bundle.atr_state, 'ATR', allocated_rewards, log_list, pqc_cid, quantum_metadata)
            new_res_state = self._apply_rewards_to_token_state(current_token_bundle.res_state, 'RES', allocated_rewards, log_list, pqc_cid, quantum_metadata)
            new_nod_state = current_token_bundle.nod_state.copy() if hasattr(current_token_bundle, 'nod_state') else {'balance': '0'}
            if nod_allocations is not None and len(nod_allocations) > 0:
                new_nod_state = self._apply_nod_allocations(new_nod_state, nod_allocations, log_list, pqc_cid, quantum_metadata)
            new_token_bundle = create_token_state_bundle(chr_state=new_chr_state, flx_state=new_flx_state, psi_sync_state=new_psi_sync_state, atr_state=new_atr_state, res_state=new_res_state, lambda1=current_token_bundle.lambda1, lambda2=current_token_bundle.lambda2, c_crit=current_token_bundle.c_crit, pqc_cid=pqc_cid or current_token_bundle.pqc_cid, timestamp=deterministic_timestamp, quantum_metadata=quantum_metadata or current_token_bundle.quantum_metadata, parameters=current_token_bundle.parameters)
            ...
            self._validate_supply_deltas(current_token_bundle, new_token_bundle, allocated_rewards, nod_allocations, log_list, deterministic_timestamp)
            self._log_state_transition(current_token_bundle, new_token_bundle, allocated_rewards, log_list, pqc_cid, quantum_metadata, deterministic_timestamp)
            return StateTransitionResult(success=True, new_token_bundle=new_token_bundle, quantum_metadata=quantum_metadata)
        except Exception as e:
            self._log_state_transition_error(str(e), current_token_bundle, allocated_rewards, log_list, pqc_cid, quantum_metadata, deterministic_timestamp)
            return StateTransitionResult(success=False, new_token_bundle=None, error_message=str(e), quantum_metadata=quantum_metadata)

    def _apply_rewards_to_token_state(self, current_state: Dict[str, Any], token_type: str, allocated_rewards: Dict[str, AllocatedReward], log_list: List[Dict[str, Any]], pqc_cid: Optional[str]=None, quantum_metadata: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        """
    ...
    def _apply_nod_allocations(self, current_nod_state: Dict[str, Any], nod_allocations: Dict[str, BigNum128], log_list: List[Dict[str, Any]], pqc_cid: Optional[str]=None, quantum_metadata: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        """
    ...
    def _validate_supply_deltas(self, old_bundle: TokenStateBundle, new_bundle: TokenStateBundle, allocated_rewards: Dict[str, AllocatedReward], nod_allocations: Optional[Dict[str, BigNum128]], log_list: List[Dict[str, Any]], deterministic_timestamp: int):
        """
        ...
        for token_name, old_state, new_state in tokens:
            ...
            if token_name == 'CHR':
                validation = self.economics_guard.validate_chr_reward(chr_reward=supply_delta, total_supply_delta=supply_delta, log_list=log_list)
            elif token_name == 'FLX':
                validation = self.economics_guard.validate_flx_reward(flx_reward=supply_delta, total_supply_delta=supply_delta, log_list=log_list)
            elif token_name == 'RES':
                validation = self.economics_guard.validate_res_reward(res_reward=supply_delta, total_supply_delta=supply_delta, log_list=log_list)
            else:
                continue
            if not validation.passed:
                log_list.append({'operation': 'state_transition_supply_delta_violation', 'token': token_name, 'supply_delta': supply_delta.to_decimal_string(), 'error_code': validation.error_code, 'error_message': validation.error_message, 'details': validation.details, 'timestamp': deterministic_timestamp})
                raise ValueError(f'[GUARD] {token_name} supply delta violation: {validation.error_message} (code: {validation.error_code})')

    def _log_state_transition(self, old_bundle: TokenStateBundle, new_bundle: TokenStateBundle, allocated_rewards: Dict[str, AllocatedReward], log_list: List[Dict[str, Any]], pqc_cid: Optional[str]=None, quantum_metadata: Optional[Dict[str, Any]]=None, deterministic_timestamp: int=0):
        """
        ...
        Args:
            ...
            deterministic_timestamp: Deterministic timestamp
        """
        rewards_log = {address: {'CHR': alloc.chr_amount.to_decimal_string(), 'FLX': alloc.flx_amount.to_decimal_string(), 'RES': alloc.res_amount.to_decimal_string(), 'PsiSync': alloc.psi_sync_amount.to_decimal_string(), 'ATR': alloc.atr_amount.to_decimal_string(), 'Total': alloc.total_amount.to_decimal_string()} for address, alloc in allocated_rewards.items()}
        details = {'operation': 'state_transition', 'old_bundle_id': old_bundle.bundle_id, 'new_bundle_id': new_bundle.bundle_id, 'allocated_rewards': rewards_log, 'timestamp': deterministic_timestamp}
        self.cm._log_operation('state_transition', details, BigNum128.from_int(deterministic_timestamp), log_list, pqc_cid, quantum_metadata)

    def _log_state_transition_error(self, error_message: str, current_bundle: TokenStateBundle, allocated_rewards: Dict[str, AllocatedReward], log_list: List[Dict[str, Any]], pqc_cid: Optional[str]=None, quantum_metadata: Optional[Dict[str, Any]]=None, deterministic_timestamp: int=0):
        """
        ...
        """
        rewards_log = {address: {'CHR': alloc.chr_amount.to_decimal_string(), 'FLX': alloc.flx_amount.to_decimal_string(), 'RES': alloc.res_amount.to_decimal_string(), 'PsiSync': alloc.psi_sync_amount.to_decimal_string(), 'ATR': alloc.atr_amount.to_decimal_string(), 'Total': alloc.total_amount.to_decimal_string()} for address, alloc in allocated_rewards.items()}
        details = {'operation': 'state_transition_error', 'bundle_id': current_bundle.bundle_id, 'error_message': error_message, 'allocated_rewards': rewards_log, 'timestamp': deterministic_timestamp}
        self.cm._log_operation('state_transition_error', details, BigNum128.from_int(deterministic_timestamp), log_list, pqc_cid, quantum_metadata)
...
def test_state_transition_engine():
    ...
    current_bundle = create_token_state_bundle(chr_state=chr_state, flx_state=flx_state, psi_sync_state=psi_sync_state, atr_state=atr_state, res_state=res_state, lambda1=BigNum128.from_int(1), lambda2=BigNum128.from_int(1), c_crit=BigNum128.from_int(3), pqc_cid='test_transition_001', timestamp=1234567890)
    from ...libs.governance.RewardAllocator import AllocatedReward
    allocated_rewards = {'addr_001': AllocatedReward(address='addr_001', chr_amount=BigNum128.from_int(10), flx_amount=BigNum128.from_int(5), res_amount=BigNum128.from_int(2), psi_sync_amount=BigNum128.from_int(2), atr_amount=BigNum128.from_int(1), total_amount=BigNum128.from_int(20)), 'addr_002': AllocatedReward(address='addr_002', chr_amount=BigNum128.from_int(5), flx_amount=BigNum128.from_int(3), res_amount=BigNum128.from_int(1), psi_sync_amount=BigNum128.from_int(1), atr_amount=BigNum128.from_int(1), total_amount=BigNum128.from_int(11))}
    log_list = []
