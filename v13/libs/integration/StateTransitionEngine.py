...
class StateTransitionEngine:
    ...
    def apply_state_transition(
        ...
        nod_allocations: Optional[Dict[str, BigNum128]] = None,
        call_context: str = "user_rewards",
        governance_outcomes: Optional[Dict[str, Any]] = None,
        ...
        try:
            if nod_allocations is not None and len(nod_allocations) > 0:
                if call_context not in ["nod_allocation", "governance"]:
                    ...
                    raise ValueError(
                        f"[GUARD] NOD transfer firewall violation: NOD deltas only allowed from NODAllocator or governance (context: {call_context})"
                    )
            new_chr_state = self._apply_rewards_to_token_state(
                current_token_bundle.chr_state,
                "CHR",
                allocated_rewards,
                log_list,
                pqc_cid,
            ...
            new_flx_state = self._apply_rewards_to_token_state(
                current_token_bundle.flx_state,
                "FLX",
                allocated_rewards,
                log_list,
                pqc_cid,
            ...
            new_psi_sync_state = self._apply_rewards_to_token_state(
                current_token_bundle.psi_sync_state,
                "PsiSync",
                allocated_rewards,
                log_list,
                pqc_cid,
            ...
            new_atr_state = self._apply_rewards_to_token_state(
                current_token_bundle.atr_state,
                "ATR",
                allocated_rewards,
                log_list,
                pqc_cid,
            ...
            new_res_state = self._apply_rewards_to_token_state(
                current_token_bundle.res_state,
                "RES",
                allocated_rewards,
                log_list,
                pqc_cid,
            ...
            new_nod_state = (
                current_token_bundle.nod_state.copy()
            ...
            if nod_allocations is not None and len(nod_allocations) > 0:
                ...
                invariant_results = self.nod_invariant_checker.validate_all_invariants(
                    ...
                    previous_total_supply=BigNum128.from_string(
                        str(old_nod_state.get("balance", "0"))
                    ),
                    new_total_supply=BigNum128.from_string(
                ...
                for invariant_result in sorted(
                    ...
                    if not invariant_result.passed:
                        ...
                        raise ValueError(
                            f"[GUARD] NOD invariant violation: {invariant_result.error_message} (code: {invariant_result.error_code})"
                        )
    ...
    def _apply_rewards_to_token_state(
        ...
        for alloc in [allocated_rewards[k] for k in sorted(allocated_rewards)]:
            if token_type == "CHR":
            ...
            total_reward = self.cm.add(
                total_reward, reward_amount, log_list, pqc_cid, quantum_metadata
            )
        new_balance = self.cm.add(
            current_balance, total_reward, log_list, pqc_cid, quantum_metadata
        )
    ...
    def _apply_nod_allocations(
        ...
        for nod_amount in [nod_allocations[k] for k in sorted(nod_allocations)]:
            total_nod = self.cm.add(
                total_nod, nod_amount, log_list, pqc_cid, quantum_metadata
            )
        new_balance = self.cm.add(
            current_balance, total_nod, log_list, pqc_cid, quantum_metadata
        )
    ...
    def _validate_supply_deltas(
        ...
        for token_name, old_state, new_state in sorted(tokens):
            ...
            if not validation.passed:
                ...
                raise ValueError(
                    f"[GUARD] {token_name} supply delta violation: {validation.error_message} (code: {validation.error_code})"
                )
