...
class OracleGuidance:
    ...
    atr_directional_vector: BigNum128
...
class UtilityOracle:
    ...
    def validate_oracle_update(self, f_atr: BigNum128, directional_penalty: BigNum128, oracle_signature: bytes, expected_metadata: Dict[str, Any], log_list: List[Dict[str, Any]], pqc_cid: Optional[str]=None, quantum_metadata: Optional[Dict[str, Any]]=None, deterministic_timestamp: int=0) -> OracleGuidance:
        """
        ...
        if self.cm.lt(f_atr, zero, log_list, pqc_cid, quantum_metadata) or self.cm.gt(f_atr, one, log_list, pqc_cid, quantum_metadata):
        ...
        validated_guidance = OracleGuidance(atr_directional_vector=f_atr, directional_penalty=directional_penalty, quantum_metadata=expected_metadata)
        self._log_oracle_validation(f_atr, directional_penalty, expected_metadata, log_list, pqc_cid, quantum_metadata, deterministic_timestamp)
        return validated_guidance

    def _log_oracle_validation(self, f_atr: BigNum128, directional_penalty: BigNum128, metadata: Dict[str, Any], log_list: List[Dict[str, Any]], pqc_cid: Optional[str]=None, quantum_metadata: Optional[Dict[str, Any]]=None, deterministic_timestamp: int=0):
        """
        ...
        """
        details = {'operation': 'oracle_update_validation', 'f_atr': f_atr.to_decimal_string(), 'directional_penalty': directional_penalty.to_decimal_string(), 'metadata_keys': list(metadata.keys()) if metadata else [], 'timestamp': deterministic_timestamp}
        self.cm._log_operation('oracle_update_validation', details, f_atr, log_list, pqc_cid, quantum_metadata)
...
def test_utility_oracle():
    ...
    test_penalty = BigNum128.from_int(10)
    test_signature = b'test_signature_bytes'
    test_metadata = {'source': 'test_oracle', 'timestamp': 1234567890, 'aggregation_method': 'median_of_medians'}
    log_list = []
    try:
