...
class LogLevel(Enum):
    ...
    CRITICAL = 5   # System failures (triggers CIR handlers)
...
class QFSLogger:
    ...
    def _generate_session_id(self) -> str:
        ...
        # Use DRV_Packet timestamp if available, otherwise datetime
        return hashlib.sha3_256(
            f"{self.component}:{datetime.utcnow().isoformat()}".encode()
        ).hexdigest()[:16]
    ...
    def log(
        ...
        # Add log hash for integrity verification
        ...
        # Trigger CIR handler for CRITICAL level
    ...
    def _write_log(self, entry: Dict):
        ...
        # Console output (GitHub Actions visible)
    ...
    def _trigger_cir_handler(self, entry: Dict):
        ...
        try:
            # Dynamically import to separate logging concern from handler implementation
            # Note: This assumes handlers are in python path
            from v13.handlers.CIR302_Handler import CIR302Handler
            handler = CIR302Handler()
            # Assuming handle_critical_event exists on CIR302Handler
