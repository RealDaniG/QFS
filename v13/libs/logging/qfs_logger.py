...
class QFSLogger:
    ...
    def _generate_session_id(self) -> str:
        ...
        return hashlib.sha3_256(f'{self.component}:{det_time_isoformat()}'.encode()).hexdigest()[:16]

    def log(self, level: LogLevel, category: LogCategory, message: str, details: Optional[Dict[str, Any]]=None, exception: Optional[Exception]=None, stack_trace: Optional[str]=None):
        """Core logging method with structured output"""
        log_entry = {'timestamp': det_time_isoformat(), 'session_id': self.session_id, 'component': self.component, 'level': level.name, 'category': category.value, 'message': message, 'context': self.context, 'details': details or {}}
        if exception:
            log_entry['exception'] = {'type': type(exception).__name__, 'message': str(exception), 'args': exception.args}
        if stack_trace:
    ...
    def _hash_entry(self, entry: Dict) -> str:
        ...
        return hashlib.sha3_512(json.dumps(hashable, sort_keys=True, default=str).encode()).hexdigest()
    ...
    def export_session_logs(self, output_path: Path):
        ...
        with output_path.open('w') as f:
            json.dump({'session_id': self.session_id, 'component': self.component, 'logs': self.log_buffer, 'metadata': {'total_entries': len(self.log_buffer), 'levels': {level.name: sum((1 for log in self.log_buffer if log['level'] == level.name)) for level in LogLevel}}}, f, indent=2, default=str)