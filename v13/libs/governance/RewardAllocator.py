...
class RewardAllocator:
    ...
    def allocate_rewards(
        ...
        if allocation_weights is None:
            allocation_weights = self._create_equal_weights(
                recipient_addresses, log_list, pqc_cid, quantum_metadata
            )
        ...
        for address in sorted(recipient_addresses):
            ...
            chr_amount = self.cm.mul(
                reward_bundle.chr_reward, weight, log_list, pqc_cid, quantum_metadata
            )
            flx_amount = self.cm.mul(
                reward_bundle.flx_reward, weight, log_list, pqc_cid, quantum_metadata
            )
            res_amount = self.cm.mul(
                reward_bundle.res_reward, weight, log_list, pqc_cid, quantum_metadata
            )
            psi_sync_amount = self.cm.mul(
                reward_bundle.psi_sync_reward,
                weight,
                log_list,
                pqc_cid,
                quantum_metadata,
            )
            atr_amount = self.cm.mul(
                reward_bundle.atr_reward, weight, log_list, pqc_cid, quantum_metadata
            )
            total_amount = self.cm.add(
                self.cm.add(
                    chr_amount, flx_amount, log_list, pqc_cid, quantum_metadata
                ),
                self.cm.add(
                    self.cm.add(
                        res_amount, psi_sync_amount, log_list, pqc_cid, quantum_metadata
                    ),
                    atr_amount,
                    log_list,
                    pqc_cid,
            ...
            if not addr_validation.passed:
                ...
                raise ValueError(
                    f"[GUARD] Per-address reward violation for {address}: {addr_validation.error_message} (code: {addr_validation.error_code})"
                )
            is_dust = self.cm.lt(
                total_amount, MIN_DUST_THRESHOLD, log_list, pqc_cid, quantum_metadata
            )
            if is_dust:
                log_list.append(
                    {
    ...
    def _create_equal_weights(
        ...
        equal_weight = self.cm.div(
            one, num_recipients, log_list, pqc_cid, quantum_metadata
        )
    ...
    def _normalize_weights(
        ...
        for weight in [allocation_weights[k] for k in sorted(allocation_weights)]:
            weight_sum = self.cm.add(
                weight_sum, weight, log_list, pqc_cid, quantum_metadata
            )
        if weight_sum.value == 0:
            return {
                address: BigNum128.from_int(1) for address in allocation_weights.keys()
            }
        normalized_weights = {}
        for address in sorted(allocation_weights.keys()):
            weight = allocation_weights[address]
            normalized_weight = self.cm.div(
                weight, weight_sum, log_list, pqc_cid, quantum_metadata
            )
...
def test_reward_allocator():
    ...
    reward_bundle = RewardBundle(
        chr_reward=BigNum128.from_int(100),
        flx_reward=BigNum128.from_int(50),
    ...
    for address, alloc in sorted(allocated_rewards.items()):
        pass
