...
class InvariantCheckResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {'passed': self.passed, 'invariant_id': self.invariant_id, 'error_code': self.error_code, 'error_message': self.error_message, 'details': self.details or {}, 'violation_severity': self.violation_severity}
...
class NODInvariantChecker:
    ...
    def check_non_transferability(self, caller_module: str, operation_type: str, node_balances: Dict[str, BigNum128], log_list: Optional[List[Dict[str, Any]]]=None) -> InvariantCheckResult:
        """
        ...
        if caller_module not in self.authorized_writers:
            return InvariantCheckResult(passed=False, invariant_id='NOD-I1', error_code=NODInvariantViolationType.INVARIANT_NOD_I1_UNAUTHORIZED_WRITE.value, error_message=f'NOD-I1 violation: Unauthorized write from {caller_module}. Only NODAllocator may modify NOD balances.', details={'caller_module': caller_module, 'authorized_writers': list(self.authorized_writers), 'operation_type': operation_type})
        forbidden_operations = {'transfer', 'trade', 'swap', 'exchange', 'send', 'receive'}
        if operation_type.lower() in forbidden_operations:
            return InvariantCheckResult(passed=False, invariant_id='NOD-I1', error_code=NODInvariantViolationType.INVARIANT_NOD_I1_TRANSFER_DETECTED.value, error_message=f"NOD-I1 violation: NOD transfer operation '{operation_type}' is forbidden. NOD is non-transferable.", details={'operation_type': operation_type, 'forbidden_operations': list(forbidden_operations)})
        return InvariantCheckResult(passed=True, invariant_id='NOD-I1')

    def check_supply_conservation(self, previous_total_supply: BigNum128, new_total_supply: BigNum128, allocations: List['NODAllocation'], log_list: Optional[List[Dict[str, Any]]]=None) -> InvariantCheckResult:
        """
    ...
    def check_voting_power_bounds(self, node_balances: Dict[str, BigNum128], total_nod_supply: BigNum128, log_list: Optional[List[Dict[str, Any]]]=None) -> InvariantCheckResult:
        """
        ...
        for node_id, node_balance in node_balances.items():
            ...
            if voting_power_ratio.value > MAX_NOD_VOTING_POWER_RATIO.value:
                return InvariantCheckResult(passed=False, invariant_id='NOD-I3', error_code=NODInvariantViolationType.INVARIANT_NOD_I3_VOTING_POWER_EXCEEDED.value, error_message=f'NOD-I3 violation: Node {node_id} voting power {voting_power_ratio.to_decimal_string()} exceeds maximum {MAX_NOD_VOTING_POWER_RATIO.to_decimal_string()}', details={'node_id': node_id, 'node_balance': node_balance.to_decimal_string(), 'total_supply': total_nod_supply.to_decimal_string(), 'voting_power_ratio': voting_power_ratio.to_decimal_string(), 'max_allowed': MAX_NOD_VOTING_POWER_RATIO.to_decimal_string()})
        return InvariantCheckResult(passed=True, invariant_id='NOD-I3')

    def check_deterministic_allocation(self, allocations: List['NODAllocation'], expected_hash: Optional[str]=None, log_list: Optional[List[Dict[str, Any]]]=None) -> InvariantCheckResult:
        """
        ...
        if expected_hash is not None:
            ...
            if actual_hash != expected_hash:
                return InvariantCheckResult(passed=False, invariant_id='NOD-I4', error_code=NODInvariantViolationType.INVARIANT_NOD_I4_HASH_VERIFICATION_FAILURE.value, error_message=f'NOD-I4 violation: Allocation hash mismatch (replay failure)', details={'expected_hash': expected_hash, 'actual_hash': actual_hash, 'allocation_count': len(allocations)})
        return InvariantCheckResult(passed=True, invariant_id='NOD-I4')

    def validate_all_invariants(self, caller_module: str, operation_type: str, previous_total_supply: BigNum128, new_total_supply: BigNum128, node_balances: Dict[str, BigNum128], allocations: List['NODAllocation'], expected_hash: Optional[str]=None, log_list: Optional[List[Dict[str, Any]]]=None) -> List[InvariantCheckResult]:
        """
        ...
        if log_list is None:
            log_list = []
        results = []
        results.append(self.check_non_transferability(caller_module, operation_type, node_balances, log_list))
        results.append(self.check_supply_conservation(previous_total_supply, new_total_supply, allocations, log_list))
        results.append(self.check_voting_power_bounds(node_balances, new_total_supply, log_list))
        results.append(self.check_deterministic_allocation(allocations, expected_hash, log_list))
        return results
    ...
    def generate_allocation_hash(self, allocations: List['NODAllocation']) -> str:
        ...
        """
        sorted_allocations = sorted(allocations, key=lambda a: a.node_id)
        allocation_data = {'allocations': [{'node_id': a.node_id, 'nod_amount': a.nod_amount.to_decimal_string(), 'contribution_score': a.contribution_score.to_decimal_string(), 'timestamp': a.timestamp} for a in sorted_allocations]}
    ...
    def generate_violation_event_hash(self, result: InvariantCheckResult, timestamp: int) -> str:
        """
        ...
        Returns:
            str: 64-character SHA-256 hash
        """
        event_data = {'operation': 'nod_invariant_violation', 'invariant_id': result.invariant_id, 'error_code': result.error_code, 'error_message': result.error_message, 'details': result.details, 'timestamp': timestamp}
