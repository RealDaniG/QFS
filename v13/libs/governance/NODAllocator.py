...
try:
    ...
    from ...libs.governance.AEGIS_Node_Verification import (
        AEGIS_Node_Verifier,
    ...
    try:
        ...
        from governance.NODInvariantChecker import (
            NODInvariantChecker,
            InvariantCheckResult,
        )
        from governance.AEGIS_Node_Verification import (
            AEGIS_Node_Verifier,
            NodeVerificationResult,
        )
...
class NODAllocator:
    ...
    def __init__(
        self, cm_instance: CertifiedMath, allocation_fraction: BigNum128 = None
    ):
        """
        ...
        if allocation_fraction is None:
            ...
            if self.cm.lt(
                allocation_fraction, MIN_NOD_ALLOCATION_FRACTION, [], None, None
            ):
                raise ValueError(
                    f"NOD allocation fraction {allocation_fraction.to_decimal_string()} below minimum {MIN_NOD_ALLOCATION_FRACTION.to_decimal_string()}"
                )
            if self.cm.gt(
                allocation_fraction, MAX_NOD_ALLOCATION_FRACTION, [], None, None
            ):
                raise ValueError(
                    f"NOD allocation fraction {allocation_fraction.to_decimal_string()} above maximum {MAX_NOD_ALLOCATION_FRACTION.to_decimal_string()}"
                )
    ...
    def allocate_from_atr_fees(
        ...
        if self.cm.gt(
            nod_share, NOD_MAX_ISSUANCE_PER_EPOCH, log_list, pqc_cid, quantum_metadata
        ):
        ...
        if self.cm.lte(
            nod_share, NOD_ZERO_ACTIVITY_FLOOR, log_list, pqc_cid, quantum_metadata
        ):
            if log_list is not None:
                log_list.append(
                    {
        ...
        for score in [verified_nodes[k] for k in sorted(verified_nodes)]:
            total_verified_contribution = self.cm.add(
                total_verified_contribution, score, log_list, pqc_cid, quantum_metadata
            )
        econ_validation = self.economics_guard.validate_nod_allocation(
            nod_amount=nod_share,
            total_fees=atr_total_fees,
            node_voting_power=BigNum128.from_int(0),
            total_voting_power=BigNum128.from_int(1),
        ...
        if not econ_validation.passed:
            ...
            raise ValueError(
                f"[GUARD] Economic bound violation: {econ_validation.error_message} (code: {econ_validation.error_code})"
            )
        return self.allocate_nod(
            nod_reward_pool=nod_share,
            node_contributions=verified_nodes,
            log_list=log_list,
            pqc_cid=pqc_cid,
    ...
    def allocate_nod(
        ...
        for score in [node_contributions[k] for k in sorted(node_contributions)]:
            total_contribution = self.cm.add(
                total_contribution, score, log_list, pqc_cid, quantum_metadata
            )
        ...
        max_node_share = self.cm.mul(
            nod_reward_pool, MAX_NODE_REWARD_SHARE, log_list, pqc_cid, quantum_metadata
        )
        ...
        for i, node_id in enumerate(sorted_nodes[:-1]):
            ...
            share_numerator = self.cm.mul(
                contribution_score, nod_reward_pool, log_list, pqc_cid, quantum_metadata
            )
            share = self.cm.div(
                share_numerator, total_contribution, log_list, pqc_cid, quantum_metadata
            )
            capped = False
            ...
            remaining_nod = self.cm.sub(
                remaining_nod, share, log_list, pqc_cid, quantum_metadata
            )
        last_node_id = sorted_nodes[-1]
        last_contribution_score = node_contributions[last_node_id]
        last_node_share = remaining_nod
        if self.cm.gt(
            last_node_share, max_node_share, log_list, pqc_cid, quantum_metadata
        ):
    ...
    def _allocate_equal(
        ...
        if num_nodes == 0:
            return []
        equal_share = self.cm.div(
            nod_reward_pool,
            BigNum128.from_int(num_nodes),
            log_list,
            pqc_cid,
            quantum_metadata,
        )
        ...
        for i, node_id in enumerate(sorted_nodes[:-1]):
            ...
            remaining_nod = self.cm.sub(
                remaining_nod, equal_share, log_list, pqc_cid, quantum_metadata
            )
        last_node_id = sorted_nodes[-1]
        last_node_share = remaining_nod
        if self.cm.gt(
    ...
    def _log_nod_allocation(
        ...
        for alloc in sorted(allocations):
            total_allocated = self.cm.add(
                total_allocated, alloc.nod_amount, log_list, pqc_cid, quantum_metadata
            )
        details = {
            "operation": "nod_allocation",
            "nod_reward_pool": nod_reward_pool.to_decimal_string(),
            "total_contributions": sum(
                (score.value for score in node_contributions.values())
            ),
...
def test_nod_allocator():
    ...
    node_contributions = {
        "node_0xabc123": BigNum128.from_int(50),
        "node_0xdef456": BigNum128.from_int(30),
        "node_0xghi789": BigNum128.from_int(20),
    }
    log_list = []
    ...
    total_allocated = BigNum128(0)
    for alloc in sorted(allocations):
        total_allocated = cm.add(
            total_allocated, alloc.nod_amount, log_list, "test_nod_001", {}
        )
