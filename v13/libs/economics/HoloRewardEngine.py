# HoloRewardEngine.py
# QFS V13 - Phase 3 HoloReward Engine (PRODUCTION-PERFECT)
# Zero-Simulation | PQC-Secured | Harmonic-Aligned
...
class HoloRewardEngine:
    ...
    def __init__(self, certified_math: Any, cir302_handler: Any, psi_field_engine: Any):
        ...
        self.SECURITY_LEVELS = {
            ...
            "ELEVATED": {"a_max": 500},
            "HIGH": {"a_max": 100},
            "CRITICAL": {"a_max": 0}   # Complete halt
        }
        self.A_MAX = 1000
    ...
    def _calculate_reward_multiplier(self, intensity: int, resonance: int) -> int:
        ...
        if intensity < self.MIN_INTENSITY_THRESHOLD:
            return 0
        # Simple multiplication for now, assuming inputs are scaled or raw integers
        # Using certified math if possible, else standard int math (safe for integers)
    ...
    def _calculate_harmonic_dividend(self, reward_multiplier: int, treasury_state: Any) -> int:
        ...
        # Use BigNum/CertifiedMath logic to avoid floats
        # dividend = (reward_multiplier * total_distributed) // SCALE_FACTOR
    ...
    def _generate_reward_evidence(
        ...
        # Fix: Use div_floor to avoid float division
        ...
        # Check for dissonance amplification (feedback loops)
        dissonance_present = False
        if hasattr(harmonic_state, 'shards') and harmonic_state.shards:
             # Fix: Deterministic iteration order
        ...
        # Fix: Enforce Zero Resonance Amplification
        if dissonance_present:
            reward_multiplier = 0
            harmonic_dividend = 0 # Cut dividend too
        ...
        return {
            ...
            "boundedness_verification": {
                ...
                "security_level_a_max": self.SECURITY_LEVELS[self.current_security_level]["a_max"]
            },
    ...
    def _update_reward_history(self, reward_state: Dict):
        ...
        # Fix: Reset operation counters per epoch to prevent exhaustion false positives over time
        self.operation_counters = {}
    
    def _handle_reward_failure(self, error_message: str):
        """Handle reward computation failures with security escalation."""
        # Escalate security level based on error type
