...
GENESIS_STATE = {
    ...
    "token_allocations": {
        ...
        "treasury_reserve": {"CHR": 0, "FLX": 0, "ATR": 0, "RES": 0, "ΨSync": 0},
    },
    "system_constants": {
        "A_MAX": CONST.A_MAX,
        "δ_max": CONST.δ_max,
...
def _verify_golden_ratio_allocations() -> bool:
    ...
    for shard_id, allocations in sorted(shards.items()):
        ...
        if abs(actual_flx - expected_flx) > tolerance:
            raise ValueError(
                f"Golden ratio violation in {shard_id}: CHR={chr_amount}, FLX={actual_flx}, expected FLX≈{expected_flx}"
            )
    return True
...
def _validate_pqc_key_format(key: str) -> bool:
    ...
    if not key.startswith("DIL5_PK_") or len(key) < 50:
        raise ValueError(
            "Invalid PQC key format - must be 50+ chars and start with DIL5_PK_"
        )
    return True
...
def validate_genesis_constraints(certified_math=None, pqc=None) -> dict:
    ...
    try:
        total_chr = sum(
            (
                shard["CHR"]
                for shard in GENESIS_STATE["token_allocations"]["shards"].values()
            )
        )
        total_flx = sum(
            (
        ...
        if total_chr != CONST.MAX_CHR_SUPPLY:
            violations.append(
                f"CHR conservation: {total_chr} != {CONST.MAX_CHR_SUPPLY}"
            )
        if total_flx != CONST.MAX_FLX_SUPPLY:
            violations.append(
                f"FLX conservation: {total_flx} != {CONST.MAX_FLX_SUPPLY}"
            )
        proofs["golden_ratio_verified"] = _verify_golden_ratio_allocations()
        proofs["topology_verified"] = _verify_toroidal_topology()
        active_nodes = [
            node for node in FOUNDING_NODE_REGISTRY.values() if node["active"]
        ]
        ...
        if certified_math is not None:
            proofs["certified_math_validation"] = certified_math.verify_genesis_state(
                GENESIS_STATE
            )
        if pqc is not None:
            proofs["pqc_registry_valid"] = pqc.validate_node_registry(
                FOUNDING_NODE_REGISTRY
            )
        return {
...
def export_genesis_evidence() -> dict:
    ...
    return {
        ...
        "token_metrics": {
            ...
            "avg_chr_per_shard": sum(
                (
                    shard["CHR"]
                    for shard in GENESIS_STATE["token_allocations"]["shards"].values()
                )
            )
            // len(GENESIS_STATE["token_allocations"]["shards"]),
        },
        "governance_metrics": {
            "founding_nodes": len(FOUNDING_NODE_REGISTRY),
            "active_nodes": len(
                [n for n in FOUNDING_NODE_REGISTRY.values() if n["active"]]
            ),
            "recovery_threshold": CONST.EMERGENCY_RECOVERY_THRESHOLD,
...
class GenesisSafetyManager:
    ...
    def verify_runtime_integrity(self, current_hash):
        ...
        if current_hash != expected_hash:
            raise SecurityError(
                f"Genesis integrity compromised: {current_hash} != {expected_hash}"
            )
...
try:
    ...
    for _, node in sorted(FOUNDING_NODE_REGISTRY.items()):
