...
class PsiSyncProtocol:
    ...
    def compute_global_psisync(
        self,
        shard_psisync_values: Dict[str, int],
        epsilon_sync: int,
        ...
        if not shard_psisync_values:
            return {
                "global_psisync": BigNum128(0).to_decimal_string(),
                "consensus_achieved": False,
                "timestamp": deterministic_timestamp,
                "timestamp_source": f"drv_packet:{drv_packet_seq}",
            }

        # Enforce deterministic order for consensus calculation
        sorted_shards = sorted(shard_psisync_values.keys())
        values = [shard_psisync_values[sid] for sid in sorted_shards]
        shard_ids = sorted_shards

        outlier_result = self._detect_outliers(values, shard_ids)
        cleaned_values = outlier_result["cleaned_values"]
        if not cleaned_values:
            consensus_val = BigNum128(0)
        else:
            consensus_val = self._compute_secure_median(cleaned_values)

        metrics = self._compute_consensus_metrics(
            cleaned_values, consensus_val, epsilon_sync
        )
        if not metrics["consensus_achieved"]:
            self._handle_consensus_failure(metrics, epsilon_sync, cleaned_values)

        return {
            "global_psisync": consensus_val.to_decimal_string(),
    ...
    def _detect_outliers(
        self, values: List[int], shard_ids: List[str]
    ) -> Dict[str, Any]:
        ...
        max_outliers = min(
            len(sorted_values) // self.MAX_OUTLIER_RATIO,
            len(sorted_values)
            - len(sorted_values) * self.OUTLIER_THRESHOLD_PERCENTILE // 100,
        )
        ...
        accepted_shards = []
        outlier_shards = []
        for val, idx in sorted_values:  # Already sorted by val, then index
            if len(outlier_indices) >= max_outliers:
                clean_values.append(val)
                accepted_shards.append(shard_ids[idx])
                continue
            is_outlier = val < self.math.sub(
                q1, outlier_threshold, log_list
            ) or val > self.math.add(q3, outlier_threshold, log_list)
            if is_outlier:
    ...
    def _compute_secure_median(self, values: List[int]) -> int:
        ...
        if not values:
            raise SecurityError(
                "Cannot compute median of empty list", "EMPTY_LIST", {}, "CIR-302"
            )
    ...
    def _compute_consensus_metrics(
        self, values: List[int], global_psisync: int, epsilon_sync: int
    ) -> Dict[str, Any]:
        ...
        byzantine_resistance_score = self._compute_byzantine_score(
            deviations, epsilon_sync
        )
        return {
            ...
            "consensus_quality": {
                "shard_agreement_ratio": shard_agreement_ratio,
                "consensus_stability": consensus_stability,
                "byzantine_resistance_score": byzantine_resistance_score,
            },
            "deviation_distribution": {
                "min_deviation": self._min_list(deviations),
                "max_deviation": max_deviation,
                "average_deviation": average_deviation,
            },
        }
    ...
    def _compute_consensus_stability(self, values: List[int]) -> int:
        ...
        stability = self.math.div_floor(
            BigNum128(1000000), BigNum128(max(1, variance_int // 1000))
        )
    ...
    def _compute_trimmed_mean(self, values: List[int]) -> int:
        ...
        trimmed_values = (
            sorted_values[trim_count : len(sorted_values) - trim_count] or sorted_values
        )
        total = sum(trimmed_values)
        return self.math.div_floor(total, max(1, len(trimmed_values)))

    def _handle_consensus_failure(
        self, result: Dict[str, Any], epsilon_sync: int, cleaned_values: List[int]
    ):
        ...
        evidence = {
            ...
            "progressive_degradation": self.PROGRESSIVE_DEGRADATION,
            "fallback_available": len(cleaned_values) >= self.MIN_CONSENSUS_SHARDS,
        }
        if (
            self.PROGRESSIVE_DEGRADATION
            and len(cleaned_values) >= self.MIN_CONSENSUS_SHARDS
        ):
            ...
            if self.cir412_handler and hasattr(self.cir412_handler, "notify"):
                self.cir412_handler.notify(
                    reason="PSI_SYNC_CONSENSUS_DEGRADED",
                    details=f"Max deviation {result['max_deviation']} > ε_sync {epsilon_sync}",
                    severity="WARNING",
                    evidence=evidence,
                )
        elif self.cir412_handler and hasattr(self.cir412_handler, "halt"):
            self.cir412_handler.halt(
                reason="PSI_SYNC_CONSENSUS_FAILURE",
                details=f"Max deviation {result['max_deviation']} > ε_sync {epsilon_sync}",
                evidence=evidence,
            )
        else:
            raise SecurityError(
                f"PSI_SYNC_CONSENSUS_FAILURE: Max deviation {result['max_deviation']} > ε_sync {epsilon_sync}",
                violation_type="CONSENSUS_FAILURE",
                evidence=evidence,
                cir_code="CIR-412",
            )

    def validate_shard_psisync_proposal(
        self,
        shard_id: str,
        proposed_psisync: int,
        global_psisync: int,
        epsilon_sync: int,
        previous_psisync: Optional[int] = None,
    ) -> Dict[str, Any]:
        ...
        validation = {
            ...
            "evidence": {
                "shard_id": shard_id,
                "proposed_psisync": proposed_psisync,
                "global_psisync": global_psisync,
                "epsilon_sync": epsilon_sync,
            },
        }
        ...
        if deviation > epsilon_sync:
            ...
            validation["violations"].append(
                f"Deviation {deviation} > ε_sync {epsilon_sync}"
            )
        ...
        if previous_psisync is not None:
            ...
            if change > max_allowed_change:
                validation["valid"] = False
                validation["violations"].append(
                    f"Sudden ΨSync change: {change} > {max_allowed_change}"
                )
        return validation


def create_psisync_protocol(
    certified_math: Any, cir412_handler: Any = None, security_level: str = "HIGH"
) -> PsiSyncProtocol:
...
def generate_psisync_evidence(
    protocol: PsiSyncProtocol, consensus_result: Dict[str, Any]
) -> Dict[str, Any]:
    """Generate comprehensive ΨSync evidence for Phase3EvidenceBuilder."""
    return {
        "psi_sync_consensus": consensus_result,
        "protocol_configuration": {
            "outlier_threshold_percentile": protocol.OUTLIER_THRESHOLD_PERCENTILE,
            "max_outlier_ratio": protocol.MAX_OUTLIER_RATIO,
            "min_consensus_shards": protocol.MIN_CONSENSUS_SHARDS,
            "progressive_degradation": protocol.PROGRESSIVE_DEGRADATION,
            "degradation_fallback_mode": protocol.DEGRADATION_FALLBACK_MODE,
            "max_consensus_attempts": protocol.MAX_CONSENSUS_ATTEMPTS,
            "consensus_timeout_ms": protocol.CONSENSUS_TIMEOUT_MS,
        },
        "security_status": {
            "consensus_achieved": consensus_result["consensus_achieved"],
            "outlier_ratio": consensus_result["outlier_ratio"]
            if "outlier_ratio" in consensus_result
            else 0,
            "max_deviation": consensus_result["max_deviation"],
            "shard_participation": 0,
            "consensus_quality": consensus_result.get("consensus_quality", {}),
            "fallback_used": "fallback_consensus" in consensus_result,
        },
    }
...
__all__ = [
    "PsiSyncProtocol",
    "SecurityError",
    "create_psisync_protocol",
    "generate_psisync_evidence",
]
