...
class TestCIRHandler:
    ...
    def halt(self, reason: str, details: str, evidence: Dict = None):
        ...
        event_id = hashlib.sha3_256(
            f"cir_halt_{reason}_{len(self.halt_events)}".encode()
        ).hexdigest()
        event = {
            "event_id": event_id,
            "reason": reason,
    ...
    def notify(self, reason: str, details: str, severity: str, evidence: Dict = None):
        ...
        if not self.active:
            return
        event_id = hashlib.sha3_256(
            f"cir_notify_{reason}_{len(self.notify_events)}".encode()
        ).hexdigest()[:16]
        event = {
            "event_id": event_id,
            "reason": reason,
...
class DeterministicResourceMonitor:
    ...
    def increment_operations(self, test_id: str, count: int = 1) -> bool:
        ...
        # Whitelisted update: self.operation_counters[test_id] = self.math.add(...)
        self.operation_counters[test_id] = self.math.add(
            self.operation_counters.get(test_id, 0), count
        )
        return self.operation_counters[test_id] <= self.max_operations

    def allocate_memory(self, test_id: str, bytes_needed: int) -> bool:
        """Simulate memory allocation deterministically."""
        # Whitelisted update: self.memory_simulation[test_id] = self.math.add(...)
        self.memory_simulation[test_id] = self.math.add(
            self.memory_simulation.get(test_id, 0), bytes_needed
        )
        return self.memory_simulation[test_id] <= self.max_memory_bytes
...
class EconomicAdversarySuite:
    ...
    ADVERSARIES = {
        ...
        "EA-8": {
            ...
            "severity_weight": Fraction(13, 10),
        },
        "EA-9": {
            ...
            "severity_weight": Fraction(17, 10),
        },
        "EA-10": {
        ...
        "EA-11": {
            ...
            "expected_response": "BLOCKED",
            "attack_vector": "timing_attack",
            "severity_weight": Fraction(11, 10),
        },
        "EA-12": {
            "name": "QPU Mismatch",
    ...
    def _create_deterministic_test_state(self, test_id: str, run_number: int) -> Any:
        ...
        for char in sorted(test_id):
            test_id_sum = self.math.add(
                test_id_sum, ord(char), log_list, pqc_cid, quantum_metadata
            )
        ...
        for i, shard_id in enumerate(shard_keys):
            ...
            shard_seed = self.math.mul(
                combined_seed, position_factor, log_list, pqc_cid, quantum_metadata
            )
            if hasattr(shard, "CHR"):
                current_chr = self.math.from_int(shard.CHR)
                variation = self.math.mod(
                    shard_seed,
                    self.math.from_int(1000),
                    log_list,
                    pqc_cid,
                    quantum_metadata,
                )
                centered_variation = self.math.sub(
                    variation,
                    self.math.from_int(500),
                    log_list,
                    pqc_cid,
                    quantum_metadata,
                )
                new_chr = self.math.add(
                    current_chr, centered_variation, log_list, pqc_cid, quantum_metadata
                )
                shard.CHR = new_chr
            if hasattr(shard, "ATR"):
                ...
                centered_variation = self.math.sub(
                    variation,
                    self.math.from_int(50),
                    log_list,
                    pqc_cid,
                    quantum_metadata,
                )
                new_atr = self.math.add(
                    current_atr, centered_variation, log_list, pqc_cid, quantum_metadata
                )
                shard.ATR = new_atr
            if hasattr(shard, "RES"):
                ...
                centered_variation = self.math.sub(
                    variation,
                    self.math.from_int(5000),
                    log_list,
                    pqc_cid,
                    quantum_metadata,
                )
                new_res = self.math.add(
                    current_res, centered_variation, log_list, pqc_cid, quantum_metadata
                )
                shard.RES = new_res
        return test_state
    ...
    def _execute_attack_scenario(self, test_id: str, test_state: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.math, "compute_audit_hash"):
                audit_hash = self.math.compute_audit_hash(
                    self.current_log_list, pqc_cid, quantum_metadata
                )
            else:
    ...
    def _create_coherence_spoof_scenario(self, test_state: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "system_response": "UNEXPECTED_SUCCESS",
                "triggered_cir": "None",
                "result": str(result),
            }
        except Exception as e:
    ...
    def _create_treasury_siphon_scenario(self, test_state: Any) -> Dict[str, Any]:
        ...
        new_memory = self.math.add(
            current_memory, 2048, log_list, pqc_cid, quantum_metadata
        )
    ...
    def _ensure_deterministic_reproduction(
        self, test_id: str, attack_func: Callable
    ) -> Any:
    ...
    def _compare_attack_results(self, result1: Any, result2: Any) -> bool:
        ...
        if isinstance(result1, dict) and isinstance(result2, dict):
            ...
            return all(
                (
                    self._compare_attack_results(r1, r2)
    ...
    def _analyze_real_exception(
        self, exception: Exception, test_id: str
    ) -> Dict[str, Any]:
        ...
        if self.test_cir302.halt_events:
            cir_triggers.extend(
                [f"CIR-302: {e['reason']}" for e in self.test_cir302.halt_events]
            )
        if self.test_cir412.halt_events:
            cir_triggers.extend(
                [f"CIR-412: {e['reason']}" for e in self.test_cir412.halt_events]
            )
        if self.test_cir511.halt_events:
            cir_triggers.extend(
                [f"CIR-511: {e['reason']}" for e in self.test_cir511.halt_events]
            )
        if cir_triggers:
            return {
    ...
    def _verify_expected_response(
        self, test_id: str, attack_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if expected_cir == "None":
            cir_match = len(actual_cirs) == 0 or all(
                ("None" in cir for cir in actual_cirs)
            )
        else:
            cir_match = any(
                (expected_cir in str(actual_cir) for actual_cir in actual_cirs)
            )
        response_match = (
            expected_response in str(actual_response)
            or actual_response == expected_response
        )
        ...
        return {
            ...
            "resource_monitoring": {
                "total_operations": sum(
                    self.resource_monitor.operation_counters.values()
                ),
                "total_memory_bytes": sum(
                    self.resource_monitor.memory_simulation.values()
                ),
