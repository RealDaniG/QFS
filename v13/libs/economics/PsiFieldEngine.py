# PsiFieldEngine.py
# QFS V13 - Phase 3 ψ-Field Physics Engine (PRODUCTION-HARDENED)
# Zero-Simulation Compliant | Deterministic | Integer-Only | Byzantine-Resistant
# Implements Step 1: ψ-Dynamics Core (Enhanced with Security & Performance)
...
class SecurityThresholds:
    ...
    SUSPICIOUS_CURL_MAGNITUDE = 1_000_000  # Unreasonably large curl
    MAX_DISSONANCE = 1_000_000             # Reasonable upper bound
    MAX_CHR_VALUE = 10**12                 # 1 trillion max
    MAX_ATR_VALUE = 10**6                  # 1 million max
...
class DiscretePsiField:
    ...
    def __init__(self, genesis_topology: Dict[str, Any], certified_math: Any):
        ...
        self.cycle_basis = []    
        self.cycle_basis_hash = None  # For integrity verification
        
        # Security bounds (prevent resource exhaustion)
        self.MAX_CYCLE_LENGTH = 8     # Maximum cycle length to analyze
        self.MAX_CYCLES_PER_SHARD = 5 # Prevent combinatorial explosion
        self.MAX_EDGES_PER_SHARD = 10 # Prevent dense graph attacks
    ...
    def _validate_and_build_graph(self, topology: Dict[str, Any]):
        ...
        # Validate connection format and bounds
        if len(connections) > 100:  # Reasonable upper bound
            evidence = {"connections_count": len(connections)}
            raise SecurityError(
                f"EXCESSIVE_CONNECTIONS: Excessive connections: {len(connections)}",
                violation_type="EXCESSIVE_CONNECTIONS",
                evidence=evidence,
                cir_code="CIR-412"
            )
        ...
        for a, b in connections:
            if not isinstance(a, str) or not isinstance(b, str):
                ...
                raise SecurityError(
                    f"INVALID_SHARD_ID_TYPES: Invalid shard ID types: {type(a)}, {type(b)}",
                    violation_type="INVALID_SHARD_ID_TYPES",
                    evidence=evidence,
                    cir_code="CIR-412"
                )
            shard_set.add(a)
        ...
        # Initialize graph with security bounds
        self.graph = {shard: set() for shard in self.shard_ids}
        
        # Build undirected graph with duplicate prevention
        edge_count = {}
        for a, b in connections:
            if a not in self.shard_ids or b not in self.shard_ids:
                evidence = {"shard_a": a, "shard_b": b, "valid_shards": list(self.shard_ids)}
                raise SecurityError(
                    f"INVALID_SHARD_IN_CONNECTION: Invalid shard in connection: {a}-{b}",
                    violation_type="INVALID_SHARD_IN_CONNECTION",
                    evidence=evidence,
                    cir_code="CIR-412"
                )
            ...
            if a == b:
                evidence = {"shard_id": a}
                raise SecurityError(
                    f"SELF_LOOP_DETECTED: Self-loop detected: {a}",
                    violation_type="SELF_LOOP_DETECTED",
                    evidence=evidence,
                    cir_code="CIR-412"
                )
            ...
            if edge_key in edge_count:
                evidence = {"edge": f"{a}-{b}"}
                raise SecurityError(
                    f"DUPLICATE_EDGE: Duplicate edge: {a}-{b}",
        ...
        # Validate degree constraints with security bounds
        ...
        for shard, neighbors in self.graph.items():
            ...
            if degree < min_degree:
                ...
                raise SecurityError(
                    f"UNDER_CONNECTED_SHARD: Shard {shard} under-connected: {degree} < {min_degree}",
                    violation_type="UNDER_CONNECTED_SHARD",
                    evidence=evidence,
                    cir_code="CIR-412"
                )
            if degree > self.MAX_EDGES_PER_SHARD:
                ...
                raise SecurityError(
                    f"OVER_CONNECTED_SHARD: Shard {shard} over-connected: {degree} > {self.MAX_EDGES_PER_SHARD}",
                    violation_type="OVER_CONNECTED_SHARD",
                    evidence=evidence,
                    cir_code="CIR-412"
                )
    ...
    def _compute_secure_cycle_basis(self):
        ...
        parent = {}
        cycles = []
        cycle_cache = set()  # Prevent duplicate cycles
        
        def secure_dfs(u, v, depth=0):
            if depth > self.MAX_CYCLE_LENGTH:
                return  # Bound cycle length
            
            visited.add(v)
            parent[v] = u
            
            # Process neighbors in deterministic order
            neighbors = sorted(self.graph[v])  # Ensures cross-runtime consistency
    ...
    def _extract_secure_cycle(self, start: str, end: str, parent: Dict[str, str]) -> Optional[List[str]]:
        ...
        try:
            ...
            # path2: lca ← end (reversed, excluding lca)
            lca_index = path2.index(lca)
            for i in range(lca_index - 1, -1, -1):
                cycle.append(path2[i])
            
            cycle.append(start)  # Close cycle
            
            # Validate cycle is simple and within bounds
    ...
    def verify_cycle_basis_integrity(self) -> bool:
        ...
        if not is_valid:
            ...
            raise SecurityError(
                "CYCLE_BASIS_TAMPERING: Cycle basis integrity compromised",
                violation_type="CYCLE_BASIS_TAMPERING",
                evidence=evidence,
                cir_code="CIR-412"
            )
        return is_valid
    ...
    def psi_density(self, shard_id: str, harmonic_state: 'TokenStateBundle') -> int:
        ...
        atr_val = self.certified_math.clamp(
            shard["ATR"], 
            0, 
            SecurityThresholds.MAX_ATR_VALUE
        )   # 1 million max
        
        # Compute dissonance safely
        dissonance = max(0, shard.get("DISSONANCE", 0))
        dissonance = min(dissonance, SecurityThresholds.MAX_DISSONANCE)  # Reasonable upper bound
        
        # Compute ψ-density with overflow protection
        try:
    ...
    def psi_curl_around_cycle(self, cycle: List[str], harmonic_state: TokenStateBundle) -> int:
        ...
        # Validate cycle is simple and connected
        if len(cycle) != len(set(cycle)):
            return 0
        ...
        try:
            for idx in range(n):
                u = cycle[idx]
                v = cycle[(idx + 1) % n]  # Wrap around
                
                if v not in self.graph.get(u, set()):
                    # Invalid cycle - edges don't exist
                    return 0
            ...
            return 0  # Safe fallback for computational errors

    def compute_psi_curls_with_anomaly_detection(
        self, 
        ...
        for cycle in self.cycle_basis:
            ...
            # Detect potential Byzantine manipulation
            curl_mag = self.certified_math.abs(curl_val)
            
            if curl_mag > 1000000:  # Unreasonably large curl
                anomalies.append(f"SUSPICIOUS_CURL: Cycle {cycle} has magnitude {curl_mag}")
            elif curl_val != 0 and len(cycle) == 3:
                # Small cycles should rarely have non-zero curl in stable state
    ...
    def global_psi_sync_metric(self, harmonic_state: TokenStateBundle) -> Dict[str, int]:
        ...
        for shard_id in self.shard_ids:
            ...
            # Multiple sync measures for consistency checking
            sync_simple = self.certified_math.mul(chr_val, atr_val)
            sync_resonant = self.certified_math.mul(
                sync_simple,
                self.certified_math.add(1, res_val // 1000)  # Scaled RES contribution
            )
            
            shard_syncs[shard_id] = {
                "simple": sync_simple,
                "resonant": sync_resonant,
                "density_based": self.psi_density(shard_id, harmonic_state)
            }
        ...
        return {
            "total_simple_sync": total_simple,
            "total_resonant_sync": total_resonant, 
            "average_sync": avg_sync,
            "sync_variance": variance,
            "shard_syncs": shard_syncs
        }

    def validate_psi_field_integrity(
        self, 
        harmonic_state: TokenStateBundle, 
        delta_curl_threshold: int
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            critical_violations = [
                ...
                if "CURL_THRESHOLD_EXCEEDED" in v
            ]
            
            if critical_violations:
                raise SecurityError(
                    f"PSI_FIELD_CRITICAL_VIOLATION: {critical_violations}",
                    violation_type="PSI_FIELD_CRITICAL_VIOLATION",
...
# =============================================================================
# SECURITY ENHANCEMENTS & BYZANTINE DETECTION
# =============================================================================

def create_secure_psi_field(
...
# =============================================================================
# EVIDENCE GENERATION & CROSS-RUNTIME VALIDATION
# =============================================================================

def generate_psi_field_evidence(
    field_engine: DiscretePsiField,
    harmonic_state: TokenStateBundle,
    delta_curl_threshold: int
) -> Dict[str, Any]:
    """
    ...
    validation_result = field_engine.validate_psi_field_integrity(
        harmonic_state, 
        delta_curl_threshold
    )
    
    evidence = {
        "psi_field_validation": validation_result,
        "topology_metrics": {
            "shard_count": len(field_engine.shard_ids),
            "total_edges": sum(len(neighbors) for neighbors in field_engine.graph.values()) // 2,
            "cycle_basis_size": len(field_engine.cycle_basis),
            "cycle_basis_hash": field_engine.cycle_basis_hash
        },
        "computation_bounds": {
            "max_cycle_length": field_engine.MAX_CYCLE_LENGTH,
            "max_cycles_per_shard": field_engine.MAX_CYCLES_PER_SHARD,
            "max_edges_per_shard": field_engine.MAX_EDGES_PER_SHARD
        },
        "security_status": {
            "cycle_basis_integrity": field_engine.verify_cycle_basis_integrity(),
...
if __name__ == "__main__":
    print("PsiFieldEngine.py is a hardened library module. Do not execute directly.")
    sys.exit(1)

__all__ = [
    "DiscretePsiField", 
    "create_secure_psi_field", 
    "generate_psi_field_evidence",
    "SecurityError"
]