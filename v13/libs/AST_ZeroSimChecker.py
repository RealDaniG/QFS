...
class ZeroSimASTVisitor(ast.NodeVisitor):
    ...
    def __init__(self, file_path: str):
        ...
        strict_deterministic_files = {'TreasuryEngine.py', 'RewardAllocator.py', 'StateTransitionEngine.py', 'NODAllocator.py', 'NODInvariantChecker.py', 'EconomicsGuard.py', 'CoherenceEngine.py', 'AEGIS_Node_Verification.py'}
        self.is_deterministic_module = base_name in strict_deterministic_files
        self.is_certified_math = base_name == 'CertifiedMath.py'
        self.is_economics_related = any((part in path_parts for part in ['economics', 'governance', 'reward']))
        self.forbidden_modules = {'random', 'time', 'datetime', 'secrets', 'uuid', 'os', 'sys', 'math', 'statistics', 'decimal', 'fractions', 'numpy', 'scipy', 'socket', 'requests', 'urllib', 'http', 'threading', 'asyncio', 'queue'}
        self.forbidden_functions = {'random', 'randint', 'choice', 'shuffle', 'uniform', 'gauss', 'getrandbits', 'time', 'sleep', 'perf_counter', 'now', 'today', 'utcnow', 'fromtimestamp', 'urandom', 'uuid4', 'float', 'round', 'pow', 'hash', 'open', 'input', 'print'}
    ...
    def add_violation(self, node, violation_type, message):
        ...
        self.violations.append(Violation(file_path=self.file_path, line_number=line, column=col, violation_type=violation_type, message=message, code_snippet=snippet))
    ...
    def visit_BinOp(self, node: ast.BinOp):
        if isinstance(node.op, ast.Div):
            ...
            if op_name in ('Add', 'Sub', 'Mult'):
                self.add_violation(node, 'UNCERTIFIED_ARITHMETIC', f'Direct arithmetic forbidden in economics. Use CertifiedMath.{op_name.lower()}()')
        self.generic_visit(node)
    ...
    def visit_FunctionDef(self, node):
        ...
        if self.is_deterministic_module and (not node.name.startswith('_')):
            if 'deterministic_timestamp' not in self.current_function_args:
                self.add_violation(node, 'MISSING_TIMESTAMP_PARAM', f'Missing param: deterministic_timestamp (required in {os.path.basename(self.file_path)})')
        self.generic_visit(node)
        self.inside_function = False
...
class AST_ZeroSimChecker:
    ...
    def scan_file(self, file_path: str) -> List[Violation]:
        ...
        try:
            ...
            visitor.visit(tree)
            uses_timestamp = any((isinstance(node, ast.arg) and node.arg == 'deterministic_timestamp' for node in ast.walk(tree)))
            ...
            if visitor.is_deterministic_module or visitor.is_economics_related:
                visitor.add_violation(ast.Module(), 'SYNTAX_ERROR', f'Syntax error: {e}')
        except Exception as e:
            logger.warning(f'Error parsing {file_path}: {e}')
            if visitor.is_deterministic_module or visitor.is_economics_related:
                visitor.add_violation(ast.Module(), 'PARSING_ERROR', f'Parsing error: {e}')
...
=======
    def scan_directory(self, directory: str, exclude_patterns: List[str]=None) -> List[Violation]:
        exclude_patterns = exclude_patterns or ['__pycache__', 'test_*', '*_test.py', 'AST_ZeroSimChecker.py', 'migrations', 'tests', 'audit', '*env*', 'venv', '.venv', 'scripts', 'checks_tests', 'qfs_v13_project', 'api', 'ATLAS', 'node_modules', 'epoch', 'economics/simple_violations.py', 'tools_root', 'tests_root', 'legacy_root', 'atlas_api', 'genesis_ledger.py', 'CertifiedMath.py']
