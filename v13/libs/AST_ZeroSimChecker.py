...
class ZeroSimASTVisitor(ast.NodeVisitor):
    def __init__(self, file_path: str):
        ...
        self.is_economics_related = any(
            (part in path_parts for part in ["economics", "governance", "reward"])
        )
            ...
            for target in sorted(node.targets):
                if isinstance(target, ast.Name):
                    ...
                    self.add_violation(
                        node,
                        "GLOBAL_MUTATION",
                        f"Global assignment to '{target.id}' forbidden",
                    )
    ...
    def visit_Global(self, node: ast.Global):
        self.add_violation(
            node,
            "GLOBAL_KEYWORD",
            f"'global' keyword forbidden: {', '.join(node.names)}",
        )
        self.generic_visit(node)
    ...
    def visit_BinOp(self, node: ast.BinOp):
        if isinstance(node.op, ast.Div):
            self.add_violation(
                node,
                "FORBIDDEN_OPERATION",
                "Use CertifiedMath.idiv(), not / (produces float)",
            )
        elif isinstance(node.op, ast.FloorDiv):
            self.add_violation(
                node,
                "FORBIDDEN_OPERATION",
                "Use CertifiedMath.idiv(), not // (can produce float)",
            )
        elif isinstance(node.op, ast.Pow):
            self.add_violation(
                node, "FORBIDDEN_OPERATION", "Use CertifiedMath.pow(), not **"
            )
    ...
    def visit_For(self, node: ast.For):
        if self._is_dict_like(node.iter) and (not self._is_sorted_call(node.iter)):
            self.add_violation(
                ...
                "Dict/set iteration must use sorted()",
            )
        self.generic_visit(node)
    ...
    def visit_Call(self, node: ast.Call):
        if isinstance(node.func, ast.Name):
            if node.func.id in self.forbidden_functions:
                self.add_violation(
                    node, "FORBIDDEN_CALL", f"Forbidden function: {node.func.id}"
                )
            if node.func.id == "hash":
                self.add_violation(
                    node, "FORBIDDEN_HASH", "hash() is randomized — forbidden"
                )
            ...
            if isinstance(obj, ast.Name) and obj.id in self.forbidden_modules:
                self.add_violation(
                    node, "FORBIDDEN_MODULE_CALL", f"{obj.id}.{attr} forbidden"
                )
            if (
                isinstance(obj, ast.Name)
                and obj.id == "importlib"
    ...
    def visit_ListComp(self, node):
        for gen in sorted(node.generators):
            if self._is_dict_like(gen.iter) and (not self._is_sorted_call(gen.iter)):
                self.add_violation(
                    node,
                    "NONDETERMINISTIC_COMP",
                    "Dict iteration in comprehension must use sorted()",
                )
        self.generic_visit(node)
    ...
    def visit_Yield(self, node):
        if self.is_deterministic_module:
            self.add_violation(
                node,
                "FORBIDDEN_GENERATOR",
                "Generators forbidden in deterministic modules",
            )
        self.generic_visit(node)
    ...
    def visit_AsyncFor(self, node):
        ...
        if self._is_dict_like(node.iter) and (not self._is_sorted_call(node.iter)):
            self.add_violation(
                node,
                "NONDETERMINISTIC_ITERATION",
                "Async dict/set iteration must use sorted()",
            )
        self.generic_visit(node)
    ...
    def visit_Import(self, node):
        for alias in sorted(node.names):
            ...
            if alias.name == "*":
                self.add_violation(
                    node, "WILDCARD_IMPORT", "Wildcard imports forbidden"
                )
        self.generic_visit(node)
    ...
    def visit_Attribute(self, node):
        if isinstance(node.ctx, ast.Store) and (not self.inside_function):
            self.add_violation(
                node,
                "GLOBAL_ATTR_MUTATION",
                f"Global attribute assignment forbidden: {node.attr}",
            )
        self.generic_visit(node)
    ...
    def visit_GeneratorExp(self, node):
        if self.is_deterministic_module:
            self.add_violation(
                node,
                "FORBIDDEN_GENERATOR",
                "Generator expressions forbidden in deterministic modules",
            )
        self.generic_visit(node)
...
class AST_ZeroSimChecker:
    def scan_file(self, file_path: str) -> List[Violation]:
        ...
        try:
            ...
            if (
                uses_timestamp
                and visitor.is_deterministic_module
                and (not visitor.is_certified_math)
            ):
                has_det_time = False
                for node in ast.walk(tree):
                    if isinstance(node, ast.ImportFrom) and "DeterministicTime" in (
                        node.module or ""
                    ):
                        has_det_time = True
                        break
                ...
                if not has_det_time:
                    logger.debug(
                        f"File {file_path} uses deterministic_timestamp but doesn't import DeterministicTime"
                    )
    ...
    def scan_directory(
        ...
        exclude_patterns = exclude_patterns or [
            ...
            "*node_modules*",
            "*epoch*",
            "*economics/simple_violations.py*",
            "*tools_root*",
            "*tests_root*",
            "*legacy_root*",
            "*examples*",
        ...
        for root, dirs, files in os.walk(directory):
            excluded = [
                d
                for d in dirs
                if any((fnmatch.fnmatch(d, p) for p in exclude_patterns))
            ]
            if excluded:
                pass
            dirs[:] = [
