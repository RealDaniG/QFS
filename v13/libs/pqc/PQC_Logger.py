...
class PQC_Logger:
    ...
    @staticmethod
    def log_pqc_operation(operation: str, details: Dict[str, Any], log_list: List[Dict[str, Any]], pqc_cid: Optional[str]=None, quantum_metadata: Optional[Dict[str, Any]]=None, deterministic_timestamp: int=0, error: Optional[Exception]=None) -> None:
        """
        ...
        entry = {'log_index': log_index, 'operation': operation, 'details': details, 'pqc_cid': pqc_cid, 'quantum_metadata': quantum_metadata, 'timestamp': deterministic_timestamp, 'system_fingerprint': PQC_Logger.SYSTEM_FINGERPRINT, 'prev_hash': PQC_Logger.ZERO_HASH}
        if error:
