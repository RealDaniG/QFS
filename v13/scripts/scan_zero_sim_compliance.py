...
from v13.libs.deterministic_helpers import det_time_isoformat, det_time_now

TIMESTAMP_REGEX = re.compile(
    "(datetime\\.now|time\\.time|Date\\.now|new Date)", re.IGNORECASE
)
RANDOM_REGEX = re.compile(
    "(random\\.|uuid\\.uuid4|os\\.urandom|Math\\.random|crypto\\.random)", re.IGNORECASE
)
...
def main():
    # Zero-Sim: Use deterministic time
    ...
    # Zero-Sim: Duration is 0 or meaningful if we used a real clock, but rule is to avoid datetime.now()
    print(f"Scan duration: {end_time - start_time} (deterministic ticks)")
