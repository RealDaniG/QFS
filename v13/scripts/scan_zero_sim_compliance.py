...
TIMESTAMP_REGEX = re.compile(r'(datetime\.now|time\.time|Date\.now|new Date)', re.IGNORECASE)
RANDOM_REGEX = re.compile(r'(random\.|uuid\.uuid4|os\.urandom|Math\.random|crypto\.random)', re.IGNORECASE)
FLOAT_REGEX = re.compile(r'(float\(|/ \d|\d \/|%)') # Simplified float check, will need AST for better precision in Python
...
EXCLUDED_DIRS = [
    '__pycache__', 'node_modules', '.git', '.idea', '.vscode', 'dist', 'build', 'coverage',
    'venv', 'env', '.gemini', 'artifacts','legacy_root', 'archive'
]

NO_SCAN_FILES = [
    'scan_zero_sim_compliance.py', 'run_zero_sim_suite.py'
]
...
def scan_test_file(filepath):
    ...
    scan_python_file(filepath) # Reuse python scanner but maybe downgrade severity for tests?
...
def suggest_fix(v_type):
    if "RANDOM" in v_type: return "Use deterministic PRNG or hash-based ID."
    if "TIME" in v_type: return "Use passed-in timestamp or ledger block time."
    if "IO" in v_type: return "Remove I/O. Pass data as arguments."
    if "PRISMA" in v_type: return "Remove DB write. Send intent to QFS."
...
def walk_and_scan(root_dir):
    for root, dirs, files in os.walk(root_dir):
        ...
        for file in files:
            if file in NO_SCAN_FILES: continue
