...
async def verify_trust_loop():
    ...
    entry_login_a = GenesisEntry(wallet=alice_wallet, event_type='LOGIN', value=0, metadata={'user_id': alice_id, 'action': 'connect'})
    await ledger.append(entry_login_a)
    ref_code = alice_id
    print(f'   [Step C] Bob Joins with Ref Code: {ref_code}...')
    entry_login_b = GenesisEntry(wallet=bob_wallet, event_type='LOGIN', value=0, metadata={'user_id': bob_id, 'referred_by': ref_code})
    await ledger.append(entry_login_b)
    print('   [Step D] Logging REFERRAL_USE...')
    entry_ref = GenesisEntry(wallet=bob_wallet, event_type='REFERRAL_USE', value=0, metadata={'referrer_wallet': alice_wallet, 'referral_code': ref_code, 'program_id': 'v1_beta'})
    ...
    with CertifiedMath.LogContext() as log_list:
        ...
        new_bundle = engine.apply_hsmf_transition(current_bundle=bundle, log_list=log_list, processed_events=engine_events)
