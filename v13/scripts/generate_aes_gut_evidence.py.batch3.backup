...
def generate_evidence():
    ...
    metadata = {'width': 1618, 'height': 1000, 'palette': [{'hue': 0}, {'hue': 137507764}], 'elements': [{'x': 809, 'y': 500, 'type': 'circle'}], 'structures': [{'scale': 10, 'pattern_id': 'p1'}, {'scale': 100, 'pattern_id': 'p1'}]}
    ...
    evidence = {'evidence_type': 'AES_GUT_DETERMINISM', 'timestamp': timestamp, 'status': 'VERIFIED', 'policy_version': policy.policy.version, 'input_metadata_hash': hashlib.sha256(json.dumps(metadata, sort_keys=True).encode()).hexdigest(), 'output_vector': vector_dict, 'verification_check': 'PASS' if vector.vector_hash else 'FAIL'}
