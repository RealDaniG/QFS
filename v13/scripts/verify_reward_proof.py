...
def verify_proof(proof_data: dict) -> bool:
    ...
    try:
        # Extract fields
        # Note: 'entry_data_snapshot' in JSON is 'entry_data' in hash preimage
        ...
        # Reconstruct element for hashing
        # This MUST match v13.core.CoherenceLedger.CoherenceLedger._generate_entry_hash
        data_to_hash = {
        ...
        # Canonical JSON serialization (matching CoherenceLedger.py)
        ...
        if calculated_hash == expected_hash:
            ...
            # Debug tip
            # print(f"DEBUG: Hashed Content: {data_json}")
            return False
...
def main():
    ...
    try:
        ...
        # Locate the proof object
        proof = None
        
        # 1. Check "explanation" -> "zero_sim_proof" (Standard Gateway Response)
        if "explanation" in data and "zero_sim_proof" in data["explanation"]:
            proof = data["explanation"]["zero_sim_proof"]
            
        # 2. Direct proof object check (Standalone)
        elif "input_state_hash" in data:
            proof = data
            
        # 3. Check "zero_sim_proof" key (Partial)
        elif "zero_sim_proof" in data:
            proof = data["zero_sim_proof"]

        # 4. Check "proof" key (Blocked/Error Response schema)
        elif "proof" in data:
            proof = data["proof"]
