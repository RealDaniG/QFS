...
def find_non_deterministic_ast(file_path: Path) -> List[Tuple[str, int]]:
    ...
    """
    patterns_to_check = {'random': ['random', 'randint', 'choice', 'shuffle'], 'time': ['time', 'sleep', 'time_ns'], 'uuid': ['uuid4', 'uuid1'], 'os_urandom': ['urandom'], 'datetime': ['now', 'utcnow'], 'math_special': ['inf', 'nan', 'isnan', 'isinf']}
    hits = []
    try:
...
def scan_src_modules() -> Dict[str, Dict]:
    ...
    for pyfile in sorted(SRC_DIR.rglob('*.py')):
        ...
        try:
            ...
            classes = re.findall('^\\s*class\\s+([A-Za-z0-9_]+)\\s*[\\(:]', text, re.MULTILINE)
            funcs = re.findall('^\\s*def\\s+([A-Za-z0-9_]+)\\s*\\(', text, re.MULTILINE)
            imports = re.findall('^\\s*import\\s+([A-Za-z0-9_\\.]+)', text, re.MULTILINE)
            from_imports = re.findall('^\\s*from\\s+([A-Za-z0-9_\\.]+)\\s+import', text, re.MULTILINE)
            non_det_ast = find_non_deterministic_ast(pyfile)
            non_det_keywords = list(set([p[0] for p in non_det_ast]))
            modules[str(rel)] = {'classes': classes, 'functions': funcs, 'imports': sorted(set(imports + from_imports)), 'non_det_patterns': non_det_keywords, 'non_det_details': non_det_ast, 'file_size': len(text)}
        except Exception as e:
...
def run_tests_deterministic() -> Dict:
    ...
    try:
        proc = subprocess.run(cmd, cwd=REPO_ROOT, env=env, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, timeout=300)
        output = proc.stdout
        PYTEST_OUTPUT_PATH.write_text(output, encoding='utf-8')
        tests_collected = 0
        match = re.search('(\\d+)\\s+test', output)
        if match:
            tests_collected = int(match.group(1))
        errors = output.count('ERROR')
        logger.info(f'Tests collected: {tests_collected}, Errors: {errors}')
        return {'cmd': ' '.join(cmd), 'exit_code': proc.returncode, 'output_path': str(PYTEST_OUTPUT_PATH.relative_to(REPO_ROOT)), 'tests_collected': tests_collected, 'errors': errors, 'raw_output': output}
...
def extract_test_names(pytest_output: str) -> Set[str]:
    ...
    tests = set()
    for match in re.finditer('(tests/[^\\s:]+)::', pytest_output):
        tests.add(match.group(1))
    return tests

def assess_component_status(config: Dict, modules: Dict, tests_found: Set[str], baseline_data: Dict) -> List[ComponentStatus]:
    """
    ...
    for comp in sorted(components):
        ...
        if not file_exists:
            ...
            status = 'UNKNOWN'
        statuses.append(ComponentStatus(name=name, file=file_path, status=status, tests_collected=len(matching_tests), evidence_found=found_evidence, non_det_patterns=non_det, criticality=criticality))
    return statuses

def compare_with_baseline(current_tests: int, current_errors: int) -> List[BaselineComparison]:
    """
    ...
    try:
        ...
        if baseline_file.exists():
            ...
            if current_tests > baseline_collected:
                ...
                test_status = 'PASS'
            comparisons.append(BaselineComparison(metric='Tests collected', baseline_value=baseline_collected, current_value=current_tests, status=test_status))
            if current_errors > baseline_errors:
                ...
                error_status = 'PASS'
            comparisons.append(BaselineComparison(metric='Collection errors', baseline_value=baseline_errors, current_value=current_errors, status=error_status))
    except Exception as e:
        logger.warning(f'Could not load baseline for comparison: {e}')
    return comparisons

def generate_verdict(components: List[ComponentStatus], test_results: Dict, comparisons: List[BaselineComparison], config: Dict) -> AuditVerdict:
    """
    ...
    for comp in sorted(components):
        if comp.status == 'UNKNOWN':
            ...
            recommendations.append(f'Complete {comp.name} implementation with evidence artifacts')
    if exit_code >= 2:
        overall_status = 'FAIL'
    elif exit_code >= 1:
        overall_status = 'WARN'
        ...
        overall_status = 'PASS'
    return AuditVerdict(overall_status=overall_status, critical_issues=critical_issues, recommendations=recommendations, exit_code=exit_code)

def generate_markdown_report(config: Dict, modules: Dict, components: List[ComponentStatus], test_results: Dict, comparisons: List[BaselineComparison], verdict: AuditVerdict) -> str:
    ...
    lines.extend(['', '---', ''])
    lines.extend(['## Component Implementation Status', '', '| Component | Status | Tests | Evidence | Non-Det | Criticality |', '|-----------|--------|-------|----------|---------|-------------|'])
    for comp in sorted(components, key=lambda c: c.criticality, reverse=True):
        status_icon = {'IMPLEMENTED': '✅', 'PARTIALLY_IMPLEMENTED': '🟡', 'UNKNOWN': '❓', 'MISSING': '❌'}.get(comp.status, '?')
        non_det_str = ', '.join(comp.non_det_patterns[:2]) if comp.non_det_patterns else '-'
        lines.append(f'| {comp.name} | {status_icon} {comp.status} | {comp.tests_collected} | {len(comp.evidence_found)} | {non_det_str} | {comp.criticality} |')
    lines.extend(['', '---', ''])
    ...
    if comparisons:
        ...
        lines.append('')
    lines.extend(['## Non-Determinism Analysis', ''])
    ...
    if critical_with_issues:
        ...
        lines.append('')
    lines.extend(['---', '', '**Report generated by:** QFS V13.5 Autonomous Audit v2.0', '**Exit code:** ' + str(verdict.exit_code)])
    return '\n'.join(lines)

def generate_json_report(components: List[ComponentStatus], verdict: AuditVerdict) -> Dict:
    """Generate machine-readable JSON report for CI integration."""
    return {'audit_version': '2.0', 'timestamp': datetime.utcnow().isoformat() + 'Z', 'git_commit': get_git_commit(), 'verdict': {'overall_status': verdict.overall_status, 'exit_code': verdict.exit_code, 'critical_issues': verdict.critical_issues, 'recommendations': verdict.recommendations}, 'components': [asdict(c) for c in components], 'non_det_patterns': {c.name: c.non_det_patterns for c in components if c.non_det_patterns}}
...
def run_audit():
    ...
    comparisons = compare_with_baseline(test_results.get('tests_collected', 0), test_results.get('errors', 0))
    logger.info('\n[5/6] Generating verdict...')
    verdict = generate_verdict(components, test_results, comparisons, config)
    logger.info('\n[6/6] Generating reports...')
    md_report = generate_markdown_report(config, modules, components, test_results, comparisons, verdict)
