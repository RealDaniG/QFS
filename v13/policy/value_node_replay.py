...
=======
from v13.policy.value_node_explainability import ValueNodeExplainabilityHelper, ValueNodeRewardExplanation, ContentRankingExplanation
...
=======
    def explain_specific_reward(self, reward_event_id: str, events_context: List[Dict[str, Any]]) -> Optional[ValueNodeRewardExplanation]:
>>>>>>> b27f784 (fix(ci/structure): structural cleanup and genesis_ledger AST fixes)
        """
...
=======
        ...
        timestamp = target_event.get('timestamp', 0)
        log_details = target_event.get('log_details', {})
        base_reward = log_details.get('base_reward', target_event.get('base_reward', {}))
        bonuses = log_details.get('bonuses', [])
        caps = log_details.get('caps', [])
        guards = log_details.get('guards', [])
        if not base_reward and 'amount_atr' in target_event:
            base_reward = {'ATR': f"{target_event['amount_atr']} ATR"}
        explanation = self.explainability_helper.explain_value_node_reward(wallet_id=wallet_id, user_id=user_id, reward_event_id=reward_event_id, epoch=epoch, base_reward=base_reward, bonuses=bonuses, caps=caps, guards=guards, timestamp=timestamp)
        return explanation

    def explain_content_ranking(self, content_id: str, events_context: List[Dict[str, Any]]) -> Optional[ContentRankingExplanation]:
>>>>>>> b27f784 (fix(ci/structure): structural cleanup and genesis_ledger AST fixes)
        """
...
=======
        interactions = [i for i in self.graph.interactions if i.content_id == content_id]
        interaction_count = len(interactions)
        interaction_score = sum((i.weight for i in interactions))
        signals = [{'name': 'Interaction Volume', 'weight': 0.5, 'score': min(interaction_count / 10.0, 1.0), 'description': f'{interaction_count} total interactions'}, {'name': 'Interaction Quality', 'weight': 0.5, 'score': min(interaction_score / 20.0, 1.0), 'description': 'Weighted score from likes/replies'}]
>>>>>>> b27f784 (fix(ci/structure): structural cleanup and genesis_ledger AST fixes)
        import hashlib
...
=======
        neighbors = [{'metric': 'Overall Score', 'value': h % 100 / 100.0, 'rank': h % 20 + 1}]
        final_rank = h % 20 + 1
        creation_event = next((e for e in events_context if e.get('type') == 'ContentCreated' and e.get('content_id') == content_id), None)
        timestamp = creation_event.get('timestamp', 0) if creation_event else 0
        return self.explainability_helper.explain_content_ranking(content_id=content_id, epoch=1, signals=signals, neighbors=neighbors, final_rank=final_rank, timestamp=timestamp)
>>>>>>> b27f784 (fix(ci/structure): structural cleanup and genesis_ledger AST fixes)
...
=======
        return {'users': {uid: u.__dict__ for uid, u in self.graph.users.items()}, 'contents': {cid: c.__dict__ for cid, c in self.graph.contents.items()}, 'interaction_count': len(self.graph.interactions), 'reward_count': len(self.graph.rewards), 'events_processed': self.processed_events_count}
>>>>>>> b27f784 (fix(ci/structure): structural cleanup and genesis_ledger AST fixes)
