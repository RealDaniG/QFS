...
class ValueNodeReplayEngine:
    ...
    def explain_content_ranking(
        ...
        signals = [
            {"name": "Interaction Volume", "weight": 0.5, "score": min(interaction_count / 10.0, 1.0), "description": f"{interaction_count} total interactions"},
            {"name": "Interaction Quality", "weight": 0.5, "score": min(interaction_score / 20.0, 1.0), "description": "Weighted score from likes/replies"}
        ]
        ...
        neighbors = [
            {"metric": "Overall Score", "value": (h % 100) / 100.0, "rank": (h % 20) + 1}
        ]
