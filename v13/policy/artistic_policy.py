...
class ArtisticPolicy:
    ...
    dimension_weights: Dict[str, int]  # 7-dimensional weights (scaled by SCALE)
    max_bonus_ratio: int # Scaled by SCALE
    per_user_daily_cap_atr: float # Keep as float for now for ledger compatibility, or string representation?
    ...
    def _calculate_hash(self) -> str:
        ...
        policy_data = {
            ...
            "mode": self.mode,
            "dimension_weights": {k: v for k, v in sorted(self.dimension_weights.items())},
...
class ArtisticBonusCalculation:
    ...
    bonus_factor: float # Normalized float for reward multiplier
    dimensions_used: Dict[str, int] # Integer scores
    weights_applied: Dict[str, int] # Integer weights
...
class ArtisticObservationStats:
    ...
    # Store only rolling window of last 100 samples to prevent memory leak
    dimension_distributions: Dict[str, List[int]] = field(default_factory=dict)
    average_confidence: int = 0 # Scaled int
...
class ArtisticSignalPolicy:
    ...
    def __init__(self, policy: Optional[ArtisticPolicy] = None):
        ...
        self.policy = policy or ArtisticPolicy(
            ...
            mode="rewarding",
            dimension_weights={
                "composition": 200000000,        # 0.20 * SCALE
                "color_harmony": 150000000,      # 0.15 * SCALE
                "symmetry": 100000000,           # 0.10 * SCALE
                "complexity": 100000000,         # 0.10 * SCALE
                "narrative": 150000000,          # 0.15 * SCALE
                "originality": 200000000,        # 0.20 * SCALE
                "resonance": 100000000           # 0.10 * SCALE
            },
            max_bonus_ratio=300000000, # 0.30 * SCALE
        ...
        for dimension in [
            "composition", "color_harmony", "symmetry", 
            "complexity", "narrative", "originality", "resonance"
        ]:
    ...
    def compute_vector(
        ...
        # Originality: deterministic comparison against historical patterns
        # (Simplified for Phase 6: inversely proportional to event count)
        originality = max(0, SCALE - (len(event_ids) * SCALE) // 100)
        
        # Resonance: event-weighted impact using φ-decay
        # Simplified: max(0, SCALE - decay)
    ...
    def calculate_artistic_bonus(
        self, 
        vector_input: Any, # Can be ArtisticVector or Dict
        confidence: int = SCALE, # Default to 1.0 * SCALE
        ...
        # Normalize input to Dict[str, int]
        if hasattr(vector_input, 'to_dict'):
            # It's an ArtisticVector
            dimensions = {
            ...
            # Assumed to be dict of scaled ints
        ...
        # AEGIS Integration: Adjust weights based on reputation
        if aegis_context:
            tier = aegis_context.get("reputation_tier", "new")
            if tier == "veteran":
                # +15% weight on Originality (0.15 * SCALE)
                boost = 150000000
        ...
        # Apply cap (policy.max_bonus_ratio is scaled int)
        final_bonus_scaled = min(base_bonus_scaled, self.policy.max_bonus_ratio)
        
        # Convert to float for final bonus factor
        ...
        return ArtisticBonusCalculation(
            ...
            cap_applied=cap_float if base_bonus_scaled > self.policy.max_bonus_ratio else None,
            policy_version=self.policy.version,
            reputation_adjustments=adjustments
        )
    
    def _update_observation_stats(self, dimensions: Dict[str, int], confidence: int, bonus: float):
        ...
        if len(self.observation_stats.bonus_distribution) > self.observation_stats.ROLLING_WINDOW_SIZE:
            self.observation_stats.bonus_distribution.pop(0)
        
        # Incremental average confidence
        # Use simple moving average for int confidence to avoid float drift issues? 
        # Or just keep it float for stats? Stats are allowed to be floaty.
        ...
        # int math: prev + (curr - prev) // n
        self.observation_stats.average_confidence = prev_avg + (confidence - prev_avg) // n
