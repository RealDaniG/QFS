...
class ArtisticPolicy:
    ...
    def _calculate_hash(self) -> str:
        ...
        policy_data = {'enabled': self.enabled, 'mode': self.mode, 'dimension_weights': {k: v for k, v in sorted(self.dimension_weights.items())}, 'max_bonus_ratio': self.max_bonus_ratio, 'per_user_daily_cap_atr': self.per_user_daily_cap_atr, 'version': self.version}
...
class ArtisticSignalPolicy:
    ...
    def compute_vector(self, content_id: str, content_metadata: Dict[str, Any], event_ids: List[str]) -> ArtisticVector:
        """
        ...
        return ArtisticVector(composition=composition, color_harmony=color_harmony, symmetry=symmetry, complexity=complexity, narrative=narrative, originality=originality, resonance=resonance, content_id=content_id, event_ids=sorted(event_ids), phi_weights=self.policy.dimension_weights, reason_codes=['generated_via_gut_extractors'])

    def calculate_artistic_bonus(self, vector_input: Any, confidence: int=SCALE, aegis_context: Optional[Dict[str, Any]]=None) -> ArtisticBonusCalculation:
        """
        ...
        return ArtisticBonusCalculation(bonus_factor=final_bonus_float, dimensions_used=dimensions, weights_applied=weights, cap_applied=cap_float if base_bonus_scaled > self.policy.max_bonus_ratio else None, policy_version=self.policy.version, reputation_adjustments=adjustments)
