...
class HumorObservabilityReport:
    ...
    dimension_distributions: Dict[str, Dict[str, float]]  # bucket -> count
    bonus_statistics: Dict[str, float]  # mean, median, std_dev, min, max
    anomaly_count: int
...
class HumorSignalObservatory:
    ...
    def __init__(self):
        ...
        self.bucket_size = 0.1  # For histogram distributions
    ...
    def get_observability_report(self, policy_version: str = "", policy_hash: str = "") -> HumorObservabilityReport:
        ...
        dimension_averages = {
            dim: dimension_sums[dim] / dimension_counts[dim]
            for dim in dimension_sums
        }
        
        # Calculate dimension distributions (histograms)
        dimension_distributions = {}
        ...
        # Count anomalies (simplified - in reality would integrate with policy anomaly detection)
        ...
        top_performing_content = [
            {
                ...
                "bonus_factor": signal.bonus_factor,
    ...
    def _calculate_histogram(self, dimension: str) -> Dict[str, float]:
        ...
        """
        # Filter signals that have this dimension
        dimension_scores = [
    ...
    def get_dimension_correlations(self) -> Dict[str, Dict[str, float]]:
        ...
        correlations = {}
        dimension_lists = {
            dim: [snapshot.dimensions.get(dim, 0.0) for snapshot in self.signal_history]
    ...
    def export_observability_data(self, policy_version: str = "", policy_hash: str = "") -> Dict[str, Any]:
        ...
        export_data = {
            ...
            "correlations": self.get_dimension_correlations(),
            "raw_data_sample": [
                {
                    ...
                    "bonus_factor": snapshot.bonus_factor
                }
                for snapshot in self.signal_history[-100:]  # Last 100 samples
            ]
        }
...
# Test function
def test_humor_observatory():
    """Test the humor observatory implementation."""
    print("Testing HumorSignalObservatory...")
    
    # Create observatory
    observatory = HumorSignalObservatory()
    ...
    for snapshot in test_snapshots:
        observatory.record_signal(snapshot)
    
    # Generate report
    report = observatory.get_observability_report()
    print(f"Report: {report.total_signals_processed} signals processed")
    print(f"Average confidence: {report.average_confidence:.3f}")
    print(f"Dimension averages: {list(report.dimension_averages.keys())}")
    
    # Check correlations
    correlations = observatory.get_dimension_correlations()
    print(f"Correlations calculated for {len(correlations)} dimensions")
    
    # Export data
    export_data = observatory.export_observability_data()
    print(f"Export data contains {len(export_data)} sections")
    
    print("✓ HumorObservatory test passed!")


if __name__ == "__main__":
    test_humor_observatory()