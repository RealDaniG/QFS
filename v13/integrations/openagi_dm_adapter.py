...
class OpenAGIDMAdapter:
    ...
    def dm_send_message(self, caller_id: str, thread_id: str, content: str, storage_uri: Optional[str]=None) -> Dict[str, Any]:
        """
        ...
        event = {'event_type': 'DM_MESSAGE_SENT', 'scope': self.scope, 'sender': caller_id, 'thread_id': thread_id, 'message_id': message_id, 'storage_uri': storage_uri, 'content_hash': deterministic_hash(content), 'timestamp': 'deterministic_time'}
        if self.scope == 'SIMULATION':
            return {'message_id': message_id, 'simulated': True, 'event': event}
        else:
            success = self.dm_service.send_message_signal(caller_id, 'recipient', storage_uri, deterministic_hash(content))
    ...
    def dm_get_message_history(self, caller_id: str, thread_id: str, limit: int=50) -> Dict[str, Any]:
        """
        ...
        messages = [{'message_id': 'msg_1', 'sender': caller_id, 'content_ref': 'ipfs://...'}]
        return {'messages': messages[:limit]}

    def _has_capability(self, user_id: str, capability: str) -> bool:
        """Check if user has required capability."""
        if self.scope == 'SIMULATION':
            return capability in ['DM_SEND', 'DM_READ_OWN', 'DM_CREATE_THREAD', 'DM_ADMIN_SIMULATE']
        return True
