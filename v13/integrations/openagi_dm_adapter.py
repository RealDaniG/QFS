...
class OpenAGIDMAdapter:
    ...
    def __init__(self, dm_service, scope: str = "SIMULATION"):
        ...
        self.scope = scope
        # In SIMULATION mode, auth is bypassed; in PRODUCTION, needs real ledger entries
    ...
    def dm_create_thread(self, caller_id: str, participant_id: str) -> Dict[str, Any]:
        """
        Open-AGI tool: Create a DM thread.

        Args:
            caller_id: Thread creator's wallet ID
            participant_id: Other participant's wallet ID

        Returns:
            Dict with thread_id and event or error
        """
        if not self._has_capability(caller_id, "DM_CREATE_THREAD"):
            return {"error": "UNAUTHORIZED"}
        # Generate deterministic thread_id from participants
        thread_id = f"thread_{deterministic_hash(f'{caller_id}_{participant_id}')[:16]}"
        event = {
        ...
        if self.scope == "SIMULATION":
            ...
            return {"thread_id": thread_id, "success": True, "event": event}
