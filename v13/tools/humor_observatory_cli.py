...
class HumorObservatoryCLI:
    ...
    def load_sample_data(self):
        ...
        sample_snapshots = [
            HumorSignalSnapshot(
                ...
                content_id=f"sample_content_{i}",
                dimensions={
                    "chronos": 0.5 + (i % 3) * 0.2,
                    "lexicon": 0.4 + (i % 4) * 0.15,
                    "surreal": 0.3 + (i % 5) * 0.1,
                    "empathy": 0.6 + (i % 2) * 0.2,
                    "critique": 0.5 + (i % 3) * 0.15,
                    "slapstick": 0.2 + (i % 4) * 0.2,
                    "meta": 0.4 + (i % 6) * 0.1
                },
                confidence=0.7 + (i % 4) * 0.05,
                bonus_factor=0.1 + (i % 5) * 0.03,
    ...
    def show_distributions(self):
        ...
        for dimension, distribution in report.dimension_distributions.items():
            ...
            sorted_buckets = sorted(distribution.items(), key=lambda x: float(x[0].split('-')[0]))
            for bucket, percentage in sorted_buckets:
                bar_length = int(percentage * 50)  # Scale to 50 characters
                bar = '█' * bar_length
    ...
    def show_top_content(self):
        ...
        for i, content in enumerate(report.top_performing_content, 1):
            # Find top dimension for this content
...
def main():
    ...
    parser.add_argument("--distributions", action="store_true", 
                        help="Show dimension distributions")
    parser.add_argument("--top-content", action="store_true", 
                        help="Show top performing content")
    parser.add_argument("--correlations", action="store_true", 
                        help="Show dimension correlations")
    parser.add_argument("--export", metavar="FILEPATH",
                        help="Export data to JSON file")
    
    # If no arguments provided, show summary by default
    if len(sys.argv) == 1:
