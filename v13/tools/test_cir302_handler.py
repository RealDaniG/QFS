...
class TestCIR302Handler:
    ...
    def handle_violation(self, error_type: str, error_details: str, log_list: list, 
...
def test_cir302_handler():
    ...
    # Test that it captures violations without halting
    try:
        handler.handle_violation(
            error_type="test_violation",
            error_details="Test violation for handler",
            log_list=[]
        )
    except SystemExit:
