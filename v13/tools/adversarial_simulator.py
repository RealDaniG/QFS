...
class AdversarialSimulator:
    ...
    def _create_vulnerable_bundle(self) -> TokenStateBundle:
        ...
        return create_token_state_bundle(chr_state={'coherence_metric': '0.99'}, flx_state={'scaling_metric': '0.15'}, psi_sync_state={'frequency_metric': '0.08'}, atr_state={'directional_metric': '0.85'}, res_state={'inertial_metric': '0.05'}, lambda1=BigNum128.from_int(1), lambda2=BigNum128.from_int(1), c_crit=BigNum128.from_int(1), pqc_cid='attack_test', timestamp=1234567890)
    ...
    def simulate_oracle_spoof(self) -> AttackResult:
        ...
        try:
            ...
            log_list = []
            malicious_f_atr = BigNum128.from_string('10.0')
            result = hsmf.validate_action_bundle(bundle, malicious_f_atr, 1, log_list, raise_on_failure=True)
    ...
    def simulate_pqc_replay(self) -> AttackResult:
        ...
        try:
            ...
            packet2 = DRV_Packet(1700000001, 100, 'def', log_list=[])
            is_valid = packet2.is_valid(public_key_bytes=b'pubkey', log_list=[], previous_packet=packet1)
    ...
    def simulate_coherence_crash(self) -> AttackResult:
        ...
        try:
            hsmf = HSMF(self.cm, self.cir302)
            bundle = self._create_vulnerable_bundle()
            bundle.chr_state['coherence_metric'] = '0.5'
            log_list = []
            result = hsmf.validate_action_bundle(bundle, BigNum128.from_string('0.1'), 1, log_list, raise_on_failure=True)
...
def main():
    ...
    parser.add_argument('--mode', choices=['dev', 'pre-release', 'release'], default='dev', help='Audit mode (default: dev)')
    parser.add_argument('--scenario', choices=['all', 'oracle_spoof', 'pqc_replay', 'partition'], default='all', help='Attack scenario (default: all)')
    parser.add_argument('--out', help='Output file path')
    parser.add_argument('--test_mode', choices=['true', 'false'], default='true', help='Test mode (default: true)')
    ...
    if args.scenario == 'all':
        ...
        attack_methods = {'oracle_spoof': simulator.simulate_oracle_spoof, 'pqc_replay': simulator.simulate_pqc_replay, 'partition': simulator.simulate_coherence_crash}
        if args.scenario in attack_methods:
            ...
            return
    result_data = {'mode': args.mode, 'scenario': args.scenario, 'attacks': [{'attack_name': r.attack_name, 'triggered_cir302': r.triggered_cir302, 'error_message': r.error_message, 'recovery_state': r.recovery_state} for r in results]}
    if args.out:
        import json
