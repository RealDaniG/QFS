...
class SessionManager:
    ...
    def _session_id(
        self, wallet_id: str, device_id: str, issued_at_block: int, scope: List[str]
    ) -> str:
    ...
    def rotate_session(
        self, old_token: SessionToken, current_block: int
    ) -> SessionToken:
        """Rotate an existing session, emitting SESSION_ROTATED."""
        # Create new session without emitting SESSION_STARTED
        ...
        new_token = SessionToken(
            session_id=session_id,
            wallet_id=old_token.wallet_id,
            device_id=old_token.device_id,
            issued_at_block=current_block,
            ttl_blocks=old_token.ttl_blocks,
            scope=old_token.scope,
        )
        rotate_data = {
            "old_session_id": old_token.session_id,
            "new_session_id": new_token.session_id,
            "block": current_block,
            "wallet_id": old_token.wallet_id,
            "device_id": old_token.device_id,
            "scope": old_token.scope,
    ...
    def revoke_session(
        self, token: SessionToken, reason: str, current_block: int
    ) -> None:
