...
def post_session_challenge(wallet_id: str, device_id: str, current_block: int) -> Tuple[str, str, int]:
    ...
    """
    # Generate a deterministic nonce based on inputs
    nonce_input = f"{wallet_id}||{device_id}||{current_block}"
    nonce = sha256(nonce_input)[:16]  # Shorten for practicality
    
    # Compute challenge
    challenge = compute_challenge(wallet_id, device_id, current_block, nonce)
    
    # Set expiry (typically TTL_BLOCKS from session manager)
    expiry_block = current_block + 100  # Standard TTL
    
    return challenge, nonce, expiry_block


def post_session_establish(wallet_id: str, device_id: str, challenge_response: str, 
                          current_block: int, session_manager: Any, ttl_blocks: int = 1000) -> Dict[str, Any]:
    ...
    # Verify challenge response (in a real implementation, this would involve signature verification)
    ...
    return {
        ...
        "session_token": {
            ...
            "issued_at_block": token.issued_at_block,
            "ttl_blocks": token.ttl_blocks,
            "scope": token.scope
        }
    }