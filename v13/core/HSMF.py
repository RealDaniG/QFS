...
class ValidationResult:
    ...
    def metrics(self) -> Dict[str, str]:
        ...
        return dict(
            sorted(((k, v.to_decimal_string()) for k, v in self.raw_metrics.items()))
        )
...
class HSMFProof:
    """
    Proof-of-Evaluation record for HSMF computations.

    Captures inputs, outputs, and metadata for auditing and replay.
    See docs/HSMF_MathContracts.md for schema specification.
    """
    ...
    # Inputs (as decimal strings for serialization)
    s_res: str
    s_flx: str
    s_psi_sync: str
    f_atr: str
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "outputs": {
                ...
                "c_holo": self.c_holo,
                "chr_reward": self.chr_reward,
                "flx_reward": self.flx_reward,
                "res_reward": self.res_reward,
                "psi_sync_reward": self.psi_sync_reward,
                "atr_reward": self.atr_reward,
...
class HSMF:
    ...
    def _emit_hsmf_poe(
        ...
        """
        Emit a structured HSMFProof entry into the log for PoE auditing.

        This is a pure side-effect on the provided log_list, maintaining
        Zero-Sim compliance (no external I/O, no randomness).

        Args:
            proof: The HSMFProof record to emit
            log_list: The audit log to append the proof to
        """
        log_list.append(
            {
                "op_name": "hsmf_proof",
    ...
    def _calculate_I_eff(
        self,
        s_chr: BigNum128,
        beta_penalty: BigNum128,
        log_list: List[Dict[str, Any]],
        quantum_meta: Optional[Dict[str, Any]],
        ...
        self.cm._log_operation(
            "calc_s_res",
            {"s_chr": s_chr, "beta": beta_penalty},
            s_res,
            log_list,
            "SRES_PQC" if pqc_cid is None else pqc_cid,
            quantum_meta,
        )
        return s_res
    ...
    def _safe_two_to_the_power(
        ...
        return self.cm.two_to_the_power(
            exponent, 50, log_list, pqc_cid, quantum_metadata
        )

    def _calculate_delta_lambda(
        self,
        ...
        if len(magnitudes) < 2:
            self.cm._log_operation(
                "calc_s_flx_insufficient_data",
                {"magnitudes_len": BigNum128.from_int(len(magnitudes))},
                total_deviation,
                log_list,
            )
        ...
        for i in range(1, len(magnitudes)):
            ...
            if mag_prev.value == 0:
                ...
                return self.ONE
            ratio_actual = self.cm.div(
                mag_curr, mag_prev, log_list, pqc_cid, quantum_meta
            )
            if self.cm.gte(ratio_actual, phi, log_list, pqc_cid, quantum_meta):
                deviation = self.cm.sub(
                    ratio_actual, phi, log_list, pqc_cid, quantum_meta
                )
            else:
                deviation = self.cm.sub(
                    phi, ratio_actual, log_list, pqc_cid, quantum_meta
                )
            abs_deviation = deviation
            total_deviation = self.cm.add(
                total_deviation, abs_deviation, log_list, pqc_cid, quantum_meta
            )
        self.cm._log_operation(
            "calc_s_flx_final",
            {"magnitudes_len": BigNum128.from_int(len(magnitudes)), "phi_const": phi},
            total_deviation,
            log_list,
            "SFLX_PQC" if pqc_cid is None else pqc_cid,
            quantum_meta,
        )
        return total_deviation

    def _calculate_delta_h(
        self,
        psi_state: Dict[str, BigNum128],
        drv_packet_sequence: BigNum128,
        log_list: List[Dict[str, Any]],
        quantum_meta: Optional[Dict[str, Any]],
        pqc_cid: Optional[str] = None,
    ) -> BigNum128:
        ...
        if self.cm.gte(
            expected_seq, drv_packet_sequence, log_list, pqc_cid, quantum_meta
        ):
            diff = self.cm.sub(
                expected_seq, drv_packet_sequence, log_list, pqc_cid, quantum_meta
            )
            ...
            diff = self.cm.sub(
                drv_packet_sequence, expected_seq, log_list, pqc_cid, quantum_meta
            )
        s_psi_sync = diff
        self.cm._log_operation(
            "calc_s_psi_sync_final",
            {"expected_seq": expected_seq, "drv_seq": drv_packet_sequence},
            s_psi_sync,
            log_list,
            "SPS_PQC" if pqc_cid is None else pqc_cid,
            quantum_meta,
        )
        return s_psi_sync

    def _check_atr_coherence(
        self,
        ...
        threshold = self.cm.mul(
            atr_magnitude, self.ONE_PERCENT, log_list, pqc_cid, quantum_meta
        )
        is_coherent = self.cm.lte(f_atr, threshold, log_list, pqc_cid, quantum_meta)
        self.cm._log_operation(
            "check_atr_coherence",
            {"f_atr": f_atr, "atr_mag": atr_magnitude},
            BigNum128.from_int(1 if is_coherent else 0),
            log_list,
            None,
            quantum_meta,
        )
    ...
    def _check_directional_encoding(
        self,
        f_atr_value: BigNum128,
        log_list: List[Dict[str, Any]],
        pqc_cid: Optional[str] = None,
    ) -> bool:
        """Check directional encoding (DEZ structural check) for HSMF validation."""
        is_valid = self.cm.gte(
            f_atr_value, self.ZERO, log_list, pqc_cid
        ) and self.cm.lte(f_atr_value, self.ONE, log_list, pqc_cid)
        self.cm._log_operation(
            "check_dez_structural",
            {"f_atr": f_atr_value},
    ...
    def _calculate_action_cost_qfs(
        self,
        s_res: BigNum128,
        s_flx: BigNum128,
        ...
        lambda2_s_psi_sync = self.cm.mul(
            lambda2, s_psi_sync, log_list, pqc_cid, quantum_meta
        )
        sum_terms = self.cm.add(s_res, lambda1_s_flx, log_list, pqc_cid, quantum_meta)
        sum_terms = self.cm.add(
            sum_terms, lambda2_s_psi_sync, log_list, pqc_cid, quantum_meta
        )
        action_cost = self.cm.add(sum_terms, f_atr, log_list, pqc_cid, quantum_meta)
        self.cm._log_operation(
            "calc_action_cost_qfs",
            {"s_res": s_res, "s_flx": s_flx, "s_psi_sync": s_psi_sync, "f_atr": f_atr},
            action_cost,
            log_list,
            "ACQFS_PQC" if pqc_cid is None else pqc_cid,
            quantum_meta,
        )
        return action_cost

    def _calculate_c_holo(
        self,
        s_res: BigNum128,
        s_flx: BigNum128,
        s_psi_sync: BigNum128,
        ...
        sum_dissonance = self.cm.add(
            sum_dissonance, s_psi_sync, log_list, pqc_cid, quantum_meta
        )
        one_plus_dissonance = self.cm.add(
            self.ONE, sum_dissonance, log_list, pqc_cid, quantum_meta
        )
        c_holo = self.cm.div(
            self.ONE, one_plus_dissonance, log_list, pqc_cid, quantum_meta
        )
        self.cm._log_operation(
            "calc_c_holo_final",
            {"total_dissonance": sum_dissonance},
            c_holo,
            log_list,
            "CHOLO_PQC" if pqc_cid is None else pqc_cid,
            quantum_meta,
        )
        return c_holo
    ...
    def apply_hsmf_transition(
        ...
        if not result.is_valid:
            error_details = (
                f"HSMF Transition Failed. Errors: {'; '.join(result.errors)}"
            )
            if self.cir302_handler:
                self.cir302_handler.handle_violation(
        ...
        if self.state_transition_engine:
            ...
            if not transition_result.success:
                error_details = (
                    f"Atomic State Transition Failed: {transition_result.error_message}"
                )
                if self.cir302_handler:
                    self.cir302_handler.handle_violation(
            ...
            if transition_result.new_token_bundle is None:
                error_details = (
                    f"Atomic State Transition Failed: No token bundle returned"
                )
                if self.cir302_handler:
                    self.cir302_handler.handle_violation(
                        "state_transition_failure",
            ...
            raise RuntimeError(
                "StateTransitionEngine not configured for atomic 5-token updates"
            )
    ...
    def _compute_hsmf_rewards(
        ...
        flx_reward = self.cm.mul(
            s_flx, action_cost, log_list, pqc_cid, quantum_metadata
        )
        res_reward = self.cm.mul(s_res, c_holo, log_list, pqc_cid, quantum_metadata)
        psi_sync_reward = self.cm.mul(
            s_psi_sync, f_atr, log_list, pqc_cid, quantum_metadata
        )
    ...
    def validate_action_bundle(
        ...
        strict_atr_coherence: bool = False,
        quantum_metadata: Optional[Dict[str, Any]] = None,
    ) -> ValidationResult:
        """
        ...
        if not dez_ok:
            ...
            is_valid = False
        survival_ok = self.cm.gte(
            token_bundle.get_coherence_metric(),
            token_bundle.c_crit,
            log_list,
            pqc_cid,
            quantum_metadata,
        ...
        if not survival_ok:
            ...
            is_valid = False
        atr_coherent = self._check_atr_coherence(
            token_bundle.atr_state, f_atr, log_list, quantum_metadata, pqc_cid
        )
        ...
        BETA_PENALTY = token_bundle.parameters.get(
        ...
        s_res = self._calculate_I_eff(
            s_chr, BETA_PENALTY, log_list, quantum_metadata, pqc_cid
        )
        s_flx = self._calculate_delta_lambda(
            token_bundle.flx_state, PHI_CONSTANT, log_list, quantum_metadata, pqc_cid
        )
        s_psi_sync = self._calculate_delta_h(
            token_bundle.psi_sync_state,
            BigNum128.from_int(drv_packet_sequence),
            log_list,
            quantum_metadata,
            pqc_cid,
        )
        action_cost = self._calculate_action_cost_qfs(
            s_res,
            s_flx,
            s_psi_sync,
            f_atr,
        ...
        c_holo = self._calculate_c_holo(
            s_res, s_flx, s_psi_sync, log_list, quantum_metadata, pqc_cid
        )
        self.cm._log_operation(
            "bundle_audit_ref",
            {"chr": s_chr, "c_crit": token_bundle.c_crit, "f_atr": f_atr},
            BigNum128.from_int(token_bundle.timestamp),
            log_list,
            None,
            {
                "version": token_bundle.bundle_id
                if hasattr(token_bundle, "bundle_id")
                else "unknown",
                "drv_id": token_bundle.pqc_cid
                if hasattr(token_bundle, "pqc_cid")
                else "unknown",
            },
        )
        if raise_on_failure and (not is_valid):
            ...
            if self.cir302_handler:
                ...
                raise RuntimeError(f"CIR-302: {error_details}")
        self.cm._log_operation(
            "validate_action_bundle_final_result",
            {},
            BigNum128.from_int(1 if is_valid else 0),
            log_list,
            pqc_cid,
