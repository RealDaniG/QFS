...
class TokenStateBundle:
    ...
    def __post_init__(self):
        ...
        if self.parameters is None:
            self.parameters = {'beta_penalty': BigNum128.from_int(100000000), 'phi': BigNum128(1618033988749894848)}
        if self.storage_metrics is None:
            self.storage_metrics = {'storage_bytes_stored': {}, 'storage_uptime_bucket': {}, 'storage_proofs_verified': {}}
        self._validate_states()
        self._validate_parameters()

    def _validate_states(self):
        """Validate that all token states are properly structured."""
        required_states = [self.chr_state, self.flx_state, self.psi_sync_state, self.atr_state, self.res_state, self.nod_state]
    ...
    def validate_signature(self, public_key_bytes: bytes) -> bool:
        ...
        try:
            ...
            result = PQC.verify_signature(public_key=public_key_bytes, data=data_to_verify, signature=signature_bytes, log_list=[], pqc_cid=f'{self.pqc_cid}_VERIFY', quantum_metadata=self.quantum_metadata, deterministic_timestamp=self.timestamp)
    ...
    def to_dict(self, include_signature: bool=True) -> Dict[str, Any]:
        ...
        bundle_data = {'chr_state': chr_state_serializable, 'flx_state': flx_state_serializable, 'psi_sync_state': psi_sync_state_serializable, 'atr_state': atr_state_serializable, 'res_state': res_state_serializable, 'nod_state': nod_state_serializable, 'storage_metrics': storage_metrics_serializable, 'timestamp': getattr(self, 'timestamp', 0), 'bundle_id': self.bundle_id, 'pqc_cid': self.pqc_cid, 'quantum_metadata': self.quantum_metadata, 'lambda1': self.lambda1.to_decimal_string(), 'lambda2': self.lambda2.to_decimal_string(), 'c_crit': self.c_crit.to_decimal_string(), 'parameters': parameters_serializable}
...
def create_token_state_bundle(chr_state: Dict[str, Any], flx_state: Dict[str, Any], psi_sync_state: Dict[str, Any], atr_state: Dict[str, Any], res_state: Dict[str, Any], nod_state: Dict[str, Any], lambda1: BigNum128, lambda2: BigNum128, c_crit: BigNum128, pqc_cid: str, timestamp: int, storage_metrics: Optional[Dict[str, Any]]=None, quantum_metadata: Optional[Dict[str, Any]]=None, bundle_id: Optional[str]=None, parameters: Optional[Dict[str, BigNum128]]=None) -> TokenStateBundle:
    """
    ...
    if bundle_id is None:
        temp_bundle = TokenStateBundle(chr_state=chr_state, flx_state=flx_state, psi_sync_state=psi_sync_state, atr_state=atr_state, res_state=res_state, nod_state=nod_state, storage_metrics=storage_metrics or {'storage_bytes_stored': {}, 'storage_uptime_bucket': {}, 'storage_proofs_verified': {}}, signature='', timestamp=timestamp, bundle_id='', pqc_cid=pqc_cid, quantum_metadata=quantum_metadata or {}, lambda1=lambda1, lambda2=lambda2, c_crit=c_crit, parameters=parameters or {'beta_penalty': BigNum128.from_int(100000000), 'phi': BigNum128(1618033988749894848)})
        bundle_id = temp_bundle.get_deterministic_hash(include_signature=False)
    return TokenStateBundle(chr_state=chr_state, flx_state=flx_state, psi_sync_state=psi_sync_state, atr_state=atr_state, res_state=res_state, nod_state=nod_state, storage_metrics=storage_metrics or {'storage_bytes_stored': {}, 'storage_uptime_bucket': {}, 'storage_proofs_verified': {}}, signature='', timestamp=timestamp, bundle_id=bundle_id, pqc_cid=pqc_cid, quantum_metadata=quantum_metadata or {}, lambda1=lambda1, lambda2=lambda2, c_crit=c_crit, parameters=parameters or {'beta_penalty': BigNum128.from_int(100000000), 'phi': BigNum128(1618033988749894848)})
...
def load_token_state_bundle(bundle_data: Dict[str, Any]) -> TokenStateBundle:
    ...
    if 'parameters' in bundle_data:
        ...
        parameters = {'beta_penalty': BigNum128.from_string('100000000.0'), 'phi': BigNum128.from_string('1.618033988749894848')}
    ...
    return TokenStateBundle(chr_state=bundle_data.get('chr_state', {}), flx_state=bundle_data.get('flx_state', {}), psi_sync_state=bundle_data.get('psi_sync_state', {}), atr_state=bundle_data.get('atr_state', {}), res_state=bundle_data.get('res_state', {}), nod_state=bundle_data.get('nod_state', {}), storage_metrics=storage_metrics, signature=bundle_data.get('signature', ''), timestamp=bundle_data.get('timestamp', 0), bundle_id=bundle_data.get('bundle_id', ''), pqc_cid=bundle_data.get('pqc_cid', ''), quantum_metadata=bundle_data.get('quantum_metadata', {}), lambda1=BigNum128.from_string(bundle_data.get('lambda1', '1.618033988749894848')), lambda2=BigNum128.from_string(bundle_data.get('lambda2', '0.618033988749894848')), c_crit=BigNum128.from_string(bundle_data.get('c_crit', '1.0')), parameters=parameters)