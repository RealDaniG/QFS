...
class ContentStorageExplanation:
    ...
    def _calculate_integrity_hash(self) -> str:
        ...
        data = {'content_id': self.content_id, 'nodes': sorted(self.storage_nodes), 'shards': sorted(self.shard_ids), 'epoch': self.epoch_assigned}
        json_str = json.dumps(data, sort_keys=True, separators=(',', ':'))
        return hashlib.sha256(json_str.encode('utf-8')).hexdigest()

    def _calculate_explanation_hash(self) -> str:
        """Calculate full explanation hash for governance audit."""
        data = {'content_id': self.content_id, 'storage_nodes': sorted(self.storage_nodes), 'shard_ids': sorted(self.shard_ids), 'replica_count': self.replica_count, 'proof_outcomes': self.proof_outcomes, 'epoch_assigned': self.epoch_assigned, 'integrity_hash': self.integrity_hash, 'policy_version': self.policy_version}
        json_str = json.dumps(data, sort_keys=True, separators=(',', ':'))
        return hashlib.sha256(json_str.encode('utf-8')).hexdigest()
...
def explain_storage_placement(content_id: str, storage_events: List[Dict[str, Any]]) -> ContentStorageExplanation:
    ...
    explanation = {'total_nodes': len(nodes), 'total_shards': len(shards), 'events_analyzed': len(storage_events)}
    return ContentStorageExplanation(content_id=content_id, storage_nodes=list(sorted(nodes.keys())), shard_ids=list(sorted(shards.keys())), replica_count=len(nodes), proof_outcomes=proofs, epoch_assigned=epoch)