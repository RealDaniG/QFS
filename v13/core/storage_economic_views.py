...
def compute_storage_metrics_from_events(events: List[Mapping[str, Any]]) -> StorageMetricsView:
    ...
    for i in range(len(events)):
        ...
        if event_type == 'PROOF_FAILED':
            proof_failures += 1
            continue
    return StorageMetricsView(epoch=current_epoch, bytes_stored_per_node=bytes_stored_per_node, proofs_generated_per_node=proofs_generated_per_node, proof_failures=proof_failures)