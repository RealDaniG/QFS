...
class AtlasAPIRouter:
    ...
    def route_get_governance_dashboard(self, start_timestamp: int = None, 
        ...
        try:
            ...
            if not role:
                return {
                    "error_code": "MISSING_ROLE",
                    "message": "Role is required",
                    "details": "The role parameter is mandatory for governance dashboard access"
                }
            
            # Validate role is an operator role
            try:
                open_agi_role = OPENAGIRole(role)
                # Only SYSTEM and PROPOSER roles can access governance dashboard
                if open_agi_role not in [OPENAGIRole.SYSTEM, OPENAGIRole.PROPOSER]:
                    return {
                        "error_code": "UNAUTHORIZED_ROLE",
                        "message": "Unauthorized role for governance dashboard",
                        "details": "Only SYSTEM and PROPOSER roles can access governance dashboard"
                    }
            except ValueError:
                return {
            ...
            auth_result = self.gateway.open_agi_enforcer.authorize_action(
                role=open_agi_role,
                action_type=OPENAGIActionType.READ_STATE,
                inputs={"dashboard_access": True},
                deterministic_timestamp=self.gateway._get_deterministic_timestamp()
            )
            ...
            if not auth_result.get("authorized", False):
                return {
                    ...
                    "details": auth_result.get("reason", "Not authorized to access governance dashboard")
                }
            ...
            if start_timestamp is not None:
                try:
                    ...
                    return {
                        ...
                        "details": "Start timestamp must be a valid integer"
                    }
            if end_timestamp is not None:
                try:
                    ...
                    return {
                        ...
                        "details": "End timestamp must be a valid integer"
                    }
            
            # Call gateway method
            result = self.gateway.get_governance_dashboard(
    ...
    def route_get_correlated_observations(self, content_id: str = None, event_id: str = None) -> dict:
        ...
        try:
            # Call gateway method
            result = self.gateway.get_correlated_observations(
    ...
    def route_submit_agi_observation(self, role: str, action_type: str, inputs: dict,
                                  suggested_changes: dict, explanation: str,
                                  correlation_to_aegis: dict = None) -> dict:
        """
        ...
        try:
            ...
            if not inputs or not isinstance(inputs, dict):
                return {
                    ...
                    "details": "The inputs parameter is mandatory and must be a dictionary"
                }
                
            if not suggested_changes or not isinstance(suggested_changes, dict):
                return {
                    "error_code": "MISSING_SUGGESTED_CHANGES",
                    "message": "Suggested changes are required",
                    "details": "The suggested_changes parameter is mandatory and must be a dictionary"
                }
                
            if not explanation or not explanation.strip():
                return {
                    "error_code": "MISSING_EXPLANATION",
                    "message": "Explanation is required",
                    "details": "The explanation parameter is mandatory and cannot be empty"
                }
            
            # Call gateway method
            result = self.gateway.submit_agi_observation(
                role=role,
                action_type=action_type,
                inputs=inputs,
