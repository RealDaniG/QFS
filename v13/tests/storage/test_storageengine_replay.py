    ...
    for ev in events:
        ...
        if event_type == "STORE":
            ...
            objects[object_key] = {
                "object_id": object_id,
                "version": version,
                "hash_commit": ev.get("hash_commit"),
                "content_size": ev.get("content_size"),
                "shard_ids": list(ev.get("shard_ids") or []),
        ...
        if event_type == "PROOF_FAILED":
            # Proof failure has no assigned nodes; record as a global count under a sentinel.
...
def _live_snapshot(engine: StorageEngine) -> Dict[str, Any]:
    ...
    for object_key, obj in sorted(engine.objects.items()):
        ...
        # content_size is not stored on LogicalObject; infer from shards.
...
def test_storageengine_replay_snapshot_matches_live() -> None:
    ...
    engine.put_content(
        ...
        version=1,
        content=b"A" * 1000,
        metadata={"author": "u1", "tags": ["x"]},
        deterministic_timestamp=10,
    )
    engine.put_content(
        object_id="obj_b",
        version=1,
        content=b"B" * 2000,
        metadata={"author": "u2", "tags": ["y"]},
        deterministic_timestamp=11,
    )
    ...
    # Exact match on object keys and placement sets.
...
def test_storageengine_replay_state_hash_matches_live() -> None:
    ...
    engine.put_content(
        ...
        version=1,
        content=b"Z" * 4096,
        metadata={"author": "u3", "tags": ["z"], "created_at_tick": 7},
        deterministic_timestamp=99,
    )
...
def test_storageengine_replay_proof_accounting_matches_live() -> None:
    ...
    store_res = engine.put_content(
        object_id="obj_proof",
        version=1,
        content=b"P" * 1024,
        metadata={"author": "u4"},
        deterministic_timestamp=123,
    )
    ...
    # Replay proof accounting should match live proof accounting for generated proofs.
