...
class TestPQCRealBackend:
    ...
    def test_deterministic_keygen(self):
        ...
        print(f'\n[TEST] {test_name}')
        try:
            seed = b'real_test_seed_1234567890123456789'
    ...
    def test_sign_verify_round_trip(self):
        ...
        print(f'\n[TEST] {test_name}')
        try:
            seed = b'sign_roundtrip_seed_1234567890123456'
            ...
            if is_valid:
                result = {'test_name': test_name, 'status': 'PASS', 'description': 'Sign/verify round-trip successful', 'payload_hash': hashlib.sha256(canonical_bytes).hexdigest()[:16] + '...', 'signature_length': len(signature), 'backend': self.backend_name}
                self.pass_count += 1
                print(f'  ✅ PASS: {test_name}')
            else:
    ...
    def test_tamper_detection(self):
        ...
        print(f'\n[TEST] {test_name}')
        try:
            seed = b'tamper_detect_seed_12345678901234567'
            private_key, public_key = self.adapter.keygen(seed)
            ...
            if not is_valid:
                result = {'test_name': test_name, 'status': 'PASS', 'description': 'Tamper detection working - invalid signature rejected', 'original_hash': hashlib.sha256(original_bytes).hexdigest()[:16] + '...', 'tampered_hash': hashlib.sha256(tampered_bytes).hexdigest()[:16] + '...', 'backend': self.backend_name}
                self.pass_count += 1
                print(f'  ✅ PASS: {test_name}')
            else:
    ...
    def test_wrong_key_detection(self):
        ...
        print(f'\n[TEST] {test_name}')
        try:
            seed1 = b'wrong_key_seed_1_123456789012345678'
            seed2 = b'wrong_key_seed_2_123456789012345678'
            ...
            if not is_valid:
                result = {'test_name': test_name, 'status': 'PASS', 'description': 'Wrong key detection working - verification failed with wrong key', 'backend': self.backend_name}
                self.pass_count += 1
                print(f'  ✅ PASS: {test_name}')
            else:
    ...
    def test_large_message_handling(self):
        ...
        print(f'\n[TEST] {test_name}')
        try:
            seed = b'large_msg_seed_1234567890123456789'
            private_key, public_key = self.adapter.keygen(seed)
            large_message = b'A' * (100 * 1024)
            signature = self.adapter.sign(private_key, large_message)
            is_valid = self.adapter.verify(public_key, large_message, signature)
            if is_valid:
                result = {'test_name': test_name, 'status': 'PASS', 'description': 'Large message handling successful', 'message_size_bytes': len(large_message), 'signature_length': len(signature), 'backend': self.backend_name}
                self.pass_count += 1
                print(f'  ✅ PASS: {test_name}')
            else:
    ...
    def generate_evidence_artifact(self):
        ...
        evidence = {'artifact_type': 'pqc_real_backend_verification', 'version': 'V13.6', 'test_suite': 'test_pqc_real_backend.py', 'layer': 'B - Real Backend', 'backend': self.backend_name, 'backend_info': backend_info, 'environment': {'QFS_REAL_PQC_AVAILABLE': os.environ.get('QFS_REAL_PQC_AVAILABLE', '0'), 'platform': sys.platform, 'python_version': sys.version}, 'timestamp': datetime.now(timezone.utc).isoformat() + 'Z', 'summary': {'total_tests': self.pass_count + self.fail_count, 'passed': self.pass_count, 'failed': self.fail_count, 'pass_rate_percent': self.pass_count / (self.pass_count + self.fail_count) * 100 if self.pass_count + self.fail_count > 0 else 0}, 'test_results': self.test_results}
