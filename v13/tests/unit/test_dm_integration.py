...
def test_identity_flow():
    ...
    uid = "0xUserA"
    pub = "pubkeyA"
    mgr.publish_identity(uid, pub, "sig_proof", timestamp=TEST_TIMESTAMP)
...
def test_messaging_flow():
    ...
    service.identity_mgr.publish_identity(
        sender, "pubS", "proofS", timestamp=TEST_TIMESTAMP
    )
    service.identity_mgr.publish_identity(
        recipient, "pubR", "proofR", timestamp=TEST_TIMESTAMP
    )
    uri = "ipfs://QmHash"
