...
def run_drv_timestamp_tests():
    ...
    try:
        ...
        assert result.error_code == ValidationErrorCode.INVALID_TTS_TIMESTAMP, (
            f"Wrong error code: {result.error_code}"
        )
    except Exception as e:
        results.append(False)
    ...
    try:
        log_list = []
        packet = DRV_Packet(
            ttsTimestamp=-1, sequence=1, seed="test_seed_12345", log_list=log_list
        )
        results.append(False)
        assert False, (
            "Negative timestamp should have been rejected during initialization"
        )
    except ValueError:
        results.append(True)
    except Exception as e:
