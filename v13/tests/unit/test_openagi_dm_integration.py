...
def test_simulation_role_create_thread():
    ...
    dm_service.identity_mgr.publish_identity(
        "sim_user_1", "pk1", "proof1", timestamp=1234567890
    )
    dm_service.identity_mgr.publish_identity(
        "sim_user_2", "pk2", "proof2", timestamp=1234567890
    )
...
def test_simulation_role_send_message():
    ...
    adapter = OpenAGIDMAdapter(dm_service, scope="SIMULATION")
    dm_service.identity_mgr.publish_identity(
        "sim_user_1", "pubkey1", "proof1", timestamp=1234567890
    )
    dm_service.identity_mgr.publish_identity(
        "sim_user_2", "pubkey2", "proof2", timestamp=1234567890
    )
...
def test_aegis_content_flag_blocks_send():
    ...
    # The error response comes from AEGIS/Governance, check if it has the expected fields
...
def test_dm_event_determinism():
    ...
    dm_service = DirectMessagingService()
    dm_service.identity_mgr.publish_identity(
        "user_a", "pk_a", "proof_a", timestamp=1234567890
    )
    dm_service.identity_mgr.publish_identity(
        "user_b", "pk_b", "proof_b", timestamp=1234567890
    )
    adapter = OpenAGIDMAdapter(dm_service, scope="SIMULATION")
...
def test_simulated_events_are_ledger_shaped():
    ...
    dm_service = DirectMessagingService()
    dm_service.identity_mgr.publish_identity(
        "user_1", "pk1", "proof1", timestamp=1234567890
    )
    adapter = OpenAGIDMAdapter(dm_service, scope="SIMULATION")
...
def test_production_mode_no_simulation_tag():
    ...
    dm_service.identity_mgr.publish_identity(
        "user_2", "pk2", "proof2", timestamp=1234567890
    )
    adapter = OpenAGIDMAdapter(dm_service, scope="PRODUCTION")
