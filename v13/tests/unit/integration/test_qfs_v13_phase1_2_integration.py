...
<<<<<<< HEAD
    ...
    f_atr_result = oracle.get_f_atr(
        None
    )  # In real implementation, this would take a DRV_Packet
...
=======
    ...
    token_bundle = TokenStateBundle(chr_state=chr_state, flx_state={'flux_metric': '0.15'}, psi_sync_state={'psi_sync_metric': '0.08'}, atr_state={'atr_metric': '0.85'}, res_state={'resonance_metric': '0.05'}, signature='test_signature', timestamp=int(det_time_now()), bundle_id='test_bundle_id', pqc_cid='test_pqc_cid', quantum_metadata={'test': 'data'}, lambda1=BigNum128.from_string('0.3'), lambda2=BigNum128.from_string('0.2'), c_crit=BigNum128.from_string('0.9'))
    print('✓ TokenStateBundle created successfully')
    print(f'  Bundle ID: {token_bundle.bundle_id}')
    print(f'  C_CRIT: {token_bundle.c_crit.to_decimal_string()}')
    print('\n2. Testing UtilityOracleInterface...')
...
=======
            self.raw_metrics = {'c_holo': BigNum128.from_string('0.95'), 's_flx': BigNum128.from_string('0.15'), 's_psi_sync': BigNum128.from_string('0.08'), 'f_atr': f_atr_result.f_atr, 's_chr': BigNum128.from_string('0.98')}
>>>>>>> b27f784 (fix(ci/structure): structural cleanup and genesis_ledger AST fixes)
