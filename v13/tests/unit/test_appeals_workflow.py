...
def test_appeal_resolution():
    mgr = AppealManager()
    appeal = mgr.submit_appeal(
        "0xUser", "event_123", "ipfs://Qm123", "...", timestamp=1000
    )
    aid = appeal["id"]
    resolved = mgr.resolve_appeal(
        aid, "ACCEPTED", "0xCouncil", "ipfs://QmExplanation", timestamp=2000
    )
...
def test_invalid_decision():
    ...
    with pytest.raises(ValueError, match="Invalid decision"):
        mgr.resolve_appeal(
            appeal["id"], "MAYBE", "0xCouncil", "ipfs://...", timestamp=2000
        )


def test_list_pending():
    mgr = AppealManager()
    mgr.submit_appeal("0xUser1", "e1", "cid1", "r1", timestamp=1000)
    mgr.submit_appeal("0xUser2", "e2", "cid2", "r2", timestamp=1001)
    pending = mgr.list_pending()
    assert len(pending) == 2
    mgr.resolve_appeal(pending[0]["id"], "ACCEPTED", "0xC", "cid", timestamp=2000)
