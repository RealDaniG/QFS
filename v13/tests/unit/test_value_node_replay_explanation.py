...
@pytest.fixture
def engine():
    policy = HumorSignalPolicy(HumorPolicy(
        enabled=True,
        mode="rewarding",
        dimension_weights={},
...
def test_explain_full_fidelity_event(engine):
    ...
    event = {
        ...
        "timestamp": 12345,
        "log_details": {
            "base_reward": {"ATR": "80.0 ATR"},
            "bonuses": [{"label": "Loyalty", "value": "+20.0 ATR", "reason": "Old account"}],
            "caps": [],
            "guards": [{"name": "MaxReward", "result": "pass"}]
        }
    }
    
    # We pass the event as context
...
def test_explain_low_fidelity_event_fallback(engine):
    ...
    # Fallback behavior: base_reward constructed from amount_atr
...
def test_replay_integration_with_explanation(engine):
    ...
    """
    events = [
        {"type": "ContentCreated", "content_id": "c1", "user_id": "u1"},
        {"type": "RewardAllocated", "event_id": "r1", "user_id": "u1", "amount_atr": 10}
    ]
