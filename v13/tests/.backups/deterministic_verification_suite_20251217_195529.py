...
def test_certifiedmath_determinism() -> Dict[str, str]:
    ...
    assert results["pow"] == results["pow_replay"], (
        "Power function is not deterministic"
    )
...
def test_pqc_determinism() -> Dict[str, str]:
    ...
    with PQC.LogContext() as log1:
        keypair1 = PQC.generate_keypair(
            log_list=log1, seed=seed, deterministic_timestamp=1234567890
        )
    ...
    with PQC.LogContext() as log2:
        keypair2 = PQC.generate_keypair(
            log_list=log2, seed=seed, deterministic_timestamp=1234567890
        )
    ...
    # Verify deterministic behavior
    assert results["keypair1_pub"] == results["keypair2_pub"], (
        "Key generation is not deterministic"
    )
    assert results["signature1"] == results["signature2"], (
        "Signature generation is not deterministic"
    )
...
def test_drv_packet_determinism() -> Dict[str, str]:
    ...
    # Verify deterministic behavior
    assert results["packet1_hash"] == results["packet2_hash"], (
        "Packet creation is not deterministic"
    )
...
def test_token_state_bundle_determinism() -> Dict[str, str]:
    ...
    # Verify deterministic behavior
    assert results["bundle1_hash"] == results["bundle2_hash"], (
        "Bundle creation is not deterministic"
    )
...
def test_hsmf_determinism() -> Dict[str, str]:
    ...
    for key in bundle.flx_state:
        if isinstance(bundle.flx_state[key], str):
            ...
            bundle.flx_state[key] = [
                CertifiedMath.from_string(item) if isinstance(item, str) else item
                for item in bundle.flx_state[key]
            ]

    for key in bundle.psi_sync_state:
        if isinstance(bundle.psi_sync_state[key], str):
            bundle.psi_sync_state[key] = CertifiedMath.from_string(
                bundle.psi_sync_state[key]
            )
    ...
    with CertifiedMath.LogContext() as log1:
        result1 = hsmf.validate_action_bundle(
            token_bundle=bundle, f_atr=f_atr, drv_packet_sequence=10, log_list=log1
        )
    ...
    with CertifiedMath.LogContext() as log2:
        result2 = hsmf.validate_action_bundle(
            token_bundle=bundle, f_atr=f_atr, drv_packet_sequence=10, log_list=log2
        )
    ...
    # Verify deterministic behavior
    assert results["hsmf_result1_valid"] == results["hsmf_result2_valid"], (
        "HSMF validation is not deterministic"
    )
    assert results["hsmf_result1_dez"] == results["hsmf_result2_dez"], (
        "HSMF DEZ check is not deterministic"
    )
    assert results["hsmf_result1_survival"] == results["hsmf_result2_survival"], (
        "HSMF survival check is not deterministic"
    )
