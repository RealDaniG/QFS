...
def test_policy_hints_for_different_severities():
    ...
    test_cases = [
        {
            ...
            "expected_click_through": False
        },
        {
            "name": "Warning Advisory",
            "block_suggested": True,
            "severity": "warning",
            "expected_visibility": "warned",
            "expected_banner": "general",
            "expected_click_through": False
        },
        {
            "name": "Critical Advisory",
            "block_suggested": True,
            "severity": "critical",
            "expected_visibility": "hidden",
            "expected_banner": "safety",
            "expected_click_through": True
        }
    ]
    ...
    for test_case in test_cases:
        ...
        assert policy_hints["visibility_level"] == test_case["expected_visibility"], \
            f"Expected visibility '{test_case['expected_visibility']}', got '{policy_hints['visibility_level']}'"
        assert policy_hints["warning_banner"] == test_case["expected_banner"], \
            f"Expected banner '{test_case['expected_banner']}', got '{policy_hints['warning_banner']}'"
        assert policy_hints["requires_click_through"] == test_case["expected_click_through"], \
            f"Expected click_through '{test_case['expected_click_through']}', got '{policy_hints['requires_click_through']}'"
