...
from v13.libs.economics.TreasuryDistributionEngine import TreasuryDistributionEngine, create_treasury_distribution_engine
...
class TestTreasuryProportionalDistribution(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.engine = create_treasury_distribution_engine(self.math, self.pqc_signer, self.cir302, MagicMock(), MagicMock())

    def test_proportional_distribution(self):
        harmonic_state = MagicMock()
        harmonic_state.node_metrics = {'shard_A': {'uptime': 100}, 'shard_B': {'uptime': 300}}
        genesis_shards = ['shard_A', 'shard_B']
        treasury_balance = BigNum128.from_int(6000)
        timestamp = 1234567890
        packet_seq = {'seq': 1, 'packet_hash': 'abc', 'pqc_cid': 'cid'}
        result = self.engine.compute_system_treasury_distribution(harmonic_state, treasury_balance, genesis_shards, timestamp, packet_seq)
    ...
    def test_idempotency(self):
        ...
        timestamp = 100
        packet_seq = 50
        res1 = self.engine.compute_system_treasury_distribution(harmonic_state, treasury_balance, genesis_shards, timestamp, packet_seq)
        self.assertIn('distribution_map', res1)
        res2 = self.engine.compute_system_treasury_distribution(harmonic_state, treasury_balance, genesis_shards, timestamp, packet_seq)
