...
from v13.services.sessions.session_challenge import (
    compute_challenge, 
    post_session_challenge, 
    post_session_establish
)
...
def test_session_challenge_computation():
    ...
    # Verify changing any input changes the challenge
...
def test_session_challenge_api():
    ...
    assert len(nonce) == 16  # Shortened nonce
    assert expiry_block == current_block + 100  # Standard TTL


def test_session_establish_success():
    """Test successful session establishment."""
    # Create fake ledger and session manager
    ledger = FakeLedger()
    ...
    # Establish session with correct challenge response
...
def test_session_establish_failure():
    ...
    # Try to establish session with wrong challenge
