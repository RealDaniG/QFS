...
def test_session_rotation_creates_new_token():
    ...
    initial_block = 100
    ttl_blocks = 1000
    initial_token = session_manager.create_session(wallet_id=wallet_id, device_id=device_id, scope=scope, current_block=initial_block, ttl_blocks=ttl_blocks)
    rotation_block = 150
    ...
    assert rotated_token.session_id != initial_token.session_id
    another_rotated_token = session_manager.rotate_session(initial_token, rotation_block)
    assert another_rotated_token.session_id == rotated_token.session_id
