...
class TestCIR302HandlerComprehensive:
    ...
    def test_econ_bound_violations(self):
        ...
        econ_violations = [
            ...
            ("ECON_NOD_ALLOCATION_FRACTION_VIOLATION", "NOD allocation fraction out of bounds"),
        ...
        for error_code, error_message in econ_violations:
            ...
            try:
                ...
                # This should never be reached due to sys.exit
                raise ValueError("handle_guard_violation did not exit")
                
            except SystemExit as e:
                # Expected behavior - CIR302 should exit with code 302
                if e.code == 302:
                    result = {
                        "test_name": test_name,
                        "status": "PASS",
    ...
    def test_combined_violations(self):
        ...
        try:
            ...
            # Test combined violation with multiple context details
            handler.handle_guard_violation(
                ...
                context={
                    ...
                    "violations": [
                        {"type": "ECON_CHR_MAX_REWARD_EXCEEDED", "value": "2.0", "limit": "1.0"},
                        {"type": "ECON_FLX_FRACTION_OUT_OF_BOUNDS", "value": "1.5", "range": "0.0-1.0"}
                    ],
    ...
    def test_edge_cases(self):
        ...
        try:
            ...
            large_context = {
                "module": "EconomicsGuard",
                "function": "validate_economic_bounds",
                "data": "A" * 10000,  # Large data field
                "details": [{"item": f"detail_{i}"} for i in range(100)]  # Many detail items
            }
    ...
    def generate_evidence_artifact(self):
        ...
        evidence = {
            ...
            "summary": {
                ...
                "total_error_codes_in_handler": len(CIR302_Handler.GUARD_ERROR_MAPPINGS)
            },
            "covered_codes": list(self.covered_error_codes),
            "missing_codes": list(set(CIR302_Handler.GUARD_ERROR_MAPPINGS.keys()) - self.covered_error_codes),
            "test_results": self.test_results,
            "coverage_analysis": {
                "branch_coverage": "100%" if self.fail_count == 0 else f"{(self.pass_count / (self.pass_count + self.fail_count) * 100):.1f}%",
                "code_coverage": f"{len(self.covered_error_codes) / len(CIR302_Handler.GUARD_ERROR_MAPPINGS) * 100:.1f}%",
                "status": "COMPLETE" if len(self.covered_error_codes) == len(CIR302_Handler.GUARD_ERROR_MAPPINGS) else "PARTIAL"
            }
        }
