...
class TestAEGISAdvisoryExposure:
    ...
    def test_safe_interaction_includes_advisory_summary(self):
        ...
        request = InteractionRequest(user_id='test_user', target_id='test_target', content='This is a safe, family-friendly comment about quantum computing.', reason='Testing safe interaction')
    ...
    def test_unsafe_interaction_includes_advisory_summary(self):
        ...
        request = InteractionRequest(user_id='test_user', target_id='test_target', content='This is explicit adult content that should be flagged.', reason='Testing unsafe interaction')
        response = self.gateway.post_interaction('comment', request)
    ...
    def test_spam_interaction_includes_advisory_summary(self):
        ...
        request = InteractionRequest(user_id='test_user', target_id='test_target', content='Buy now! Click here for free money! Urgent limited time offer! Act now! Buy now! Click here!', reason='Testing spam interaction')
        response = self.gateway.post_interaction('comment', request)
        assert response.success is False
        assert response.event_id is not None
        assert response.guard_results is not None
    ...
    def test_like_interaction_includes_advisory_summary(self):
        ...
        request = InteractionRequest(user_id='test_user', target_id='test_target', content='Great post!', reason='Testing like interaction')
