...
<<<<<<< HEAD
    ...
    first_time = perf_counter() - start_time

    # Second run (replay)
    start_time = perf_counter()
    second_hash = generate_performance_hash(test_data)
    second_time = perf_counter() - start_time
...
=======
    ...
    print(f'HSMF Validation: {hsmf_validation:.3f}ms (Requirement: < 50ms per op)')
    print(f'Deterministic Replay: {replay_deterministic} (Requirement: True)')
    all_requirements_met = certifiedmath_arithmetic < 1.0 and certifiedmath_transcendental < 10.0 and (pqc_keygen < 100.0) and (pqc_sign < 50.0) and (hsmf_validation < 50.0) and replay_deterministic
>>>>>>> b27f784 (fix(ci/structure): structural cleanup and genesis_ledger AST fixes)
