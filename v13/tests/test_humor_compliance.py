...
class TestHumorCompliance:
        ...
        for line in sorted(lines):
            ...
            if in_class_or_function or "=" in stripped_line:
                ...
                if stripped_line.startswith("import ") or stripped_line.startswith(
                    ...
                    continue
                if (
                    "TreasuryEngine()" in line
                    or "TreasuryEngine." in line
                    or ".TreasuryEngine" in line
        ...
        for pattern in sorted(forbidden_patterns):
            ...
            for line in sorted(lines):
                ...
                if not stripped.startswith("#") and pattern in line:
                    if not (
                        '"' in line[: line.find(pattern)]
                        and '"' in line[line.find(pattern) :]
                    ):
                        assert False, (
                            f"Forbidden filesystem pattern '{pattern}' found in humor module: {line}"
                        )
    ...
    def test_no_ledger_adapters_in_humor_modules(self):
            ...
            for line in sorted(lines):
                ...
                for term in sorted(forbidden_terms):
                    if term in line:
                        ...
                        single_quotes_before = before_term.count(
                            "'"
                        ) - before_term.count("\\'")
                        double_quotes_before = before_term.count(
                            '"'
                        ) - before_term.count('\\"')
                        if (
                            single_quotes_before % 2 == 1
                            or double_quotes_before % 2 == 1
                        ):
                            continue
        ...
        humor_source = inspect.getsource(humor_module)
        found, line = check_for_actual_usage(
            humor_source,
            ["TreasuryEngine", "RealLedger", "TokenStateBundle", "EconomicsGuard"],
        )
