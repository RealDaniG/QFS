...
def test_recording_and_capping():
    ...
    obs.MAX_HISTORY = 10
    for i in range(15):
        snap = ArtisticSignalSnapshot(timestamp=1000 + i, content_id=f'c{i}', dimensions={'originality': 0.5}, confidence=1, bonus_factor=0.1, policy_version='v1')
...
def test_report_generation():
    obs = ArtisticSignalObservatory()
    obs.record_signal(ArtisticSignalSnapshot(1, 'c1', {'originality': 0.8}, 1, 0.2, 'v1'))
    obs.record_signal(ArtisticSignalSnapshot(2, 'c2', {'originality': 0.2}, 1, 0.1, 'v1'))
