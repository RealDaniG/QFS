...
class TestGatewayAEGISIntegration:
    ...
    def test_safe_interaction_passes_through(self):
        ...
        request = InteractionRequest(user_id='test_user', target_id='test_target', content='This is a safe, family-friendly comment about quantum computing.', reason='Interesting post')
    ...
    def test_unsafe_interaction_is_blocked(self):
        ...
        request = InteractionRequest(user_id='test_user', target_id='test_target', content='This is explicit adult content that should be flagged.', reason='Inappropriate comment')
        response = self.gateway.post_interaction('comment', request)
        assert response.success is False
        assert response.event_id is not None
        assert response.reward_estimate is None
    ...
    def test_spam_interaction_is_blocked(self):
        ...
        request = InteractionRequest(user_id='test_user', target_id='test_target', content='Buy now! Click here for free money! Urgent limited time offer! Act now!', reason='Spam comment')
        response = self.gateway.post_interaction('comment', request)
        assert response.success is False
        assert response.event_id is not None
        assert response.reward_estimate is None
    ...
    def test_normal_interaction_creates_ledger_entry_with_advisory_info(self):
        ...
        request = InteractionRequest(user_id='test_user', target_id='test_target', content='This is a safe, educational comment.', reason='Helpful comment')
        initial_entry_count = len(self.gateway.coherence_ledger.ledger_entries)
        response = self.gateway.post_interaction('comment', request)
        final_entry_count = len(self.gateway.coherence_ledger.ledger_entries)
        assert final_entry_count == initial_entry_count + 1
