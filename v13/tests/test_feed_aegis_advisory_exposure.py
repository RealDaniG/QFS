...
class TestFeedAEGISAdvisoryExposure:
    ...
    def test_feed_items_include_advisory_summary(self):
        ...
        # Verify each post includes AEGIS advisory summary
        for post in response.posts:
            ...
            # Verify advisory values (should be safe content by default)
    ...
    def test_feed_with_mixed_content_advisory_summaries(self):
        ...
        # Check that we have posts with different advisory levels
        # (In a real test, we would inject specific content, but for now we just verify structure)
        has_info = False
        has_warning = False
        has_critical = False
        ...
        # At minimum, we should have info level advisories
