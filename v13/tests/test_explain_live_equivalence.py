...
# Fixed Event Stream (Mock Baseline)
@pytest.fixture
def mock_events():
    ...
    return [
        ...
        },
        {
            ...
            "user_id": "did:key:zUser1",
            "reason": "engagement",
            "amount_atr": 50.0,
            "bonuses": [{"source": "humor", "amount": 10.0}],
            "caps": [],
            "guards": []
        }
    ]
...
def live_replay_source(mock_events, monkeypatch):
    ...
    def fake_get_reward_events(self, wallet_id, epoch):
        # In a real live scenario, this would fetch from ledger.
        # Here we simulate the ledger RETURNING the exact same data to prove determinism.
        return mock_events
    
    monkeypatch.setattr(QFSReplaySource, "get_reward_events", fake_get_reward_events)
    
    # We don't need real ledger/storage for this test since we patched the retrieval method
...
def test_reward_explanation_hash_equivalence(mock_events, live_replay_source, explain_helper):
    ...
    # 1. Baseline: Direct Replay with Mock Data
    engine_baseline = ValueNodeReplayEngine(explainability_helper=explain_helper)
    engine_baseline.replay_events(mock_events)
    
    explanation_baseline = engine_baseline.explain_specific_reward(
        reward_event_id=reward_evt_id,
        events_context=mock_events
    )
    ...
    # 2. Live Candidate: Use QFSReplaySource (Patched)
    # This proves that the ADAPTER doesn't mutate data and the ENGINE handles it correctly.
    ...
    explanation_live = engine_live.explain_specific_reward(
        reward_event_id=reward_evt_id,
        events_context=events_from_source
    )
