...
class TestHumorPolicy:
    ...
    def test_normal_humor_bonus_calculation(self):
        ...
        dimensions = {
            "chronos": Fraction(4, 5),
            "lexicon": Fraction(3, 5),
            "surreal": Fraction(2, 5),
            "empathy": Fraction(9, 10),
            "critique": Fraction(7, 10),
            "slapstick": Fraction(3, 10),
            "meta": Fraction(1, 2),
        }
    ...
    def test_negative_or_malformed_vectors_safely_handled(self):
        ...
        negative_dimensions = {
            "chronos": -Fraction(1, 2),
            "lexicon": Fraction(3, 5),
            "surreal": Fraction(3, 2),
            "empathy": Fraction(9, 10),
            "critique": -Fraction(1, 5),
            "slapstick": Fraction(3, 10),
            "meta": Fraction(6, 5),
        }
        confidence = Fraction(17, 20)
        bonus_calc = self.humor_policy.calculate_humor_bonus(
            negative_dimensions, confidence
        )
        assert bonus_calc.bonus_factor >= 0
        assert bonus_calc.bonus_factor <= self.humor_policy.policy.max_bonus_ratio
        incomplete_dimensions = {"chronos": Fraction(4, 5), "lexicon": Fraction(3, 5)}
        bonus_calc_incomplete = self.humor_policy.calculate_humor_bonus(
            incomplete_dimensions, confidence
        )
    ...
    def test_all_mode_combinations_with_boundary_values(self):
        ...
        bonus_calc_small = self.humor_policy.calculate_humor_bonus(
            dimensions, small_confidence
        )
        assert bonus_calc_small.bonus_factor >= 0
        max_confidence = 1
        bonus_calc_max = self.humor_policy.calculate_humor_bonus(
            dimensions, max_confidence
        )
        ...
        bonus_calc_disabled = disabled_policy.calculate_humor_bonus(
            dimensions, Fraction(17, 20)
        )
