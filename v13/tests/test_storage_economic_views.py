...
def test_storage_metrics_from_events_matches_live_bytes_stored() -> None:
    ...
    engine.put_content(
        object_id="obj_metrics",
        version=1,
        content=b"M" * 100,
        metadata={"author": "u"},
        deterministic_timestamp=1,
    )

    view = compute_storage_metrics_from_events(list(engine.storage_event_log))

    # StorageEngine updates node.bytes_stored deterministically; compare against view output.
    for node_id, node in engine.nodes.items():
        # Some nodes may not be assigned for the shard if eligible_nodes empty; here it isn't.
...
def test_storage_metrics_proof_counts_deterministic() -> None:
    ...
    res = engine.put_content(
        object_id="obj_metrics_proof",
        version=1,
        content=b"Q" * 200,
        metadata={"author": "u"},
        deterministic_timestamp=2,
    )

    # Trigger one proof per shard
    ...
    # At least one node should have a proof count
