...
class TestStorageEngine:
    ...
    def test_register_storage_node(self):
        ...
        # Test that nodes are registered correctly
    ...
    def test_get_eligible_nodes(self):
        ...
        # Check that nodes are sorted deterministically
    ...
    def test_get_content(self):
        ...
        self.storage_engine.put_content(object_id, version, content, metadata, 1234567891)
    ...
    def test_get_storage_proof(self):
        ...
        # First put the content
        result = self.storage_engine.put_content(object_id, version, content, metadata, 1234567892)
        
        # Get proof for first shard
        if result["shard_ids"]:
            shard_id = result["shard_ids"][0]
            proof = self.storage_engine.get_storage_proof(object_id, version, shard_id)
            
            # Check that proof contains expected fields
            assert "merkle_root" in proof
            assert "proof" in proof
            assert "assigned_nodes" in proof
            assert len(proof["assigned_nodes"]) == 3  # REPLICATION_FACTOR
    ...
    def test_deterministic_shard_assignment(self):
        ...
        version = 1
        content = b"Deterministic test content" * 100  # Large enough to create multiple shards
        metadata = {"test": "deterministic"}
        
        # Put content twice
        result1 = self.storage_engine.put_content(object_id, version, content, metadata, 1234567895)
        result2 = self.storage_engine.put_content(object_id, version, content, metadata, 1234567895)
    ...
    def test_node_metrics_update(self):
        ...
        # Put content
        result = self.storage_engine.put_content(object_id, version, content, metadata, 1234567896)
        
        # Get the shard ID and check which nodes were assigned
        assert len(result["shard_ids"]) > 0
        shard_id = result["shard_ids"][0]
        assigned_nodes = self.storage_engine.shards[shard_id].assigned_nodes
        
        # Check that at least one assigned node has updated metrics
        ...
        for node_id in assigned_nodes:
            if node_id in self.storage_engine.nodes:
                ...
                # Bytes stored should be greater than 0 for assigned nodes
                if final_bytes.value > 0:
                    updated_node_found = True
                    break
        
        assert updated_node_found, f"No assigned nodes ({assigned_nodes}) had updated metrics"
...
class TestStorageEngineDeterminism:
    ...
    def test_deterministic_node_assignment(self):
        ...
        assert len(nodes1) == 3  # REPLICATION_FACTOR
...
class TestStorageEngineAEGISIntegration:
    ...
    def test_node_registration_without_aegis(self):
        ...
        # Register node without AEGIS context
        result = self.storage_engine.register_storage_node("test_node", "192.168.1.10", 8080)
    ...
    def test_node_registration_with_aegis(self):
        ...
        # Set AEGIS context
        self.storage_engine.set_aegis_context(self.registry_snapshot, self.telemetry_snapshot)
        
        # Register a valid node
        result = self.storage_engine.register_storage_node("node_valid", "192.168.1.11", 8080)
        
        assert result == True
        assert "node_valid" in self.storage_engine.nodes
        # Node should be marked as AEGIS verified
    ...
    def test_node_eligibility_without_aegis(self):
        ...
        # Both nodes should be eligible (since no AEGIS filtering)
        eligible_nodes = self.storage_engine.get_eligible_nodes()
    ...
    def test_node_eligibility_with_aegis(self):
        ...
        # Register both valid and revoked nodes
        ...
        # Only the valid node should be eligible
        eligible_nodes = self.storage_engine.get_eligible_nodes()
    ...
    def test_epoch_advancement(self):
        ...
        # Check initial epoch and verification status
    ...
    def test_node_status_management(self):
        ...
        # Try to change status of non-existent node
...
class TestStorageEngineEconomics:
    ...
    def setup_method(self):
        ...
        # Set nodes as AEGIS verified for testing
    ...
    def test_atr_storage_cost_calculation(self):
        ...
        large_cost = self.storage_engine._calculate_atr_storage_cost(len(large_content), large_metadata)
        assert isinstance(large_cost, BigNum128)
        assert large_cost.value > cost.value  # Larger content should cost more
    ...
    def test_put_content_with_atr_cost(self):
        ...
        # Check that result includes ATR cost
        assert "atr_cost" in result
        assert float(result["atr_cost"]) > 0
        
        # Check that total ATR fees collected is updated
    ...
    def test_nod_reward_calculation(self):
        ...
        # Node2 should get more rewards due to higher metrics
    ...
    def test_storage_economics_summary(self):
        ...
        # Put some content to generate ATR fees
        object_id = "test_object_conservation"
        version = 1
        content = b"Conservation test content" * 100  # Make it larger to generate more ATR fees
        metadata = {"author": "test_user"}
        
        self.storage_engine.put_content(object_id, version, content, metadata, 1234567890)
        
        # Calculate some NOD rewards with smaller values to ensure conservation
        self.storage_engine.nodes["node1"].bytes_stored = BigNum128.from_int(1000)  # Much smaller
