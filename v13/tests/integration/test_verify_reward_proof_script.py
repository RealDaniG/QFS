...
def test_verify_reward_proof_cli():
    ...
    timestamp = 12345
    previous_hash = 'abc12345' * 4
    entry_data = {'test': 'data', 'rewards': {'amount': 10}}
    expected_hash = cl._generate_entry_hash(entry_data, previous_hash, timestamp)
    proof_json = {'explanation': {'zero_sim_proof': {'input_state_hash': previous_hash, 'output_state_hash': expected_hash, 'timestamp': timestamp, 'entry_data_snapshot': entry_data, 'logic_version': 'test', 'pqc_cid': 'cid'}}}
...
def test_verify_reward_proof_cli_failure():
    proof_json = {'explanation': {'zero_sim_proof': {'input_state_hash': 'abc', 'output_state_hash': 'def', 'timestamp': 123, 'entry_data_snapshot': {}}}}
