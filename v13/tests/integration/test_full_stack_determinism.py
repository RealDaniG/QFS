...
def run_full_stack_simulation(seed_offset: int = 0) -> Dict[str, Any]:
    ...
    # Artistic expects integer confidence
    a_res = artistic.calculate_artistic_bonus(dims_artistic, confidence=int(0.95 * SCALE))
    
    signals_hash = get_deterministic_hash({
        "humor": h_res.bonus_factor,
        "artistic": a_res.bonus_factor
    })
    ...
    events = [
        {"type": "ContentCreated", "content_id": "c1", "user_id": "u1", "timestamp": 1000 + seed_offset},
        {"type": "RewardAllocated", "user_id": "u1", "amount_atr": 50, "timestamp": 1100 + seed_offset}
    ]
    replay.replay_events(events)
    
    explanation = helper.explain_value_node_reward(
        "w1", "u1", "evt1", 1, 
        {"ATR": "50.0 ATR"}, 
        [{"label": "Humor", "value": f"+{h_res.bonus_factor}"}], 
        [], [], 1200
    )
    
    # 4. Final State Aggregation
    final_state = {
        "core": core_hash,
        "signals": signals_hash,
        "replay_graph": str(replay.graph.users["u1"].total_rewards_atr),
        "explanation": explanation.explanation_hash
    }
...
def test_system_sensitivity():
    ...
    run1 = run_full_stack_simulation(seed_offset=0)
    run2 = run_full_stack_simulation(seed_offset=1) # Change input
