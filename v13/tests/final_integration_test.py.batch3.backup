...
def test_full_system_determinism():
    ...
    with PQC.LogContext() as log1:
        keypair1 = PQC.generate_keypair(log_list=log1, seed=seed, deterministic_timestamp=1234567890)
        keypair1_pub = keypair1.public_key.hex()
    with PQC.LogContext() as log2:
        keypair2 = PQC.generate_keypair(log_list=log2, seed=seed, deterministic_timestamp=1234567890)
    ...
    packet1 = DRV_Packet(ttsTimestamp=1700000000, sequence=1, seed='integration_test_seed', metadata={'source': 'integration_test'}, previous_hash='0' * 64)
    packet1_hash = packet1.get_hash()
    packet2 = DRV_Packet(ttsTimestamp=1700000000, sequence=1, seed='integration_test_seed', metadata={'source': 'integration_test'}, previous_hash='0' * 64)
    ...
    bundle1 = create_token_state_bundle(chr_state=chr_state, flx_state=flx_state, psi_sync_state=psi_sync_state, atr_state=atr_state, res_state=res_state, lambda1=lambda1, lambda2=lambda2, c_crit=c_crit, pqc_cid='integration_test_001', timestamp=1700000000)
    bundle1_hash = bundle1.get_deterministic_hash()
    bundle2 = create_token_state_bundle(chr_state=chr_state, flx_state=flx_state, psi_sync_state=psi_sync_state, atr_state=atr_state, res_state=res_state, lambda1=lambda1, lambda2=lambda2, c_crit=c_crit, pqc_cid='integration_test_001', timestamp=1700000000)
    ...
    with CertifiedMath.LogContext() as log1:
        result1 = hsmf.validate_action_bundle(token_bundle=bundle1, f_atr=f_atr, drv_packet_sequence=10, log_list=log1)
        hsmf_result1_valid = result1.is_valid
        hsmf_result1_dez = result1.dez_ok
        hsmf_result1_survival = result1.survival_ok
    with CertifiedMath.LogContext() as log2:
        result2 = hsmf.validate_action_bundle(token_bundle=bundle1, f_atr=f_atr, drv_packet_sequence=10, log_list=log2)
    ...
    test_scenario = {'certifiedmath_result': certifiedmath_result, 'pqc_keypair_hash': deterministic_hash(keypair1_pub), 'drv_packet_hash': packet1_hash, 'token_bundle_hash': bundle1_hash, 'hsmf_validation': {'is_valid': hsmf_result1_valid, 'dez_ok': hsmf_result1_dez, 'survival_ok': hsmf_result1_survival}}
    ...
    with PQC.LogContext() as log:
        keypair = PQC.generate_keypair(log_list=log, seed=seed, deterministic_timestamp=1234567890)
        keypair_pub = keypair.public_key.hex()
    packet = DRV_Packet(ttsTimestamp=1700000000, sequence=1, seed='integration_test_seed', metadata={'source': 'integration_test'}, previous_hash='0' * 64)
    packet_hash = packet.get_hash()
    bundle = create_token_state_bundle(chr_state=chr_state, flx_state=flx_state, psi_sync_state=psi_sync_state, atr_state=atr_state, res_state=res_state, lambda1=lambda1, lambda2=lambda2, c_crit=c_crit, pqc_cid='integration_test_001', timestamp=1700000000)
    bundle_hash = bundle.get_deterministic_hash()
    with CertifiedMath.LogContext() as log:
        result = hsmf.validate_action_bundle(token_bundle=bundle, f_atr=f_atr, drv_packet_sequence=10, log_list=log)
        hsmf_valid = result.is_valid
        hsmf_dez = result.dez_ok
        hsmf_survival = result.survival_ok
    test_scenario_replay = {'certifiedmath_result': certifiedmath_result, 'pqc_keypair_hash': deterministic_hash(keypair_pub), 'drv_packet_hash': packet_hash, 'token_bundle_hash': bundle_hash, 'hsmf_validation': {'is_valid': hsmf_valid, 'dez_ok': hsmf_dez, 'survival_ok': hsmf_survival}}
    ...
    final_hash = deterministic_hash({'scenario_hash': scenario_hash1, 'components_verified': ['CertifiedMath', 'PQC', 'DRV_Packet', 'TokenStateBundle', 'HSMF']})
    print(f'Final verification hash: {final_hash[:32]}...')
    return final_hash
if __name__ == '__main__':
    final_hash = test_full_system_determinism()
    results = {'final_verification_hash': final_hash, 'test_completed': '2025-11-17', 'system_status': 'PRODUCTION_READY'}
