...
def run_deterministic_test(test_name, run_number):
    ...
    interaction_request = InteractionRequest(user_id='test_user_123', target_id='test_post_456', content='This is a test comment')
    interaction_response = gateway.post_interaction('comment', interaction_request)
    results = {'feed_response': {'posts_count': len(feed_response.posts), 'policy_version': feed_response.policy_metadata.get('version'), 'timestamp': feed_response.policy_metadata.get('applied_at')}, 'interaction_response': {'success': interaction_response.success, 'event_id': interaction_response.event_id, 'safety_guard_passed': interaction_response.guard_results.safety_guard_passed if interaction_response.guard_results else False, 'economics_guard_passed': interaction_response.guard_results.economics_guard_passed if interaction_response.guard_results else False}}
...
def main():
    ...
    if hash1 == hash2:
        ...
        print(f"Safety guard passed: {results1['interaction_response']['safety_guard_passed']}")
        print(f"Economics guard passed: {results1['interaction_response']['economics_guard_passed']}")
        return True
    else:
