...
def test_hsmf_compliance():
    ...
    cm = CertifiedMath()
    hsmf = HSMF(cm)
    token_bundle = MockTokenStateBundle(atr_state={'atr_magnitude': BigNum128.from_int(1)}, flx_state={'magnitudes': [BigNum128.from_int(1), BigNum128.from_int(2), BigNum128.from_int(3)]}, psi_sync_state={'current_sequence': BigNum128.from_int(100)}, lambda1=BigNum128.from_int(1), lambda2=BigNum128.from_int(1), c_crit=BigNum128.from_int(0), timestamp=1234567890)
    with CertifiedMath.LogContext() as log_list:
        f_atr = BigNum128.from_int(1)
        drv_packet_sequence = 100
        try:
            result = hsmf.validate_action_bundle(token_bundle=token_bundle, f_atr=f_atr, drv_packet_sequence=drv_packet_sequence, log_list=log_list, pqc_cid='test_pqc_id', raise_on_failure=False, strict_atr_coherence=False, quantum_metadata={'test': 'metadata'})
