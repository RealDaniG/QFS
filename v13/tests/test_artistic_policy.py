...
def test_basic_bonus_calculation(policy):
    ...
    assert result.bonus_factor == Fraction(3, 10)
    assert result.cap_applied == Fraction(3, 10)
    conf_low = int(Fraction(2, 5) * SCALE)
    result_low_conf = policy.calculate_artistic_bonus(dims, confidence=conf_low)
    assert abs(result_low_conf.bonus_factor - Fraction(1, 5)) < 0.0001
...
def test_aegis_reputation_boost(policy):
    ...
    assert abs(res_new.bonus_factor - Fraction(1, 5)) < 0.0001
    ...
    assert res_vet.bonus_factor == Fraction(3, 10)
    ...
    assert abs(res_est.bonus_factor - Fraction(1, 5)) < 0.0001
