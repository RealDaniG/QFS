...
class TestAEGISAdvisoryGate:
    ...
    def test_safe_interaction_no_blocking_suggested(self):
        ...
        interaction_inputs = {
            ...
            "content": "This is a safe, family-friendly post about quantum computing."
        }
        
        observation = self.aegis_guard.observe_event(
            event_type="social_interaction",
            inputs=interaction_inputs,
            token_bundle=self.token_bundle,
            deterministic_timestamp=1234567890
        )
        
        # Check that no blocking is suggested
    ...
    def test_unsafe_content_suggests_blocking(self):
        ...
        # Test with unsafe content that should trigger high risk score
        interaction_inputs = {
            "user_id": "test_user",
            "target_id": "test_target",
            "interaction_type": "comment",
        ...
        # Check that blocking is suggested due to high risk content
        ...
        # Risk score should be high for explicit content
        assert float(observation.safety_guard_result["risk_score"]) > 0.7

    def test_spam_content_suggests_blocking(self):
        """Test that spam content suggests blocking with appropriate severity"""
        # Test with spam content that should trigger medium risk score
        interaction_inputs = {
            "user_id": "test_user",
            "target_id": "test_target",
            "interaction_type": "comment",
        ...
        # Check that blocking is suggested due to spam content
        assert observation.block_suggested is True
        assert observation.severity == "warning"  # Spam gets warning severity
        assert observation.safety_guard_result["passed"] is False
        # Risk score should be medium for spam content
        risk_score = float(observation.safety_guard_result["risk_score"])
    ...
    def test_economic_violation_suggests_blocking(self):
        ...
        # Create a ledger economics service that returns values that will cause economic violations
        class ViolatingLedgerEconomicsService:
            def get_chr_daily_totals(self):
                # Return a value that will cause economic violations
                return {"current_daily_total": BigNum128.from_int(1000000000)}  # Very high daily total
            
            def get_chr_total_supply(self):
                return {"current_total_supply": BigNum128.from_int(1000000000000)}  # Very high supply

        violating_service = ViolatingLedgerEconomicsService()
        self.aegis_guard.set_ledger_economics_service(violating_service)
        
        # Test with normal content but economic violations
        interaction_inputs = {
            "user_id": "test_user",
            "target_id": "test_target",
            "interaction_type": "like",
            "content": "This is a normal, safe comment."
        }
        ...
        # Check that blocking is suggested due to economic violation
        assert observation.block_suggested is True
    ...
    def test_feed_ranking_safe_content_no_blocking(self):
        ...
        feed_inputs = {
            ...
            "content": "This is a safe, educational post about quantum physics."
        }
        ...
        # Check that no blocking is suggested for safe feed content
        assert observation.block_suggested is False
        assert observation.severity == "info"
        assert observation.safety_guard_result["passed"] is True
    ...
    def test_feed_ranking_unsafe_content_suggests_blocking(self):
        ...
        feed_inputs = {
            ...
            "features": ["feature1", "feature2"],
            "coherence_score": "0.95",
            "content": "This post contains explicit adult material."
        }
        ...
        observation = self.aegis_guard.observe_event(
            ...
            inputs=feed_inputs,
            token_bundle=self.token_bundle,
            deterministic_timestamp=1234567895
        )
        
        # Check that blocking is suggested for unsafe feed content
        assert observation.block_suggested is True
        assert observation.severity == "critical"
        assert observation.safety_guard_result["passed"] is False
