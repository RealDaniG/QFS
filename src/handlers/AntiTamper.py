...
class AntiTamper:
    ...
    def check_memory_integrity(
        ...
        result = IntegrityCheckResult(
            ...
            is_valid=is_valid,
            timestamp=BigNum128.from_int(deterministic_timestamp),  # Changed to BigNum128
...
# Test function
def test_antitamper():
    """Test the AntiTamper implementation."""
    print("Testing AntiTamper...")
    
    # Create a CertifiedMath instance
    cm = CertifiedMath()
    
    # Create a CIR412_Handler (we won't actually trigger it in the test)
    cir412_handler = CIR412_Handler(cm)
    
    # Create a CIR302_Handler
    cir302_handler = CIR302_Handler(CertifiedMath)
    
    # Create AntiTamper
    antitamper = AntiTamper(cm, cir412_handler, cir302_handler)
    
    # Set a reference hash
    antitamper.set_reference_hash("test_section", "abc123def456")
    
    log_list = []
    
    # Test code hash check (this will fail because hashes don't match)
    # We'll catch the SystemExit to prevent the test from exiting
    try:
        is_valid = antitamper.check_code_hash(
            section_name="test_section",
            log_list=log_list,
            pqc_cid="test_antitamper_001",
            deterministic_timestamp=1234567890
        )
    except SystemExit:
        print("CIR302/CIR412 would have been triggered (SystemExit caught in test)")
        is_valid = False
    
    print(f"Code hash check result: {is_valid}")
