...
from libs.PQC import PQC, ValidationResult
...
class EpochReplayer:
    ...
    def __init__(self, pqc_log: List[Dict[str, Any]]):
        ...
        self.token_state: Dict[str, BigNum128] = {}  # token balances per user/account
        self.treasury: Dict[str, BigNum128] = {}     # treasury accounts
        self.hsmf_hashes: List[str] = []            # intermediate HSMF/Coherence hashes
    ...
    def replay(self) -> ValidationResult:
        ...
        for entry in self.log:
            ...
            if operation == "token_mint":
                ...
                # Apply transfer (using arithmetic operators)
                ...
                # Capture intermediate HSMF hash for deterministic verification
                self.hsmf_hashes.append(entry.get("entry_hash"))

            elif operation in ["generate_keypair", "sign_data", "verify_signature"]:
                # No effect on token/treasury balances
                continue

            else:
                # Unknown operations are allowed but logged
                pass

        # Compute final hash for the epoch using canonical serialization
        self.final_hash = hashlib.sha3_512(PQC._canonical_serialize(self.log)).hexdigest()
        return ValidationResult(
            is_valid=True,
            error_message=None,
            quantum_metadata={
                "replayed_entries": len(self.log),
                "final_epoch_hash": self.final_hash,
                "token_accounts": len(self.token_state),
                "treasury_accounts": len(self.treasury),
                "hsmf_checkpoints": len(self.hsmf_hashes)
            }
        )
    ...
    def export_state(self, path: str):
        ...
        """
        export_data = {
            "token_state": {k: str(v) for k, v in self.token_state.items()},
            "treasury": {k: str(v) for k, v in self.treasury.items()},
            "hsmf_hashes": self.hsmf_hashes,
            "final_epoch_hash": self.final_hash
        }
...
if __name__ == "__main__":
    # Example: Replay a single epoch from a PQC audit log
    if len(sys.argv) > 1:
        log_file = sys.argv[1]
