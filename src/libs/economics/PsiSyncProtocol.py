# PsiSyncProtocol.py
# QFS V13 - Phase 3 ΨSync Protocol (PRODUCTION-PERFECT)
# Zero-Simulation | PQC-Secured | Consensus Hardening
...
class PsiSyncProtocol:
    ...
    def _detect_outliers(self, values: List[int], shard_ids: List[str]) -> Dict[str, Any]:
        ...
        max_outliers = min(
            ...
            len(sorted_values) - (len(sorted_values) * self.OUTLIER_THRESHOLD_PERCENTILE // 100)
        )
        
        # Interquartile range calculation with INDEX BOUNDS PROTECTION
        q1_index = len(sorted_values) // 4
        q3_index = (3 * len(sorted_values)) // 4
        # 🔴 CRITICAL FIX: Index bounds protection
        q1 = sorted_values[min(q1_index, len(sorted_values)-1)][0]
        q3 = sorted_values[min(q3_index, len(sorted_values)-1)][0]
        
        # Create log list for audit trail
        log_list = []
        
        iqr = self.math.sub(q3, q1, log_list)
        outlier_threshold = self.math.mul(iqr, 15, log_list) # BigNum128(15) implied
        outlier_threshold = self.math.div_floor(outlier_threshold, 10, log_list)  # 1.5 * IQR
        ...
        for val, idx in sorted_values:
            ...
            is_outlier = (
                val < self.math.sub(q1, outlier_threshold, log_list) or 
                val > self.math.add(q3, outlier_threshold, log_list)
            )
    ...
    def _compute_consensus_metrics(
        ...
        # Enhanced consensus quality metrics
        tight_agreement_count = len([d for d in deviations if d <= epsilon_sync // 2])
        tight_agreement_count = len([d for d in deviations if d <= epsilon_sync // 2])
        # Use scaled integer (basis points 0-10000) instead of float
    ...
    def _compute_consensus_stability(self, values: List[int]) -> int:
        ...
        stability = self.math.div_floor(1000000, max(1, variance // 1000))  # Inverse relationship
        return min(stability, 1000)  # Cap at 1000
    ...
    def _compute_trimmed_mean(self, values: List[int]) -> int:
        ...
        sorted_values = sorted(values)
        trim_count = len(sorted_values) // 10  # Trim 10% from each end
    ...
    def _handle_consensus_failure(self, result: Dict[str, Any], epsilon_sync: int, cleaned_values: List[int]):
        ...
        if self.PROGRESSIVE_DEGRADATION and len(cleaned_values) >= self.MIN_CONSENSUS_SHARDS:
            ...
            # 🔴 CRITICAL FIX: Validate CIR handler methods before calling
...
def generate_psisync_evidence(
    ...
    return {
        ...
        "security_status": {
            ...
            "outlier_ratio": consensus_result["outlier_ratio"] if "outlier_ratio" in consensus_result else 0,
            "max_deviation": consensus_result["max_deviation"],
            "shard_participation": 0, # Placeholder
