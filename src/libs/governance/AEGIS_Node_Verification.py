...
class AEGIS_Node_Verifier:
    ...
    def _check_registry_entry(
        ...
        Returns:
            NodeVerificationResult
        """
        # Validate registry snapshot structure
        if "nodes" not in registry_snapshot:
            return NodeVerificationResult(
                node_id=node_id,
                is_valid=False,
                status=NodeVerificationStatus.UNVERIFIED_NO_REGISTRY_ENTRY,
                reason_code="REGISTRY_SNAPSHOT_MALFORMED",
                reason_message="Registry snapshot missing 'nodes' field"
            )
        ...
        if node_id not in registry_snapshot["nodes"]:
            return NodeVerificationResult(
                ...
                is_valid=False,
                status=NodeVerificationStatus.UNVERIFIED_NO_REGISTRY_ENTRY,
                reason_code="NODE_NOT_IN_REGISTRY",
                reason_message=f"Node {node_id} not found in AEGIS registry"
            )
        ...
        if node_entry.get("revoked", False):
            return NodeVerificationResult(
                node_id=node_id,
                is_valid=False,
                status=NodeVerificationStatus.UNVERIFIED_REVOKED,
                reason_code="NODE_REVOKED",
                reason_message=f"Node {node_id} is revoked in AEGIS registry",
                metrics={"revocation_reason": node_entry.get("revocation_reason", "unknown")}
            )
    ...
    def _check_pqc_key(
        ...
        # Check if PQC key present
        if "pqc_public_key" not in node_entry or not node_entry["pqc_public_key"]:
            return NodeVerificationResult(
                node_id=node_id,
                is_valid=False,
                status=NodeVerificationStatus.UNVERIFIED_NO_PQC_KEY,
                reason_code="PQC_KEY_MISSING",
                reason_message=f"Node {node_id} missing PQC public key"
            )
        
        # Check if PQC scheme supported
        pqc_scheme = node_entry.get("pqc_scheme", "")
        if pqc_scheme not in NodeVerificationPolicy.SUPPORTED_PQC_SCHEMES:
            return NodeVerificationResult(
                node_id=node_id,
                is_valid=False,
                status=NodeVerificationStatus.UNVERIFIED_UNSUPPORTED_PQC_SCHEME,
                reason_code="PQC_SCHEME_UNSUPPORTED",
                reason_message=f"Node {node_id} PQC scheme '{pqc_scheme}' not supported",
                metrics={
                    "pqc_scheme": pqc_scheme,
                    "supported_schemes": list(NodeVerificationPolicy.SUPPORTED_PQC_SCHEMES)
                }
            )
    ...
    def _check_uptime(
        ...
        Returns:
            NodeVerificationResult
        """
        # Validate telemetry snapshot structure
        if "nodes" not in telemetry_snapshot:
            return NodeVerificationResult(
                node_id=node_id,
                is_valid=False,
                status=NodeVerificationStatus.UNVERIFIED_TELEMETRY_MALFORMED,
                reason_code="TELEMETRY_SNAPSHOT_MALFORMED",
                reason_message="Telemetry snapshot missing 'nodes' field"
            )
        
        # Check if node has telemetry data
        if node_id not in telemetry_snapshot["nodes"]:
            return NodeVerificationResult(
                node_id=node_id,
                is_valid=False,
                status=NodeVerificationStatus.UNVERIFIED_TELEMETRY_MALFORMED,
                reason_code="NODE_NOT_IN_TELEMETRY",
                reason_message=f"Node {node_id} not found in telemetry snapshot"
            )
        ...
        if "uptime_ratio" not in node_telemetry:
            return NodeVerificationResult(
                ...
                is_valid=False,
                status=NodeVerificationStatus.UNVERIFIED_TELEMETRY_MALFORMED,
                reason_code="UPTIME_RATIO_MISSING",
                reason_message=f"Node {node_id} telemetry missing uptime_ratio"
            )
        ...
        if self.cm.lt(uptime_ratio, NodeVerificationPolicy.MIN_UPTIME_RATIO, []):
            return NodeVerificationResult(
                node_id=node_id,
                is_valid=False,
                status=NodeVerificationStatus.UNVERIFIED_LOW_UPTIME,
                reason_code="UPTIME_BELOW_THRESHOLD",
                reason_message=f"Node {node_id} uptime {uptime_ratio.to_decimal_string()} below minimum {NodeVerificationPolicy.MIN_UPTIME_RATIO.to_decimal_string()}",
                metrics={
                    "uptime_ratio": uptime_ratio.to_decimal_string(),
                    "min_required": NodeVerificationPolicy.MIN_UPTIME_RATIO.to_decimal_string()
                }
            )
    ...
    def _check_health(
        ...
        if "health_score" not in node_telemetry:
            return NodeVerificationResult(
                ...
                is_valid=False,
                status=NodeVerificationStatus.UNVERIFIED_TELEMETRY_MALFORMED,
                reason_code="HEALTH_SCORE_MISSING",
                reason_message=f"Node {node_id} telemetry missing health_score"
            )
        ...
        if self.cm.lt(health_score, NodeVerificationPolicy.MIN_HEALTH_SCORE, []):
            return NodeVerificationResult(
                node_id=node_id,
                is_valid=False,
                status=NodeVerificationStatus.UNVERIFIED_UNHEALTHY,
                reason_code="HEALTH_BELOW_THRESHOLD",
                reason_message=f"Node {node_id} health {health_score.to_decimal_string()} below minimum {NodeVerificationPolicy.MIN_HEALTH_SCORE.to_decimal_string()}",
                metrics={
                    "health_score": health_score.to_decimal_string(),
                    "min_required": NodeVerificationPolicy.MIN_HEALTH_SCORE.to_decimal_string()
                }
            )
    ...
    def _check_telemetry_coherence(
        self,
        node_id: str,
        telemetry_snapshot: Dict[str, Any]
    ) -> NodeVerificationResult:
        ...
        if schema_version != NodeVerificationPolicy.REQUIRED_TELEMETRY_SCHEMA_VERSION:
            return NodeVerificationResult(
                ...
                is_valid=False,
                status=NodeVerificationStatus.UNVERIFIED_TELEMETRY_MALFORMED,
                reason_code="TELEMETRY_SCHEMA_VERSION_MISMATCH",
                ...
                metrics={
                    ...
                    "required_version": NodeVerificationPolicy.REQUIRED_TELEMETRY_SCHEMA_VERSION
                }
            )
        
        # Check telemetry hash
        ...
        if not telemetry_hash or len(telemetry_hash) != 64:
            return NodeVerificationResult(
                ...
                is_valid=False,
                status=NodeVerificationStatus.UNVERIFIED_TELEMETRY_MALFORMED,
                reason_code="TELEMETRY_HASH_INVALID",
                reason_message=f"Telemetry hash invalid (expected 64-char SHA-256, got {len(telemetry_hash)} chars)",
                metrics={"telemetry_hash": telemetry_hash}
            )
        
        # Check for conflicting entries (duplicate node_id in different sections)
        # This is a simplified check - in production, would validate full hash chain
        node_telemetry = telemetry_snapshot["nodes"][node_id]
        if "conflict_detected" in node_telemetry and node_telemetry["conflict_detected"]:
            return NodeVerificationResult(
                node_id=node_id,
                is_valid=False,
                status=NodeVerificationStatus.UNVERIFIED_TELEMETRY_HASH_CONFLICT,
                reason_code="TELEMETRY_CONFLICT_DETECTED",
                reason_message=f"Node {node_id} has conflicting telemetry entries",
                metrics={"conflict_reason": node_telemetry.get("conflict_reason", "unknown")}
            )
        
        # Telemetry coherence check passed
        return NodeVerificationResult(
            node_id=node_id,
            is_valid=True,
            status=NodeVerificationStatus.VERIFIED
    ...
    def verify_nodes_batch(
        ...
        for node_id in sorted_node_ids:
            results[node_id] = self.verify_node(
                node_id, registry_snapshot, telemetry_snapshot, log_list
            )
...
def test_aegis_node_verification():
    ...
    registry_snapshot = {
        ...
        "nodes": {
            ...
            "node_bad_scheme": {
                "pqc_public_key": "0xfedcba987654",
                "pqc_scheme": "RSA2048",  # Not supported
                "revoked": False
            }
        }
    }
    ...
    telemetry_snapshot = {
        "schema_version": "v1.0",
        "telemetry_hash": "a" * 64,  # Valid 64-char hash
        "block_height": 12345,
        "nodes": {
            "node_valid": {
                "uptime_ratio": "0.95",  # 95% > 90% threshold
                "health_score": "0.80",  # 80% > 75% threshold
                "conflict_detected": False
            },
            "node_low_uptime": {
                "uptime_ratio": "0.85",  # 85% < 90% threshold
                "health_score": "0.80",
                "conflict_detected": False
            },
            "node_unhealthy": {
                "uptime_ratio": "0.95",
                "health_score": "0.70",  # 70% < 75% threshold
                "conflict_detected": False
            },
            "node_conflict": {
                "uptime_ratio": "0.95",
                ...
                "conflict_reason": "Duplicate entries in different shards"
            }
        }
    }
    ...
    # Add node_revoked to telemetry
    telemetry_snapshot["nodes"]["node_revoked"] = {
        "uptime_ratio": "0.95",
        "health_score": "0.80",
        "conflict_detected": False
    }
    ...
    telemetry_snapshot["nodes"]["node_no_pqc"] = {
        "uptime_ratio": "0.95",
        "health_score": "0.80",
        "conflict_detected": False
    }
    ...
    telemetry_snapshot["nodes"]["node_bad_scheme"] = {
        "uptime_ratio": "0.95",
        "health_score": "0.80",
        "conflict_detected": False
    }
    ...
    registry_snapshot["nodes"]["node_low_uptime"] = {
        "pqc_public_key": "0x11111111",
        "pqc_scheme": "Dilithium5",
        "revoked": False
    }
    ...
    registry_snapshot["nodes"]["node_unhealthy"] = {
        "pqc_public_key": "0x22222222",
        "pqc_scheme": "Dilithium5",
        "revoked": False
    }
    ...
    registry_snapshot["nodes"]["node_conflict"] = {
        "pqc_public_key": "0x33333333",
        "pqc_scheme": "Dilithium5",
        "revoked": False
    }
    ...
    bad_telemetry = {"schema_version": "v0.9"}  # Missing 'nodes' field
