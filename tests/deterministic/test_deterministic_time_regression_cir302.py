...
class CIR302Event:
    ...
    def __init__(self, violation_type: str, current_ts: int, prior_ts: int,
                 packet_sequence: Optional[int] = None, packet_hash: Optional[str] = None):
...
class TestDeterministicTimeMonotonicity:
    ...
    def test_monotonicity_first_packet_always_accepted(self):
        ...
        # No prior timestamp means this is first packet
        DeterministicTime.enforce_monotonicity(current_ts=1000, prior_ts=None)
        
        # Should not raise - first packet always accepted
    ...
    def test_monotonicity_increasing_timestamps_accepted(self):
        ...
        # Simulate sequence of increasing timestamps
    ...
    def test_monotonicity_equal_timestamps_accepted(self):
        ...
        # Equal timestamps allowed (not strictly increasing, but non-regressing)
    ...
    def test_monotonicity_regression_single_step(self):
        ...
        """
        prior_ts = 1500
        current_ts = 1400  # REGRESSION: going backward 100 ticks
    ...
    def test_monotonicity_regression_large_jump(self):
        ...
        prior_ts = 10000
        current_ts = 1000  # MAJOR REGRESSION: -9000 ticks
    ...
    def test_monotonicity_regression_by_one_tick(self):
        ...
        prior_ts = 1000
        current_ts = 999  # Minimal regression: -1 tick
    ...
    def test_monotonicity_sequence_valid_then_invalid(self):
        ...
        regression_value = 1250  # Goes backward from 1400
    ...
    def test_verify_drv_packet_mismatched_timestamps(self):
        ...
        # Should raise - timestamps don't match
...
class TestCIR302Scenarios:
    ...
    def test_cir302_scenario_replay_attack(self):
        ...
        # Normal progression
        legitimate_packets = [
            MockDRVPacket(0, 1000, "0" * 64),
            MockDRVPacket(1, 1100, "a" * 64),
            MockDRVPacket(2, 1200, "b" * 64),
        ]
        
        # Attacker replays packet 1 (timestamp 1100)
        ...
        # Replay attack should trigger regression
    ...
    def test_cir302_scenario_clock_desync(self):
        ...
        ts_after_desync = 4900  # Clock jumped backward
        
        cir302_event = None
        try:
            DeterministicTime.enforce_monotonicity(ts_after_desync, ts_before_desync)
        except ValueError as e:
            cir302_event = CIR302Event(
                violation_type="CLOCK_DESYNC",
                current_ts=ts_after_desync,
                prior_ts=ts_before_desync
            )
    ...
    def test_cir302_scenario_corrupted_packet(self):
        ...
        # Extract timestamp from valid packet
        prior_ts = DeterministicTime.canonical_time_from_packet(valid_packet)
        
        # Corrupted packet triggers regression
        ...
        try:
            ...
            cir302_event = CIR302Event(
                violation_type="CORRUPTED_PACKET",
                current_ts=current_ts,
                prior_ts=prior_ts,
                packet_sequence=corrupted_packet.sequence,
                packet_hash=corrupted_packet.previous_hash[:16] + "..."
            )
...
def generate_cir302_evidence():
    ...
    """
    # Simulate CIR-302 scenarios and collect events
    cir302_events = []
    
    # Event 1: Simple regression
    try:
        DeterministicTime.enforce_monotonicity(current_ts=1000, prior_ts=1500)
    except ValueError:
        cir302_events.append(CIR302Event(
            violation_type="SIMPLE_REGRESSION",
            current_ts=1000,
            prior_ts=1500
        ))
    
    # Event 2: Replay attack
    try:
        DeterministicTime.enforce_monotonicity(current_ts=2000, prior_ts=3000)
    except ValueError:
        cir302_events.append(CIR302Event(
            violation_type="REPLAY_ATTACK_SIMULATION",
            current_ts=2000,
            prior_ts=3000,
            packet_sequence=5,
            packet_hash="abc123...def456"
        ))
    
    # Event 3: Clock desync
    try:
        DeterministicTime.enforce_monotonicity(current_ts=10000, prior_ts=10100)
    except ValueError:
        cir302_events.append(CIR302Event(
            violation_type="CLOCK_DESYNC_SIMULATION",
            current_ts=10000,
            prior_ts=10100
        ))
    ...
    evidence = {
        ...
        "phase": "1.3",
        "test_type": "CIR-302 Regression Detection",
        "timestamp": "2025-12-11T16:00:00Z",
        "test_file": "tests/deterministic/test_deterministic_time_regression_cir302.py",
        "cir302_policy": "Any regression in logical time SHALL trigger immediate halt",
        "test_scenarios": len(cir302_events),
        "all_scenarios_triggered_correctly": len(cir302_events) == 3,
        "cir302_events": [event.to_dict() for event in cir302_events],
