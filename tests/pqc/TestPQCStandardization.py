...
from src.libs.cee.interfaces.pqc_interface import PQCInterface
...
class TestPQCStandardization:
    ...
    def test_sign_verify_round_trip(self):
        ...
        try:
            # Generate keys
            seed = b"sign_roundtrip_seed_1234567890123"  # 32 bytes
    ...
    def test_tamper_detection(self):
        ...
        try:
            # Generate keys
            seed = b"tamper_detect_seed_12345678901234"  # 32 bytes
            private_key, public_key = self.adapter.keygen(seed)
    ...
    def test_signed_message_integration(self):
        ...
        try:
            # Generate keys
            seed = b"message_integ_seed_1234567890123"  # 32 bytes
            private_key, public_key = self.adapter.keygen(seed)
            ...
            if is_valid:
                # Test tampering with different aspects
                tamper_tests = []
                
                # Tamper with payload
                tampered_payload_msg = message.model_copy(update={
                    "payload": {**payload, "amount": "999999.0"}
                })
                ...
                if all_fail:
                    result = {
                        ...
                        "tick": message.tick,
                        "tamper_tests": [f"{name}: {'REJECTED' if not res else 'ACCEPTED'}" for name, res in tamper_tests],
                        "backend": self.backend_name
                    }
                    self.pass_count += 1
                    print(f"  ✅ PASS: {test_name}")
...
def demo_open_agi_reference_scenario():
    ...
    evidence = {
        ...
        "summary": {
            ...
            "invalid_signatures": sum(1 for r in verification_results if not r["valid"])
        }
    }
