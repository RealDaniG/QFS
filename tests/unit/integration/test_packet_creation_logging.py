...
try:
    ...
    from DRV_Packet import (
        DRV_Packet, get_drv_packet_audit_log, get_drv_packet_audit_hash, 
        clear_drv_packet_audit_log, ValidationResult, ValidationErrorCode
    )
...
def test_deterministic_audit_log():
    ...
    # Create first packet and capture audit log
    clear_drv_packet_audit_log()
    packet1 = DRV_Packet(
        ttsTimestamp=tts_timestamp,
        sequence=sequence,
    ...
    # Create second identical packet and capture audit log
    clear_drv_packet_audit_log()
    packet2 = DRV_Packet(
        ttsTimestamp=tts_timestamp,
        sequence=sequence,
...
def test_audit_log_serialization():
    ...
    # Get audit log and hash
    audit_log = get_drv_packet_audit_log()
    audit_hash = get_drv_packet_audit_hash()
    
    # Manually serialize and hash the audit log
...
def test_cross_check_with_sdk_compatibility():
    ...
    # Create packet with all possible fields
    packet = DRV_Packet(
        ttsTimestamp=1700000000,
        sequence=1,
        seed="sdk_compatibility_test",
        metadata={"source": "sdk_test", "version": "1.0", "test_id": "CROSS_CHECK_001"},
        previous_hash="2222222222222222222222222222222222222222222222222222222222222222",
        pqc_cid="SDK_TEST_001",
        quantum_metadata={
            "quantum_source_id": "QSO-2025-11-16-SDK-001",
            "vdf_output_hash": "sdk_vdf_hash_1234567890",
            "entropy_pool": "sdk_entropy_pool_data",
            "timestamp": 1700000000,
            "sdk_version": "1.0.0"
        }
    )
    ...
    # Verify all required fields are present for SDK cross-check
    assert len(audit_log) == 1
    entry = audit_log[0]
    
    # Required fields for SDK cross-check
    ...
    # Verify packet hash can be computed from details
    # Reconstruct packet from details to verify consistency
    reconstructed_packet = DRV_Packet(
        ttsTimestamp=details["ttsTimestamp"],
    ...
    # While we can't directly compare packet hashes (since they include signatures),
    # we can verify the core data is consistent
...
def test_audit_log_clearing():
    ...
    # Verify audit log is empty again
    audit_log = get_drv_packet_audit_log()
    assert len(audit_log) == 0, f"Audit log should be empty after clearing, got {len(audit_log)}"
